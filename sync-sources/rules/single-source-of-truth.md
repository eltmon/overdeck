...
No route handler, agent, script, or hook may read or write the SQLite DB, the state worktree,
`state.json`, or GitHub issue state **directly** for canonical state. The DB is a disposable cache,
rebuilt from the sources of truth (GitHub, `records/` on `overdeck-state`, JSONL transcripts, tmux); the two doors
are the only things that touch a store, and the writer mirrors durable state back to git so it travels.
The state branch is checked out at `${OVERDECK_HOME}/state/<project>/`.
Unmigrated projects only continue through the legacy `<projectRoot>/.pan/`
surface until a valid marker lands on `origin/overdeck-state`.
...
direct access. A CI guard fails the build on direct DB / state-worktree / `state.json` / GitHub access for
