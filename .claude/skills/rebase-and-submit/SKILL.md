...
description: Atomic submit flow for a work agent. Covers the first submission (`pan work done`) AND the re-review-after-feedback path (`pan work request-review`). Use this whenever you finish implementation OR finish addressing specialist feedback — never hand-roll curl, git push, or PR create commands.
author: Panopticon
version: 1.1.0
triggers:
  - rebase and submit
...
| Situation | Command |
|---|---|
| First submission of this issue (no PR exists yet, or no prior review feedback) | `pan work done <issue-id>` |
| You just fixed specialist feedback (review, test, or verification-gate failure) and need to re-enter the pipeline | `pan work request-review <issue-id>` |

If you are unsure which applies, check `.planning/feedback/`. If it contains any files that describe failures you have just addressed, you are in the re-review case — use `pan work request-review`.
...
- You've finished implementation for the first time → `pan work done`
- You addressed review/test/verification feedback and need to resubmit → `pan work request-review`
- Your PR was closed or stale and you need to create a fresh one → `pan work done`
- You were told "run pan work done" or "request re-review" via a tell/nudge → whichever was named
- Specialist feedback arrived (`.planning/feedback/NNN-*.md`) and you believe you've resolved it → `pan work request-review`
...
### Step 3a: First submission — `pan work done <issue-id>`
...
### Step 3b: Re-review after feedback — `pan work request-review <issue-id>`

```bash
pan work request-review <issue-id> -m "Addressed feedback: <short summary>"
```

`pan work request-review` handles everything internally:
...
- **Do NOT** call `pan work done` a second time after feedback — that is the wrong command once a PR exists. Use `pan work request-review`.
