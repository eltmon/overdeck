# Working on Issue: PAN-1579

**Workspace:** /home/eltmon/Projects/panopticon-cli/workspaces/feature-pan-1579


## CRITICAL: Stay In Your Workspace

**You MUST only operate within your workspace directory: `/home/eltmon/Projects/panopticon-cli/workspaces/feature-pan-1579`**

**Your branch is `feature/pan-1579`.** A worktree does NOT lock you to a branch — its HEAD can drift to `main` or become detached after a stray `git checkout`, a failed rebase, or a botched `pan sync-main`. Before your first edit and any time you resume, run:

```bash
git branch --show-current
```

It MUST return `feature/pan-1579`. If it returns anything else — `main`, a different feature branch, or empty output (detached HEAD) — STOP. Do not edit, do not commit. Run `git status` and `git rev-parse --show-toplevel` to confirm where you are, then report the mismatch via `pan tell` to your supervisor. Commits made on the wrong branch will land in the wrong place and require manual recovery.
...
- [closed] pan-1579: Remove better-sqlite3 dependency, bump engines, clean build externals and comments (capstone) (workspace-7298x)
- [closed] pan-1579: Migrate test files off better-sqlite3 to the adapter / node:sqlite (workspace-aw33a)
- [closed] pan-1579: Swap residual type-only better-sqlite3 imports/annotations to the adapter type (workspace-tzzpb)
- [closed] pan-1579: Migrate feature registry storage (registry/feature-registry-storage.ts) to the driver adapter (workspace-trvcx)
- [closed] pan-1579: Migrate memory FTS DB (memory/fts-db.ts) to the driver adapter (workspace-hs6kf)
- [closed] pan-1579: Migrate dashboard cache-service DB to the driver adapter (workspace-cjl8y)
- [closed] pan-1579: Migrate artifacts index store (artifacts/index-store.ts) to the driver adapter (workspace-nd6ap)
- [closed] pan-1579: Migrate docs index DB (docs/query.ts + docs/index-builder.ts) to the driver adapter (workspace-9j6v4)
- [closed] pan-1579: Migrate cloister.db (cloister/database.ts) to the driver adapter (workspace-1vyjh)
- [closed] pan-1579: Migrate panopticon.db chokepoint (database/index.ts) to the driver adapter (workspace-6k1n9)
- [closed] pan-1579: Delete native-sqlite-guard.ts and its two call sites (workspace-cazta)
- [closed] pan-1579: Create shared SQLite driver adapter (driver.ts) with pragma/transaction shims and runtime selection (workspace-rqgx1)

Follow the per-bead workflow in the mandatory section below.

**IMPORTANT:** Always use `-l pan-1579` with `bd ready` and `bd list` to scope
...
- `bd list --issue PAN-1579` — the `--issue` flag does NOT exist. Use `bd list -l pan-1579` or `bd list --title-contains "PAN-1579"`
...
item): run `bd list -l pan-1579` and close everything that is done.
...
3. If done, signal completion immediately:
   ```bash
   pan done PAN-1579 -c "Work already complete from previous session"
   ```
...
1. `bd ready -l pan-1579` — find the next unblocked bead for THIS issue
...
9. If `metadata.requiresInspection === true`, run `pan inspect PAN-1579 --bead <bead-id>` for `inspectionDepth: "fast"` or omitted, or add `--deep` for `inspectionDepth: "deep"`, then wait for the verdict via `pan tell`.
10. On `INSPECTION BLOCKED`: fix with a new commit, `bd close` again, then re-run the same inspection. On `INSPECTION ERROR`: report it to your supervisor via `pan tell PAN-1579 "<summary>"`, STOP advancing to the next bead, and do not treat it as a normal spec-fix loop.
...
- **End your turn with a multi-paragraph "what I just did" summary and idle.** Summaries cost tokens and stall the pipeline. Close the bead with `bd close --reason="…"`, then immediately call `bd ready -l pan-1579` and start the next one in the same turn.
...
- The only acceptable end state is: all beads closed (with passing inspections on flagged beads), all tests pass, all code committed and pushed, and `pan done PAN-1579` called.

**You have unlimited time and context. Use it. Do not be lazy.**

**CRITICAL: NEVER stop working without calling `pan done`.** If you have remaining tasks, keep going — do NOT end your turn to "wait for input." If ALL tasks are complete, you MUST call `pan done PAN-1579 -c "summary"` as your final action. Ending your turn without either continuing work or calling `pan done` is a failure state that blocks the entire pipeline.
...
pan done PAN-1579 -c "Brief summary"      # Signal completion — creates GitHub PR
```
...
**After `pan done`, you remain on standby.** Your tmux session stays alive and the human can send you UAT tweaks via `pan tell PAN-1579 "message"` at any time before merge. You do NOT need to be "resumed" — `pan tell` auto-wakes you. If review fails, feedback is delivered the same way.
