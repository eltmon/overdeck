# Working on Issue: PAN-1629

**Workspace:** /home/eltmon/Projects/panopticon-cli/workspaces/feature-pan-1629


## CRITICAL: Stay In Your Workspace

**You MUST only operate within your workspace directory: `/home/eltmon/Projects/panopticon-cli/workspaces/feature-pan-1629`**

**Your branch is `feature/pan-1629`.** A worktree does NOT lock you to a branch — its HEAD can drift to `main` or become detached after a stray `git checkout`, a failed rebase, or a botched `pan sync-main`. Before your first edit and any time you resume, run:

```bash
git branch --show-current
```

It MUST return `feature/pan-1629`. If it returns anything else — `main`, a different feature branch, or empty output (detached HEAD) — STOP. Do not edit, do not commit. Run `git status` and `git rev-parse --show-toplevel` to confirm where you are, then report the mismatch via `pan tell` to your supervisor. Commits made on the wrong branch will land in the wrong place and require manual recovery.
...
- [closed] pan-1629: Regression test: N concurrent beads operations all succeed (reproduces the RUN-14 failure) (workspace-wq64z)
- [closed] pan-1629: Verify `pan start --host --yes` returns promptly after spawn (the Related note) (workspace-nhtdm)
- [closed] pan-1629: Never report 'planning must create beads' on a transient bd failure; emit an honest locked-retry message (workspace-av2jb)
- [closed] pan-1629: Route the `pan start` beads-gate query and `beads-query.ts` through lock + retry (get the LIVE count, not stale JSONL) (workspace-80442)
- [closed] pan-1629: Route create-beads/dedup (`clearBeadsForIssue` → `bd list`) through the cross-process lock + retry (workspace-cw8vp)
- [closed] pan-1629: Add a cross-process bd lock + bounded-retry wrapper (mirroring restart-lock.ts) (workspace-giub6)
- [closed] pan-1629: Add a conservative transient/lock bd-error classifier (workspace-cjxa4)

Follow the per-bead workflow in the mandatory section below.

**IMPORTANT:** Always use `-l pan-1629` with `bd ready` and `bd list` to scope
...
- `bd list --issue PAN-1629` — the `--issue` flag does NOT exist. Use `bd list -l pan-1629` or `bd list --title-contains "PAN-1629"`
...
item): run `bd list -l pan-1629` and close everything that is done.
...
# Active vBRIEF Slice: PAN-1629
Plan: Substrate: concurrent 'pan start' invocations fail — bd list lock contention spawns only some agents (pan-1629) @ sequence 8

## Issue Objective
Firing multiple `pan start <id> --host --yes` in parallel (the Flywheel's core job) causes some to fail at the beads-resolution step. `pan start` runs `bd list --json -l pan-<id> --status all --limit 0` to resolve the task breakdown; under concurrent invocation that command fails on Dolt's file lock, and `pan start` then aborts with a MISLEADING 'No beads tasks found … Planning must create a task breakdown' message even though the beads exist (observed RUN-14: PAN-1613/1614/1617 failed, PAN-1615/1616 succeeded; staggering 12s apart fixed it). Root cause: the existing `withBdMutexPromise` is an IN-PROCESS mutex that serializes bd calls within one Node process but does nothing across the separate child processes the Flywheel spawns. The JSONL fallbacks (countBeadsTasks, queryBeadsForIssuePromise) failed to mask it because the committed worktree `.beads/issues.jsonl` snapshot is stale relative to live Dolt (commit 0e64035d2). A related symptom: `pan start --host --yes` does not return promptly under contention because its bd calls block on the Dolt lock.

## Current Work Set
- transient-bd-classifier: Add a conservative transient/lock bd-error classifier [pending] difficulty=simple

## Target Item
- transient-bd-classifier: Add a conservative transient/lock bd-error classifier [pending] difficulty=simple
- Action: Add a pure predicate `isTransientBdError(error): boolean` (e.g. in a new src/lib/bd-process-lock.ts or src/lib/bd-mutex.ts) that returns true ONLY for Dolt/beads lock-contention signatures observed when a concurrent process holds the DB lock — and false for everything else, especially 'genuine empty result' (which is exit 0 + `[]`, never an error). First confirm the real Dolt lock stderr text by running concurrent `bd list` against a throwaway temp DB, then match those signatures conservatively (lock held / database is locked / resource temporarily unavailable / could not acquire). Cover the error shapes execFile/execFileSync produce (error.stderr, error.message, error.code). Default to NON-transient when uncertain.

## Direct Unlocks / Dependents
- cross-process-bd-lock: Add a cross-process bd lock + bounded-retry wrapper (mirroring restart-lock.ts) [pending] difficulty=complex
- honest-transient-error-messaging: Never report 'planning must create beads' on a transient bd failure; emit an honest locked-retry message [pending] difficulty=medium

## Acceptance Criteria
- transient-bd-classifier.ac1: isTransientBdError returns true for confirmed Dolt lock-contention stderr signatures [pending]
- transient-bd-classifier.ac2: isTransientBdError returns false for a successful-but-empty bd list result and for genuine fatal errors (corrupt db, command-not-found) [pending]
- transient-bd-classifier.ac3: Unit tests cover execFile error shapes (stderr string, message, code) and the empty-result false case [pending]
- transient-bd-classifier.ac4: The confirmed Dolt lock stderr text is documented in a code comment citing how it was reproduced [pending]
...
3. If done, signal completion immediately:
   ```bash
   pan done PAN-1629 -c "Work already complete from previous session"
   ```
...
1. `bd ready -l pan-1629` — find the next unblocked bead for THIS issue
...
9. If `metadata.requiresInspection === true`, run `pan inspect PAN-1629 --bead <bead-id>` for `inspectionDepth: "fast"` or omitted, or add `--deep` for `inspectionDepth: "deep"`, then wait for the verdict via `pan tell`.
10. On `INSPECTION BLOCKED`: fix with a new commit, `bd close` again, then re-run the same inspection. On `INSPECTION ERROR`: report it to your supervisor via `pan tell PAN-1629 "<summary>"`, STOP advancing to the next bead, and do not treat it as a normal spec-fix loop.
...
- **End your turn with a multi-paragraph "what I just did" summary and idle.** Summaries cost tokens and stall the pipeline. Close the bead with `bd close --reason="…"`, then immediately call `bd ready -l pan-1629` and start the next one in the same turn.
...
- The only acceptable end state is: all beads closed (with passing inspections on flagged beads), all tests pass, all code committed and pushed, and `pan done PAN-1629` called.

**You have unlimited time and context. Use it. Do not be lazy.**

**CRITICAL: NEVER stop working without calling `pan done`.** If you have remaining tasks, keep going — do NOT end your turn to "wait for input." If ALL tasks are complete, you MUST call `pan done PAN-1629 -c "summary"` as your final action. Ending your turn without either continuing work or calling `pan done` is a failure state that blocks the entire pipeline.
...
pan done PAN-1629 -c "Brief summary"      # Signal completion — creates GitHub PR
```
...
**After `pan done`, you remain on standby.** Your tmux session stays alive and the human can send you UAT tweaks via `pan tell PAN-1629 "message"` at any time before merge. You do NOT need to be "resumed" — `pan tell` auto-wakes you. If review fails, feedback is delivered the same way.
