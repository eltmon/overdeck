# Working on Issue: PAN-1787

**Workspace:** /home/eltmon/Projects/panopticon-cli/workspaces/feature-pan-1787


## CRITICAL: Stay In Your Workspace

**You MUST only operate within your workspace directory: `/home/eltmon/Projects/panopticon-cli/workspaces/feature-pan-1787`**

**Your branch is `feature/pan-1787`.** A worktree does NOT lock you to a branch — its HEAD can drift to `main` or become detached after a stray `git checkout`, a failed rebase, or a botched `pan sync-main`. Before your first edit and any time you resume, run:

```bash
git branch --show-current
```

It MUST return `feature/pan-1787`. If it returns anything else — `main`, a different feature branch, or empty output (detached HEAD) — STOP. Do not edit, do not commit. Run `git status` and `git rev-parse --show-toplevel` to confirm where you are, then report the mismatch via `pan tell` to your supervisor. Commits made on the wrong branch will land in the wrong place and require manual recovery.
...
- [open] pan-1787: Graceful restart: 60s grace + Escape interrupt for TUI harnesses (workspace-5mymw)
- [open] pan-1787: Brand logos + registry labels in shared ModelPicker (workspace-57wb7)
- [open] pan-1787: Brand logos in Settings → Providers (workspace-30qkt)
- [open] pan-1787: Brand logos in composer ModelPicker (chat) (workspace-em4re)
- [in_progress] pan-1787: Settings → Providers: honest Default(<built-in>) harness selects (workspace-h73w7)
- [open] pan-1787: Reconcile legacy specialist_harnesses as deprecated role-tier alias (workspace-faps0)
- [open] pan-1787: Dashboard start route: validate, fail fast, forward explicit harness only (workspace-wjwqy)
- [closed] pan-1787: Migrate planning kickoff to resolveHarness() (workspace-925df)
- [closed] pan-1787: Migrate agents.ts spawn sites to resolveHarness() (workspace-75853)
- [open] pan-1787: Conversation feed: HarnessLogo instead of AGENT_ICON_PROVIDER map (workspace-ua0y6)
- [open] pan-1787: Agent dock: harness-based marks, π for pi (workspace-su46y)
- [open] pan-1787: Brand logos + registry labels in Settings → Roles (workspace-f2k42)
- [open] pan-1787: Real SVG marks for the eight letter-badged providers (workspace-mys83)
- [open] pan-1787: Fresh-session invariant: drop --resume across model/harness change (workspace-joq5j)
- [closed] pan-1787: Settings API: expose built-in defaults + delete cleared provider harness keys (workspace-bnhed)
- [closed] pan-1787: Unified resolveHarness() in new src/lib/harness-resolve.ts (workspace-jeqcg)
- [open] pan-1787: Delete dead components/Settings/Provider/ directory (workspace-kl4k1)
- [open] pan-1787: Shared branding registry: PROVIDER_BRANDS + HARNESS_BRANDS (workspace-a9852)
- [open] pan-1787: Settings → Roles: harness override select on every role card (workspace-bzy4x)
- [open] pan-1787: Label live-conversation harness switching experimental (workspace-7b92h)
- [open] pan-1787: Persist session-origin model+harness beside claudeSessionId (workspace-uci90)
- [open] pan-1787: sendEscapeKeyAsync tmux helper (workspace-98zxp)
- [open] pan-1787: Fix stale --harness help strings + widen strike harness type + sync wrapper skills (workspace-zu7cs)
- [closed] pan-1787: Built-in defaultHarness on the provider registry (workspace-160xb)

Follow the per-bead workflow in the mandatory section below.

**IMPORTANT:** Always use `-l pan-1787` with `bd ready` and `bd list` to scope
...
- `bd list --issue PAN-1787` — the `--issue` flag does NOT exist. Use `bd list -l pan-1787` or `bd list --title-contains "PAN-1787"`
...
item): run `bd list -l pan-1787` and close everything that is done.
...
# Active vBRIEF Slice: PAN-1787
Plan: Provider default harnesses, role-level harness overrides, and real brand logos (pan-1787) @ sequence 2

## Issue Objective
Every unset harness silently falls back to claude-code even for OpenAI/Google/Kimi models; harness resolution logic is duplicated inline at five spawn sites; role-level harness overrides exist in schema but only Flywheel has UI; legacy specialist_harnesses (PAN-636) is an undocumented parallel config; live model/harness switching has no fresh-session guarantee on the JSONL --resume path and only a 30s grace; only 2 of 10 providers have real brand marks and pi agents render without a brand mark.

## Current Work Set
- provider-defaults: Built-in defaultHarness on the provider registry [pending] difficulty=simple

## Target Item
- provider-defaults: Built-in defaultHarness on the provider registry [pending] difficulty=simple
- Action: Per PRD WI-1: in src/lib/providers.ts add `defaultHarness: RuntimeName` to the ProviderConfig interface (line 29; import RuntimeName from './runtimes/types.js'), set it on all 10 PROVIDERS entries (lines 61-163: anthropic→'claude-code', openai→'codex', google/kimi/minimax/zai/mimo/openrouter/nous/dashscope→'pi'), and export `getBuiltInDefaultHarness(provider: ProviderName): RuntimeName` returning the registry value with 'claude-code' for unknown providers. Add unit coverage in tests/unit/lib/providers.test.ts (create if absent, mirroring tests/unit/lib/ layout).

## Direct Unlocks / Dependents
- resolve-harness-core: Unified resolveHarness() in new src/lib/harness-resolve.ts [pending] difficulty=complex
- settings-api-builtin-defaults: Settings API: expose built-in defaults + delete cleared provider harness keys [pending] difficulty=medium

## Acceptance Criteria
- provider-defaults.ac1: Given the PROVIDERS registry, every entry stores a defaultHarness (anthropic→claude-code, openai→codex, the other eight→pi) and typecheck fails if an entry omits the now-required field [pending]
- provider-defaults.ac2: getBuiltInDefaultHarness('openai') returns 'codex', ('anthropic') returns 'claude-code', ('kimi') returns 'pi', and an unknown provider id returns 'claude-code' [pending]
- provider-defaults.ac3: A unit test in tests/unit/lib/providers.test.ts validates the three-way mapping for all 10 providers and the unknown-provider claude-code fallback [pending]
...
3. If done, signal completion immediately:
   ```bash
   pan done PAN-1787 -c "Work already complete from previous session"
   ```
...
1. `bd ready -l pan-1787` — find the next unblocked bead for THIS issue
...
9. If `metadata.requiresInspection === true`, run `pan inspect PAN-1787 --bead <bead-id>` for `inspectionDepth: "fast"` or omitted, or add `--deep` for `inspectionDepth: "deep"`, then wait for the verdict via `pan tell`.
10. On `INSPECTION BLOCKED`: fix with a new commit, `bd close` again, then re-run the same inspection. On `INSPECTION ERROR`: report it to your supervisor via `pan tell PAN-1787 "<summary>"`, STOP advancing to the next bead, and do not treat it as a normal spec-fix loop.
...
- **End your turn with a multi-paragraph "what I just did" summary and idle.** Summaries cost tokens and stall the pipeline. Close the bead with `bd close --reason="…"`, then immediately call `bd ready -l pan-1787` and start the next one in the same turn.
...
- The only acceptable end state is: all beads closed (with passing inspections on flagged beads), all tests pass, all code committed and pushed, and `pan done PAN-1787` called.

**You have unlimited time and context. Use it. Do not be lazy.**

**CRITICAL: NEVER stop working without calling `pan done`.** If you have remaining tasks, keep going — do NOT end your turn to "wait for input." If ALL tasks are complete, you MUST call `pan done PAN-1787 -c "summary"` as your final action. Ending your turn without either continuing work or calling `pan done` is a failure state that blocks the entire pipeline.
...
pan done PAN-1787 -c "Brief summary"      # Signal completion — creates GitHub PR
```
...
**After `pan done`, you remain on standby.** Your tmux session stays alive and the human can send you UAT tweaks via `pan tell PAN-1787 "message"` at any time before merge. You do NOT need to be "resumed" — `pan tell` auto-wakes you. If review fails, feedback is delivered the same way.
