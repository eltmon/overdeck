...
- **Cycle count:** The codebase had **77** cycles at implementation time (production source only, excluding test and `.d.ts` files), not 74 as estimated in the PRD.
- **Exclusions:** `lint:circular` passes `--exclude '\.(test|spec)\.(ts|tsx)$|\.d\.ts$'` to madge so the ratchet only considers production source cycles.
...
- `scripts/circular-deps-baseline.txt` — 77 canonical cycles, sorted (production source only; test/`.d.ts` files excluded).
