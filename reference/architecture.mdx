...
Because the `projects.yaml` registry (see [Directory Structure](#directory-structure)) lets one Panopticon instance manage many repositories at once, the dashboard's **God View** aggregates every project's agents and conversations into a single cross-project surface — one place to see what every agent is doing, regardless of which repo it belongs to.
