...
**State Cleanup:** When an agent is killed or aborted (`pan kill`), Overdeck automatically cleans up its state files to prevent stale data from affecting future runs.
...
- **Same workflow** — `pan admin remote` commands mirror local workspace operations

Remote workspace management:
```bash
pan admin remote init        # Initialize the Fly.io app for workspace machines
pan admin remote setup       # Install flyctl and configure auth
pan admin remote status      # Check Fly.io connection and machine status
pan admin remote resources   # Show RAM/disk usage across VMs
```

## Deep Wipe

For issues that get into a stuck or inconsistent state, use `pan wipe` to completely reset:

```bash
pan wipe MIN-123                # Asks for confirmation
pan wipe MIN-123 -y             # Skip confirmation
```
...
- Workspace files and the issue's branches
- Issue tracker status (reset to Backlog/Open)

**Dashboard UI:** When aborting planning, choose **Reset to Todo** from the issue's right-click menu for a complete reset. (`pan wipe` remains available from the CLI.)
...
The dashboard's **Metrics** page shows today's cost, agent health counts and the top spenders, served by `/api/metrics/summary` and `/api/metrics/costs` from Cloister's cost tracking.

<ThemedImage
  light="/images/dashboard/metrics-light.png"
  dark="/images/dashboard/metrics-dark.png"
  alt="Metrics page: runtime comparison and performance analytics across models"
/>
...
|------|---------|----------|
| **Production** | `pan up` | Daily usage, HTTPS at https://overdeck.localhost |
