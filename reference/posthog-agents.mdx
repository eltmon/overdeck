...
release job. `--delete-after` prevents source maps from entering the published
package. If `POSTHOG_CLI_API_KEY` is absent, Overdeck prints a warning and lets
the release continue without a network call.
