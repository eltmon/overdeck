...
description: How Panopticon builds the prompts it sends to Claude — the Mustache-based loader, frontmatter contract, template catalogue, and authoring guide.
---
...
The ten templates currently under `src/lib/cloister/prompts/`:
...
| `inspect-agent.md` | Inspect specialist (`buildInspectPrompt`) | Per-bead spec verification mid-implementation. Still uses the legacy ad-hoc replace path — see [Legacy templates](#legacy-templates). |
