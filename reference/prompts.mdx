...
a PR, the planning agent that generates an xBRIEF — is built from a versioned
...
See [BUILD.md](https://github.com/eltmon/overdeck/blob/main/docs/BUILD.md#specialist-prompt-templates)
...
---
name: work
description: Work-agent prompt — implementation-phase instructions, per-task workflow, finalization steps.
requires:
  - ISSUE_ID
  - ISSUE_TITLE
  - BRANCH
...
Templates that drive the **work agent** — the specialist that implements xBRIEF checklist items.

| Template | Used by | Purpose |
|----------|---------|---------|
| `work.md` | Work agent (`spawnSpecialist`, `buildWorkAgentPrompt`) | Implementation instructions: claim one xBRIEF item, implement it, commit and push, run `pan task done`, then continue through quality gates and finalization. |
| `resume-work.md` | `buildResumePrompt` | Context-rich restart message sent when a stopped work agent resumes. Includes continue state, open xBRIEF items, pending feedback, and next-step instructions. |
| `handoff-to-work.md` | `buildHandoffPrompt` | Model-to-model handoff: wraps serialized git state, xBRIEF item state, continue state, and summaries with a preamble explaining who took over and why. |

### Planning agent templates

Templates for the **planning agent**, the discovery-phase agent that converts a PRD or raw issue into an xBRIEF plan.

| Template | Used by | Purpose |
|----------|---------|---------|
| `planning.md` | Planning agent (`buildPlanningPrompt`) | Discovery session: explore the codebase, ask questions through AskUserQuestion, write the legacy-compatible workspace draft named by `PAN_SPEC_FILENAME` plus `.pan/continue.json`, then run `pan plan finalize` to create the canonical `specs/*.xbrief.json`. Never implements code. |
...
| Template | Used by | Purpose |
|----------|---------|---------|
| `review.md` | Review specialist (wake path) | Strict code review against xBRIEF acceptance criteria; pass/fail decision posted back to the dashboard. |
...
| Template | Used by | Purpose |
|----------|---------|---------|
| `inspect-agent.md` | Inspect specialist (`buildInspectPrompt`) | Per-task spec verification mid-implementation. Still uses the legacy `readFileSync` + `String.replace` path. |
...
- **Implementation:** [`src/lib/cloister/prompts.ts`](https://github.com/eltmon/overdeck/blob/main/src/lib/cloister/prompts.ts) — the full loader (~180 lines).
- **Templates:** [`src/lib/cloister/prompts/`](https://github.com/eltmon/overdeck/tree/main/src/lib/cloister/prompts) — the current catalogue.
- **Tests:** [`src/lib/cloister/__tests__/prompts.test.ts`](https://github.com/eltmon/overdeck/blob/main/src/lib/cloister/__tests__/prompts.test.ts) — loader contract tests + live smoke tests.
- **Build pipeline:** [`docs/BUILD.md`](https://github.com/eltmon/overdeck/blob/main/docs/BUILD.md#specialist-prompt-templates) — how templates are copied into the bundled dashboard.
- **Specialist pipeline:** [`docs/SPECIALIST_WORKFLOW.md`](https://github.com/eltmon/overdeck/blob/main/docs/SPECIALIST_WORKFLOW.md) — where each template sits in the lifecycle (work → review → test → merge).
