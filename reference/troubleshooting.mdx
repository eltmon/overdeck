...
`pan start` runs a plan-freshness preflight right before it spawns a work
agent: it refuses a missing `files_scope` path only when a commit on the
workspace's `HEAD` deleted that path *after* the plan was created. If the
listed files really did move or get deleted since planning, re-plan against
the current tree with `pan plan <ID>`. If the files are ones the plan
legitimately creates or restores, the preflight should already let it
through — `--skip-freshness` spawns the agent anyway without waiting for a
re-plan.
