...
- `config` with `--get`, `--get-all`, `--get-regexp` or `--list`;
- `remote` bare, `remote -v`, `remote show` and `remote get-url`;
- `branch --show-current`, `branch --list`, and a bare `branch` listing.
...
bypasses it, and it does not restrict file writes or network access at all.
...
parent may still need the work. They are removed when the issue is torn down
(close-out, close, approve) or its residue is reaped: `git worktree remove
--force`, then `git worktree prune`, then the branch. A teardown that keeps the
issue's branches deletes only worker branches with no commits of their own.
