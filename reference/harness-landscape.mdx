...
description: A survey of coding-agent harnesses Overdeck could drive, and how each one handles skills, MCP, and AGENTS.md
---
...
<Note>
**This is a planning survey, not a support matrix.** Overdeck supports five
harnesses today — `claude-code`, `ohmypi`, `codex`, native `acp`, and native
`kimi-code` (see [Harnesses](/configuration/harnesses) for the operational
guide). Kimi Code CLI is wired two ways: as the first agent through the `acp`
protocol harness, and natively through `kimi-code`, which drives Kimi's own
terminal UI directly with no ACP host in between. Other tools on this page
remain surveyed for evaluation until their provider adapters land. Facts are
current as of May 2026 unless a profile says otherwise and should be
re-verified before any decision. Items flagged **⚠ verify** could not be
cleanly confirmed.
</Note>
...
| `kimi-code` | **Supported** (native, Kimi-only) | `src/lib/runtimes/kimi-code.ts` |
| `opencode` | **Supported** (Go and Zen) | `src/lib/acp/opencode.ts` + shared ACP runtime |

The native `acp` harness is a reusable protocol integration, but version one
uses the `acp` harness name for Kimi Code CLI. OpenCode Go and Zen use the
`opencode` harness name with the shared ACP transport. `kimi-code` is a
separate, capability-layer harness: it drives Kimi's own terminal UI and wire
protocol directly, reaching Kimi-specific behavior the ACP lowest-common-
denominator surface can't. The remaining profiles are surveyed for planning
unless their status explicitly says supported.
...
| **OpenCode** | **Wired now** through `opencode acp` | Go and Zen; sign in with `opencode auth login`. |
...
Kimi K3 supports Low, High, and Max effort through both native CLI and ACP.
Overdeck initializes managed K3 sessions at High unless another effort was
selected. Native launches use the per-process `KIMI_MODEL_THINKING_EFFORT`
override; ACP sets the session's `thinking` option after selecting the model.
Overdeck does not edit the shared Kimi configuration. Saved Medium maps to High,
and Extra High maps to Max. K2.7 Code and HighSpeed always think and do not
offer adjustable effort.

For a running ACP session, the host accepts the authenticated `set-effort`
operation with an `effort` string. It acknowledges the normalized effort only
after Kimi accepts the session configuration change. Invalid values and models
without adjustable effort are rejected.

For Claude Code, the saved `kimi-k2.7-code` selection launches as
`kimi-for-coding` when using a Kimi coding-subscription key. The main model
and tier/subagent model variables use the same endpoint mapping. Moonshot
platform keys retain `kimi-k2.7-code`.

The native/ACP aliases `kimi-code/k3` and `kimi-code/k3-256k` have context
windows of 1,048,576 and 262,144 tokens, respectively. Both K2.7 variants have
262,144 tokens. K3's full 1M window requires the corresponding membership
entitlement. See the [official Kimi model reference](https://www.kimi.com/code/docs/en/kimi-code/models.html).
...
- **Native terminal:** a conversation's app-server listens on a private Unix
  socket, and the Terminal view runs `codex resume --remote unix://… <threadId>`
  as a second client on the same thread (verified on 0.153.4), next to the
  app-server host's Runtime log.
...
### OpenCode — Supported through ACP (Go and Zen)

Overdeck supports persistent OpenCode ACP sessions with the `opencode` (Zen)
and `opencode-go` provider namespaces. Verified version: 1.18.29. See
[OpenCode setup](/configuration/harnesses#opencode-with-go-and-zen) for credentials,
model discovery, resume behavior, and current transport limitations.
...
- **OSS / license:** Open source (**⚠ verify exact license**). The live repo is
  `anomalyco/opencode` — distinct from the archived Go `opencode-ai/opencode`.
- **Auth:** native saved credentials; OpenCode Go subscription or OpenCode Zen billing.
- **Extensibility:**
...
- **Transport:** persistent `opencode acp`, verified with session creation,
  message delivery, and resume on 1.18.29. Overdeck reuses its ACP transcript and
  message transport while retaining the OpenCode harness identity.
- **Native terminal:** the conversation Terminal view runs `opencode attach` against
  the same server and session (verified on 1.18.31), next to the ACP host's
  Runtime log.
- **Provider scope:** Go and Zen. Other OpenCode providers are outside this integration.
...
| **OpenCode** | OSS ⚠ | TS | ✅ | ✅ | ✅ | ✅ | ✅ | ⭐ Custom Agents | Go and Zen via ACP |
