# Panopticon: Complete Update Since March 26th deft.co Meeting
...
Since meeting with the deft.co team on March 26, 2026, Panopticon has shipped **765+ commits**, resolved **60+ tracked issues**, and undergone its most significant architectural evolution to date. The system is now a production-grade, multi-agent orchestration platform that has been **continuously dogfooded on itself** — every commit listed below was written by Panopticon agents working through the Panopticon pipeline.
...
**The headline for deft.co specifically.** Panopticon is one of the first real-world production implementations of vBRIEF as the connective tissue of an entire AI development workflow.
...
- **Maintained fork**: kept a Panopticon fork in case the schema needs to evolve independently while we wait for upstream
...
vBRIEF was created to solve the fragmentation of AI agent memory and planning formats. Panopticon proves vBRIEF can be the *spine* of a multi-agent development workflow — not just a planning artifact, but the live coordination format between planning, work, review, test, and merge agents. Every issue, every task, every acceptance criterion in Panopticon flows through vBRIEF end-to-end.

---

## Theme 2: PR-Based Workflow & GitHub App Integration

The biggest architectural shift since March. Panopticon used to merge locally in the main repo, bypassing GitHub. It now has a full, audited GitHub PR lifecycle.

### What we built
- **GitHub App** (`panopticon-agent[bot]`) installable from the GitHub Marketplace
- **Bot identity**: all commits and PRs show the Panopticon bot, never a personal account
...
- **Review agent posts GitHub PR reviews** (`gh pr review --approve` / `--request-changes`) — permanent audit trail outside Panopticon
...
### Marketing angle
Merging is where AI agent workflows usually fall apart. Panopticon's merge system is now bank-grade: serialized, verified, queued, observable, and resumable from any failure point.

---

## Theme 4: Multi-Provider Architecture — Provider-Agnostic by Design

Panopticon now routes across all major model providers with a unified, configurable abstraction.
...
### Marketing angle
Panopticon is **not Anthropic-exclusive**. Customers run agents on the model that fits their cost, latency, and quality envelope — including subscription-routed Claude, GLM, MiniMax, OpenRouter's full catalog, and any model-per-agent-type configuration through Cloister.

---

## Theme 5: Cloister — The Agent Lifecycle Brain

Cloister is Panopticon's watchdog and lifecycle manager. It got major reliability work since March 26th.
...
- **Agent taxonomy added to planning prompt** (PAN-605) — fixes planning agents that confused Claude Code subagents (`codebase-explorer`, `plan`) with Panopticon specialists (`review-agent`, `test-agent`, `merge-agent`)

### Marketing angle
Cloister means agents that don't get stuck. The recovery loop, the queue, the busyness gating, and the deacon make Panopticon resilient to the failure modes that destroy hand-rolled agent setups.
...
Panopticon ships as a **one-command-installable desktop app**.
...
### Marketing angle
Panopticon is not a CLI you have to babysit in a terminal. It's a desktop app with a system tray, native notifications, and one-command install via npx.
...
When Claude compacts its context window, Panopticon now rotates specialist JSONL sessions instead of losing them.
...
### Marketing angle
Panopticon is built on open standards: vBRIEF for plans, beads for tasks, GitHub for review and merge, and a multi-provider abstraction so customers are never locked into one model vendor. We *contribute* to vBRIEF rather than fork-and-forget.
...
- **MYN, Auricle, OpenClaw, Krux** are also being developed via Panopticon (multi-project orchestration in production)
- **deft.co's vBRIEF** powers it end-to-end

### Marketing angle
Panopticon doesn't have a "demo mode." The thing in the demo is the thing we build it with. Every commit is proof of work.
...
- **The deft.co line**: "Panopticon is the first production multi-agent orchestrator built on vBRIEF — every issue, plan, task, and merge flows through deft.co's open planning standard."
- **The technical line**: "Panopticon turns a GitHub issue into a squash-merged PR through a hardened review → test → merge specialist pipeline, with humans only clicking 'merge.'"
- **The dogfooding line**: "765 commits in 17 days, written by Panopticon agents using Panopticon to build Panopticon."
