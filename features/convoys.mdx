...
# Watch a specific agent: open it in the dashboard, or run the
# backend-aware Attach: command that pan start prints
# (see "Reaching an agent from a shell" in docs/TERMINAL-BACKENDS.md)
...
```bash
# Check individual agent status (either terminal backend)
pan status --json   # read each agent's `alive`

# View agent output
curl -s "http://localhost:3011/api/agents/<agent-id>/output?lines=100"
...
- Review the synthesis agent's output in the dashboard or via `GET /api/agents/<agent-id>/output`
