...
A swarm fits when an issue's xBRIEF plan splits into several items that can be implemented independently. If the plan is one linear sequence, a single work agent is faster — there is nothing to parallelize.
...
<ThemedImage
  ...
  alt="Agents page: metric strip over agent cards showing model, runtime, and last activity for each running work agent, including swarm slots"
/>
...
Two fields on `XBriefItem.metadata` control swarm behavior.
...
| `requiresSynthesis: boolean` | Marks a synthesis item. The swarm treats any item with more than one blocking parent as a synthesis item whether or not this is set, so you rarely need it. |
...
<ThemedImage
  ...
  alt="Pipeline page: metric strip over phase-grouped issue rows (Ship / Review / Verifying) with a live activity feed"
/>
