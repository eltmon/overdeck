...
branch chip on the conversation row shows the PR number, colored by state: blue
...
state of linked PRs, so the badge renders without a live GitHub call. Merged PRs
are final and are not re-read.

### Linking a pull request by hand

Open a conversation's actions (the row's `⋮`, a right-click on the row, or a
right-click on its tab) and choose **Link pull request…**. Type a PR URL, a
...
Linking never archives or stops a conversation.
