...
The review-agent is the gatekeeper. It reads the diff, the PRD, and the xBRIEF plan, and looks for:
...
# Via CLI:
pan admin specialists wake review-agent --task "review PAN-655"
```
...
Review can also run as a **convoy**: set `roles.review.mode: full` and four reviewers (correctness, security, performance, requirements) run in parallel before a review parent writes the synthesis. See [Review modes](/cli/review-commands#review-modes).
...
**Project configuration** in `~/.overdeck/projects.yaml`:
...
**What it does:**
1. Reads the PRD and the xBRIEF acceptance criteria
...
# Via CLI:
pan admin specialists wake merge-agent --task "merge PAN-655"
```
...
Once the pipeline has driven a branch to ready-to-merge, it lands on the **Awaiting Merge** page — the single place to see every feature branch waiting on the human merge gate.

<ThemedImage
  light="/images/dashboard/awaiting-merge-light.png"
  dark="/images/dashboard/awaiting-merge-dark.png"
  alt="Awaiting Merge page listing feature branches ready for human merge"
/>

## Queue Processing

Each specialist has a per-project task queue at `~/.overdeck/agents/{name}/hook.json`, managed via the **FPP (Fixed Point Principle)** — borrowed from Gastown, inspired by Doctor Who: *any runnable action is a fixed point and must resolve before the system can rest.*
...
Specialist configuration lives in `~/.overdeck/cloister.toml`:
...
pan admin specialists list --json

# Reset a wedged specialist (clears session, starts fresh)
pan admin specialists reset review-agent
```

The dashboard surfaces the same state visually. The **Agents** page rolls up every running specialist and work-agent into cards — model, runtime, and last activity — under a metric strip tracking how many agents are running, how many are stuck, and the 24-hour cost and token spend. It's the at-a-glance answer to "what is every agent doing right now" without tailing a single log.

<ThemedImage
  light="/images/dashboard/agents-light.png"
  dark="/images/dashboard/agents-dark.png"
  alt="Agents page with a metric strip (Running, Stuck, Cost 24h, Tokens 24h, Queue) over agent cards showing model, runtime, and last activity"
/>
