...
8. Record automated gates and browser UAT as separate verdicts. The test verdict (`status` in `.pan/test/result.json`, `--status` on `pan admin specialists done test`) covers only configured test-suite commands; when UAT is required, record the UAT verdict independently (`uatStatus` in the result file, `--uat-status` on the command). A UAT that cannot run or leaves any criterion unproven is a failed UAT even when the test suite passed; only a later UAT run that proves the criterion reverses it.
