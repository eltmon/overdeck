...
import { readFileSync as readFileSync4, existsSync as existsSync6, writeFileSync as writeFileSync4, mkdirSync as mkdirSync6, openSync, readSync, fstatSync, closeSync } from "fs";
...
import { existsSync as existsSync4, mkdirSync as mkdirSync4, readFileSync as readFileSync2, appendFileSync as appendFileSync2, writeFileSync as writeFileSync2, renameSync } from "fs";
...
function getDatabasePath() {
  return join3(getPanopticonHome(), "panopticon.db");
}
...
function insertCostEvent(event2, sourceFile) {
  ...
  try {
    ...
    `).run(
      ...
      event2.agentId,
      event2.issueId,
      event2.sessionType || "unknown",
      event2.provider || "anthropic",
      event2.model,
      event2.input,
      event2.output,
      event2.cacheRead,
...
var PROJECTS_CONFIG_FILE = join4(PANOPTICON_HOME, "projects.yaml");
function loadProjectsConfig() {
  if (!existsSync2(PROJECTS_CONFIG_FILE)) {
    return { projects: {} };
  }
  try {
...
function resolveWalDir(issueId2) {
  const projects = listProjects();
  const issuePrefix = issueId2.split("-")[0]?.toUpperCase();
  ...
  for (const { key, config } of projects) {
    ...
    if (projectKey === issuePrefix || projectName === issuePrefix) {
      ...
      return join5(repoPath, eventsSubdir);
    }
  }
  return null;
}
function appendToWal(event2) {
  try {
    const walDir = resolveWalDir(event2.issueId);
    if (!walDir) return false;
    if (!existsSync3(walDir)) {
      mkdirSync3(walDir, { recursive: true });
    }
    const walFile = join5(walDir, `${event2.issueId.toUpperCase()}.jsonl`);
    const line = JSON.stringify(event2) + "\n";
    appendFileSync(walFile, line, "utf-8");
    return true;
  } catch (err) {
    console.error(`[wal] Failed to write WAL for ${event2.issueId}:`, err);
    return false;
  }
}
...
function getCostsDir() {
  return join6(process.env.HOME || homedir2(), ".panopticon", "costs");
}
function getEventsFile() {
  return join6(getCostsDir(), "events.jsonl");
}
function ensureEventsFile() {
  const costsDir = getCostsDir();
  const eventsFile = getEventsFile();
  mkdirSync4(costsDir, { recursive: true });
  if (!existsSync4(eventsFile)) {
    writeFileSync2(eventsFile, "", "utf-8");
  }
}
function appendCostEvent(event2) {
  ...
  const line = JSON.stringify(event2) + "\n";
  appendFileSync2(getEventsFile(), line, "utf-8");
  try {
    insertCostEvent(event2);
  } catch (err) {
...
import { existsSync as existsSync5, writeFileSync as writeFileSync3, readFileSync as readFileSync3, mkdirSync as mkdirSync5, unlinkSync } from "fs";
import { join as join7 } from "path";
function getTldrMetrics(workspacePath, sinceCheckpoint = false) {
  const tldrDir = join7(workspacePath, ".tldr");
  const interceptionsLog = join7(tldrDir, "interceptions.log");
  const bypassesLog = join7(tldrDir, "bypasses.log");
  const checkpointFile = join7(tldrDir, "metrics-checkpoint.json");
  let interceptionsStartLine = 0;
  let bypassesStartLine = 0;
  if (sinceCheckpoint && existsSync5(checkpointFile)) {
    try {
      const checkpoint = JSON.parse(readFileSync3(checkpointFile, "utf-8"));
      interceptionsStartLine = checkpoint.interceptionsLine || 0;
      bypassesStartLine = checkpoint.bypassesLine || 0;
    } catch {
    }
  }
  const allInterceptionLines = existsSync5(interceptionsLog) ? readFileSync3(interceptionsLog, "utf-8").split("\n").filter((l) => l.trim()) : [];
  ...
  const allBypassLines = existsSync5(bypassesLog) ? readFileSync3(bypassesLog, "utf-8").split("\n").filter((l) => l.trim()) : [];
...
function captureTldrMetrics(workspacePath) {
  ...
  const bypassesLog = join7(tldrDir, "bypasses.log");
  const checkpointFile = join7(tldrDir, "metrics-checkpoint.json");
  const interceptionsTotal = existsSync5(interceptionsLog) ? readFileSync3(interceptionsLog, "utf-8").split("\n").filter((l) => l.trim()).length : 0;
  const bypassesTotal = existsSync5(bypassesLog) ? readFileSync3(bypassesLog, "utf-8").split("\n").filter((l) => l.trim()).length : 0;
  const checkpoint = {
  ...
  try {
    writeFileSync3(checkpointFile, JSON.stringify(checkpoint, null, 2), "utf-8");
  } catch {
  }
  return metrics;
}
var execAsync = promisify(exec);
var TLDR_STATE_DIR = join7(PANOPTICON_HOME, "tldr");
...
var stateDir = join8(process.env.HOME || homedir3(), ".panopticon", "costs", "state");
mkdirSync6(stateDir, { recursive: true });
var stateFile = join8(stateDir, `${sessionId}.offset`);
var lastOffset = 0;
if (existsSync6(stateFile)) {
  try {
    lastOffset = parseInt(readFileSync4(stateFile, "utf-8").trim(), 10) || 0;
  } catch {
  }
}
var seenFile = join8(stateDir, `${sessionId}.seen`);
var seenRequestIds = /* @__PURE__ */ new Set();
if (existsSync6(seenFile)) {
  try {
    const seenContent = readFileSync4(seenFile, "utf-8").trim();
    if (seenContent) {
...
if (stat.size <= lastOffset) {
  closeSync(fd);
  writeFileSync4(stateFile, String(stat.size), "utf-8");
  process.exit(0);
}
...
if (seenRequestIds.size > 0) {
  writeFileSync4(seenFile, Array.from(seenRequestIds).join("\n") + "\n", "utf-8");
}
process.exit(0);
