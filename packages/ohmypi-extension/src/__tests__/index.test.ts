...
import overdeckOhmypiExtension, {
  handleSessionStart,
...
const overdeckPiExtension = overdeckOhmypiExtension
type PiExtensionAPI = OhmypiExtensionAPI
type PiCommand = OhmypiCommand
...
describe('handleTurnEnd', () => {
  ...
  it('records the session log path once on turn_end and dedupes a repeat (PAN-3959)', async () => {
    ...
    writeFileSync(paths.sessionsIndexPath, `${JSON.stringify(
      { sessionId: 'sess-turn-end-upgrade', at: fixedTime, source: 'session-start', harness: 'ohmypi', model: 'unknown' },
    )}\n`)
    const sessionLogPath = '/tmp/ohmypi-turn-end-upgrade-fixture.jsonl'
...
// PAN-1833 regression: import.meta.url-based resolution of the extension dist path
// must not depend on process.cwd(). When agents.ts computes the path via
//   fileURLToPath(new URL('../packages/ohmypi-extension/dist/index.js', import.meta.url))
// the result must be the same regardless of the caller's CWD.
describe('PAN-1833 regression: import.meta.url-based path resolution', () => {
  it('resolves the built dist/index.js path independently of process.cwd()', async () => {
    const { fileURLToPath } = await import('node:url')
    const { existsSync } = await import('node:fs')
    const { resolve } = await import('node:path')

    // Resolve using import.meta.url (the correct pattern from agents.ts).
    // In vitest source mode, import.meta.url is the .ts file under src/__tests__/.
    // We go two levels up (to package root) and then into dist/.
    const viaMetaUrl = fileURLToPath(
      new URL('../../dist/index.js', import.meta.url)
    )

    // Verify the path is absolute and not relative (i.e., doesn't contain ".." segments)
    expect(viaMetaUrl).toMatch(/^\//)
    // The path must resolve to the same result regardless of CWD
    const originalCwd = process.cwd()
    try {
      process.chdir('/tmp')
      const viaMetaUrlFromTmp = fileURLToPath(
        new URL('../../dist/index.js', import.meta.url)
      )
      expect(viaMetaUrlFromTmp).toBe(viaMetaUrl)
      // The old broken pattern would resolve relative to /tmp
      const viaCwd = resolve('/tmp', 'packages/ohmypi-extension/dist/index.js')
      expect(viaCwd).toBe('/tmp/packages/ohmypi-extension/dist/index.js')
      expect(viaMetaUrlFromTmp).not.toBe(viaCwd)
    } finally {
      process.chdir(originalCwd)
    }

    // And the file actually exists (built by npm run build:ohmypi-extension)
    expect(existsSync(viaMetaUrl)).toBe(true)
  })
})
