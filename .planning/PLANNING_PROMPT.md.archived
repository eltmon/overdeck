...
## Issue Details
- **ID:** PAN-557
- **Title:** Add conversation deep linking and copy buttons
- **URL:** https://github.com/eltmon/panopticon-cli/issues/557

## Description
## Summary

Add support for deep linking to conversations and make it easy to share conversation links.

## Requirements

1. **Deep linking**: Each conversation should be accessible via a shareable URL that navigates directly to that conversation
2. **Copy button in conversation list**: Add a copy button next to the archive button in the conversation list view
3. **Copy button in conversation header**: Add a copy button next to the archive button at the top of an open conversation

## Implementation Notes

- Copy buttons should copy the conversation's deep link URL to clipboard
- Both UI locations should have consistent styling and behavior
- Success feedback (e.g., toast notification) on copy
...
```json
{
  "vBRIEFInfo": {
    ...
    "description": "Plan for PAN-557: <issue title>"
  },
  "plan": {
    "id": "pan-557",
    "title": "<issue title>",
    ...
    "references": [
      { "uri": "https://github.com/eltmon/panopticon-cli/issues/557", "label": "PAN-557", "type": "issue" }
    ],
    "tags": ["<relevant tags>"],
    "narratives": {
...
- `plan.id` MUST be the issue ID in lowercase (e.g., "pan-557")
