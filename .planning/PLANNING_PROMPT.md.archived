...
## Issue Details
- **ID:** PAN-558
- **Title:** Auto-update mechanism for Electron app
- **URL:** https://github.com/eltmon/panopticon-cli/issues/558

## Description
## Problem

When users run `npx panopticon`, they get the version published to npm (currently 0.3.0), not the latest local/dev version. There's no auto-update mechanism to push new versions to existing installs.

## Goals

1. **Auto-update**: App checks for updates on startup and periodically, downloads in background, installs on quit
2. **npx correctness**: `npx panopticon` should resolve to the latest published version

## Implementation Options

### electron-updater (recommended)
- Works with GitHub Releases (free, easy)
- Supports differential updates (only download changed parts)
- Signs releases for security
- Implementation: https://www.electron.build/auto-update

### Key decisions needed
- **Update server**: GitHub Releases (simplest) vs custom server
- **Update channel**: stable vs beta (git tag-based)
- **Install timing**: Silent background + install on quit, or user-prompted?
- **Rollback**: How to handle bad releases?

## Scope for v1
- Only Windows/macOS/Linux (no Electron .exe support needed yet per PAN-531)
...
- [ ] Installs on quit and relaunches
...
```json
{
  "vBRIEFInfo": {
    ...
    "description": "Plan for PAN-558: <issue title>"
  },
  "plan": {
    "id": "pan-558",
    "title": "<issue title>",
    ...
    "references": [
      { "uri": "https://github.com/eltmon/panopticon-cli/issues/558", "label": "PAN-558", "type": "issue" }
    ],
    "tags": ["<relevant tags>"],
    "narratives": {
...
- `plan.id` MUST be the issue ID in lowercase (e.g., "pan-558")
