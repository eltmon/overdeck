...
## Issue Details
- **ID:** PAN-712
- **Title:** fix: stale pan work/cloister/specialists refs in .claude/skills/
- **URL:** https://github.com/eltmon/panopticon-cli/issues/712

## Description
## Summary

Code review for PAN-705 flagged stale references in `.claude/skills/` project-level skill files. These files contain `pan work`, `cloister`, or `specialists` references that predated the PAN-705 command taxonomy reorganization.

## What to fix

Audit all files under `.claude/skills/` for:
- `pan work start/done/kill/recover/tell/resume/approve/close/reopen/sync-main/wipe` → update to new verbs
- `pan cloister` → update to new command group
- `pan specialists` → update to new command group

## Why

Stale skill docs mislead agents using those skills about the correct CLI syntax.

## Acceptance criteria

- [ ] All `.claude/skills/*.md` files pass a grep for stale refs from PAN-705's taxonomy changes
- [ ] Updated files verified against current `pan --help` output
...
```json
{
  "vBRIEFInfo": {
    ...
    "description": "Plan for PAN-712: <issue title>"
  },
  "plan": {
    "id": "pan-712",
    "title": "<issue title>",
    ...
    "references": [
      { "uri": "https://github.com/eltmon/panopticon-cli/issues/712", "label": "PAN-712", "type": "issue" }
    ],
    "tags": ["<relevant tags>"],
    "narratives": {
...
- `plan.id` MUST be the issue ID in lowercase (e.g., "pan-712")
