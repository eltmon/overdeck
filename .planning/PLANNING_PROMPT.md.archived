...
## Issue Details
- **ID:** PAN-596
- **Title:** Allow editing a conversation title
- **URL:** https://github.com/eltmon/panopticon-cli/issues/596

## Description
## Problem

Conversation titles are the human-readable names shown in Mission Control. The backend supports editing titles via `PATCH /api/conversations/:name` (body: `{ title: string }`), but there is no UI to actually do this.

Currently the only way to rename a conversation is to manually `curl` the API.

## What

Add UI to edit a conversation's title from the dashboard.

## Acceptance criteria

- [ ] Edit button / inline edit on conversation cards or detail panel
- [ ] Calls `PATCH /api/conversations/:name` with `{ title }`
- [ ] Updates the conversation title immediately in the UI
- [ ] When `title_source` is 'ai', user rename should mark it as 'manual' so AI doesn't auto-replace it
...
```json
{
  "vBRIEFInfo": {
    ...
    "description": "Plan for PAN-596: <issue title>"
  },
  "plan": {
    "id": "pan-596",
    "title": "<issue title>",
    ...
    "references": [
      { "uri": "https://github.com/eltmon/panopticon-cli/issues/596", "label": "PAN-596", "type": "issue" }
    ],
    "tags": ["<relevant tags>"],
    "narratives": {
...
- `plan.id` MUST be the issue ID in lowercase (e.g., "pan-596")
