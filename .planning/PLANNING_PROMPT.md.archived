...
## Issue Details
- **ID:** PAN-512
- **Title:** Effect yield* failures not caught by JS try/catch — audit all agent route Effect.gen blocks
- **URL:** https://github.com/eltmon/panopticon-cli/issues/512

## Description
## Problem

`yield* Effect.promise(...)` failures propagate as Effect fiber failures, not JS exceptions. Any JS `try/catch` block surrounding a `yield*` call does **not** intercept Effect failures — they silently bypass the catch and surface as top-level route errors.

This caused: `Failed to start agent: Command failed: git diff --cached --quiet`

The planning artifact commit block in `routes/agents.ts` used a nested `try/catch` inside an `Effect.gen` handler, expecting it to catch `yield*` failures. It didn't.

## Fix applied (workaround)

The immediate block was fixed in 783d85f by:
- Wrapping the entire planning commit block in `Effect.gen` + `.pipe(Effect.catchAll(() => Effect.void))`
- Handling `git diff --cached --quiet` exit-1 via `.then/.catch` in the Promise directly, avoiding the `yield*`-inside-try pattern

## Proper fix needed

Audit all `Effect.gen` handlers in `routes/agents.ts` and other route files for JS `try/catch` blocks that surround `yield*` calls. Replace with proper Effect error handling:
- `Effect.catchAll` / `Effect.catchTag` for expected failures
- `Effect.either` to turn failures into `Either<E, A>` values
- `Effect.option` for optional operations
- Remove JS `try/catch` that wraps `yield*` — it's always a no-op

Files to audit: all `src/dashboard/server/routes/*.ts` that use `Effect.gen`.
...
```json
{
  "vBRIEFInfo": {
    ...
    "description": "Plan for PAN-512: <issue title>"
  },
  "plan": {
    "id": "pan-512",
    "title": "<issue title>",
    ...
    "references": [
      { "uri": "https://github.com/eltmon/panopticon-cli/issues/512", "label": "PAN-512", "type": "issue" }
    ],
    "tags": ["<relevant tags>"],
    "narratives": {
...
- `plan.id` MUST be the issue ID in lowercase (e.g., "pan-512")
