...
## Issue Details
- **ID:** PAN-913
- **Title:** Auto-detect expired Codex auth and spawn re-authentication flow
- **URL:** https://github.com/eltmon/panopticon-cli/issues/913

## Description
## Problem

When the Codex OAuth token expires (or the refresh token gets burned), all `gpt-5.4` model agents fail with:

```
API Error: 503 auth_unavailable: no auth available (providers=codex, model=gpt-5.4)
```

Users have no visibility into this until agents start dying. The cliproxy log shows the real cause:

```
"Your refresh token has already been used to generate a new access token. Please try signing in again."
```

but this is buried in `~/.panopticon/cliproxy/cliproxy.log`.

## Current state

- `~/.panopticon/cliproxy/auth/codex-primary.json` stores the bridged OAuth token
- CLIProxy has an auto-refresh loop, but it silently fails every 5 min when the refresh token is invalid
- Reviewers/agents just get stuck with auth errors
- The user must manually discover the issue and run `codex login`

## Proposed solution

### 1. Auto-detect expired Codex auth in Panopticon

When spawning an agent/reviewer that routes to the `codex` provider, check if the token is expired or the refresh token is burned. If so:

- **Kill the agent spawn** before burning API credits on guaranteed-failing requests
- **Show a clear dashboard notification** explaining the issue
- **Offer a one-click "Re-authenticate" button** that spawns `codex login` in a terminal session

### 2. Settings page clarity: subscription vs API key

In the dashboard settings / provider config panel, make it explicit whether a model uses:

- **ChatGPT subscription (OAuth)** — requires `codex login`, token expires periodically
- **OpenAI API key** — uses the `sk-proj-...` key from config, no expiry

Currently `gpt-5.4` routes to Codex (subscription OAuth) but this is invisible to users. The settings should show:

| Model | Provider | Auth type | Status |
|---|---|---|---|
| gpt-5.4 | Codex | Subscription OAuth | Expired (re-authenticate) |
| glm-5.1 | Kimi | API key | Valid |

### 3. Support headless/device auth

For remote/headless machines where browser auth isn't available, support `codex login --device-auth`. This gives a URL + code to paste in a browser on another machine.

The dashboard re-authentication flow should detect if Panopticon is running headless and offer `--device-auth` automatically, showing the user:
- The device auth URL
- The code to enter
- A "I've completed auth" button to verify

## Acceptance criteria

- [ ] Dashboard detects expired codex auth before spawning agents
- [ ] Dashboard shows a prominent "Codex auth expired" banner with a re-auth button
- [ ] Settings page shows provider auth type (subscription OAuth vs API key) per model
- [ ] Headless mode uses `--device-auth` and surfaces URL + code to user
- [ ] After successful re-auth, agent spawn retries automatically
- [ ] Panopticon Documentation has been updated to let user know that Codex is required to use the codex subscription

## Related

- PAN-912 (dead pane detection) — same symptom area (agents dying silently)
- `skills/cliproxy/SKILL.md` — current manual workaround docs
...
```json
{
  "vBRIEFInfo": {
    ...
    "description": "Plan for PAN-913: <issue title>"
  },
  "plan": {
    "id": "pan-913",
    "title": "<issue title>",
    ...
    "references": [
      { "uri": "https://github.com/eltmon/panopticon-cli/issues/913", "label": "PAN-913", "type": "issue" }
    ],
    "tags": ["<relevant tags>"],
    "narratives": {
...
- `plan.id` MUST be the issue ID in lowercase (e.g., "pan-913")
