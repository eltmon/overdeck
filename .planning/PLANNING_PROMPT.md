...
## Issue Details
- **ID:** PAN-494
- **Title:** pan work issue fails if workspace doesn't exist — requires separate pan workspace create
- **URL:** https://github.com/eltmon/panopticon-cli/issues/494

## Description
## Bug

`pan work issue <id>` exits with an error if no workspace exists for the issue, telling the user to run `pan workspace create` first. But `pan work issue` is supposed to be the single command to create a workspace and spawn an agent — the two-step requirement is unexpected.

## Reproduction

```
$ pan work issue PAN-489
✖ No workspace found for PAN-489
CRITICAL: Work agents must run in a workspace with a feature branch.
To fix, create a workspace first:
  pan workspace PAN-489
```

## Expected

`pan work issue PAN-489` should create the workspace automatically if it doesn't exist, then spawn the agent — same as it did for PAN-488.

## Notes

PAN-488 worked in one step earlier in this session; PAN-489 required two steps. The difference may be related to whether a prior workspace existed (PAN-489 had a stale workspace from a previous attempt that was cleaned up). Worth investigating what condition triggers the two-step requirement.
...
```json
{
  "vBRIEFInfo": {
    ...
    "description": "Plan for PAN-494: <issue title>"
  },
  "plan": {
    "id": "pan-494",
    "title": "<issue title>",
    ...
    "references": [
      { "uri": "https://github.com/eltmon/panopticon-cli/issues/494", "label": "PAN-494", "type": "issue" }
    ],
    "tags": ["<relevant tags>"],
    "narratives": {
...
- `plan.id` MUST be the issue ID in lowercase (e.g., "pan-494")
