...
## Issue Details
- **ID:** PAN-453
- **Title:** Full vBRIEF v0.5 spec support — uid, author, sequence, references, timestamps
- **URL:** https://github.com/eltmon/panopticon-cli/issues/453

## Description
## Summary

Implement complete vBRIEF v0.5 spec compliance. Add fields we currently skip: uid, author (both levels), sequence counter, references (PRDs/issues), created/updated/completed timestamps, priority on items.

## PRD

**`docs/prds/planned/pan-453-vbrief-full-spec-support.md`**

## Key Fields

- `vBRIEFInfo.author` — tool identifier ("panopticon-cli/0.6.0")
- `plan.uid` — UUID v4, generated once
- `plan.author` — agent model ("agent:claude-opus-4-6")
- `plan.sequence` — incrementing counter on every write (conflict detection)
- `plan.references` — links to PRDs, GitHub issues, specs
- `plan.created/updated` + `items[].created/completed` — lifecycle timestamps

## Motivation

We're communicating with the Deft team about our vBRIEF usage. Want to demonstrate full spec compliance beyond just the required fields. Also enables better audit trail (who created, when updated, what changed).
...
- `plan.id` MUST be the issue ID in lowercase (e.g., "pan-453")
