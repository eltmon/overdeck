...
## Issue Details
- **ID:** PAN-711
- **Title:** PAN-705 follow-up: remove transitional route aliases after merge
- **URL:** https://github.com/eltmon/panopticon-cli/issues/711

## Description
## Context

PAN-705 renames the dashboard specialist feedback routes:
- `/api/workspaces/:id/review-status` → `/api/review/:id/status`
- `/api/workspaces/:id/approve` → `/api/issues/:id/approve`
- `/api/workspaces/:id/merge` → `/api/issues/:id/merge`
- (and similar for approve/merge/review-trigger/review-reset/review-request)

To allow specialist reviews to run on the PAN-705 branch itself against the main-built dashboard, we kept the old paths as transitional aliases that delegate to the new handlers.

## What this issue does

Once PAN-705 is merged to `main` and the production dashboard is rebuilt, the old route aliases are dead code. Remove them:

- Delete the alias registrations in `src/dashboard/server/routes/workspaces.ts`
- Delete any fallback handling added to the specialist prompts for the old paths
- Confirm no external callers still hit the old paths (grep + one full pipeline run)

## Blocker

- Depends on PAN-705 being merged and dashboard rebuilt on main
- Do not start until at least one full work→review→test→merge cycle has succeeded on the new routes
...
```json
{
  "vBRIEFInfo": {
    ...
    "description": "Plan for PAN-711: <issue title>"
  },
  "plan": {
    "id": "pan-711",
    "title": "<issue title>",
    ...
    "references": [
      { "uri": "https://github.com/eltmon/panopticon-cli/issues/711", "label": "PAN-711", "type": "issue" }
    ],
    "tags": ["<relevant tags>"],
    "narratives": {
...
- `plan.id` MUST be the issue ID in lowercase (e.g., "pan-711")
