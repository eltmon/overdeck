...
## Issue Details
- **ID:** PAN-619
- **Title:** Cost tracking: comma-format total cost, add day/week ticks to 30-day trend
- **URL:** https://github.com/eltmon/panopticon-cli/issues/619

## Description
## Summary

Two small polish items for the cost tracking UI:

1. **Comma-format total cost** — large dollar values (e.g. `$1234.56`) should display with thousands separators (`$1,234.56`) for readability
2. **Day/week tick marks on 30-day cost trend** — the 30-day chart currently lacks axis ticks, making it hard to orient in time; add day ticks (every day) or at minimum week ticks (every 7 days) so the viewer can quickly identify which date a spike occurred on

## Acceptance Criteria

- [ ] Total cost figure uses locale-formatted numbers with comma separators (e.g. `$1,234.56`)
- [ ] 30-day trend chart x-axis shows tick marks — at minimum weekly (every 7 days), ideally daily with labels on week boundaries
- [ ] No regression to other cost/metrics displays
...
```json
{
  "vBRIEFInfo": {
    ...
    "description": "Plan for PAN-619: <issue title>"
  },
  "plan": {
    "id": "pan-619",
    "title": "<issue title>",
    ...
    "references": [
      { "uri": "https://github.com/eltmon/panopticon-cli/issues/619", "label": "PAN-619", "type": "issue" }
    ],
    "tags": ["<relevant tags>"],
    "narratives": {
...
- `plan.id` MUST be the issue ID in lowercase (e.g., "pan-619")
