...
## Issue Details
- **ID:** PAN-560
- **Title:** RallyClient Effect service crashes on dashboard startup in v0.6.0
- **URL:** https://github.com/eltmon/panopticon-cli/issues/560

## Description
## Description

On startup, the dashboard server throws a fatal `Fiber.runLoop` error because `RallyClient` is passed as a raw Effect `Service` tag instead of a valid effect to the fiber runner.

## Error

```
[07:31:01.349] ERROR (#1): Error: Fiber.runLoop: Not a valid effect: {
  "_id": "Service",
  "key": "panopticon/dashboard/RallyClient",
  "stack": "Error\n    at Module.Service (file:///...panopticon-cli/node_modules/effect/dist/ServiceMap.js:40:15)\n    at file:///...panopticon-cli/dist/dashboard/server.js:2155:44"
}
    at causePrettyError (file:///...panopticon-cli/node_modules/effect/dist/internal/effect.js:228:13)
```

## Steps to Reproduce

1. Install `panopticon-cli@0.6.0` globally via npm
2. Run `panopticon` to start the dashboard
3. Error appears immediately after server starts on port 7825

## Environment

- panopticon-cli: 0.6.0 (global npm install)
- Node: v20.19.5
- macOS Darwin 25.1.0

## Analysis

`dist/dashboard/server.js:2155` is passing the `RallyClient` service tag directly where an Effect is expected. Likely needs to be wrapped in `Effect.flatMap` or resolved via `Effect.gen` / layer provision before being passed to the fiber runner.
...
```json
{
  "vBRIEFInfo": {
    ...
    "description": "Plan for PAN-560: <issue title>"
  },
  "plan": {
    "id": "pan-560",
    "title": "<issue title>",
    ...
    "references": [
      { "uri": "https://github.com/eltmon/panopticon-cli/issues/560", "label": "PAN-560", "type": "issue" }
    ],
    "tags": ["<relevant tags>"],
    "narratives": {
...
- `plan.id` MUST be the issue ID in lowercase (e.g., "pan-560")
