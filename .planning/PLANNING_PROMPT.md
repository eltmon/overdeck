...
## Issue Details
- **ID:** PAN-540
- **Title:** Remove convoy abstraction, inline parallel review directly into review-agent
- **URL:** https://github.com/eltmon/panopticon-cli/issues/540

## Description
## Summary

The convoy system was built to support two ideas:
1. A multi-issue fleet runner (`pan convoy create MIN-10 MIN-11 MIN-12`) — **never built**
2. A parallel code review orchestrator (3 reviewers + synthesis) — **the only real use**

The abstraction (template registry, state persistence in `~/.panopticon/convoys/`, CLI commands, dashboard nav + panel, `ConvoyContext`/`ConvoyState` types) is all overhead that exists solely to run 4 agents in a fixed order. The review-agent already knows exactly which agents it needs — it doesn't need a generic template system to tell it that.

## What to Remove

- `src/lib/convoy.ts` — runtime orchestration layer
- `src/lib/convoy-templates.ts` — template registry
- `src/cli/commands/convoy/` — all CLI convoy commands (`start`, `stop`, `list`, `status`)
- Convoy API endpoints from `src/dashboard/server/routes/metrics.ts`
- `src/dashboard/frontend/src/components/ConvoyPanel.tsx`
- `src/dashboard/frontend/src/hooks/useConvoys.ts`
- "Convoys" nav item from `src/dashboard/frontend/src/components/Sidebar.tsx`
- Convoys route from `src/dashboard/frontend/src/App.tsx`
- "Convoy Reviewers" section from Settings model assignments (`src/dashboard/frontend/src/components/Settings/`)
- Convoy work type entries from `src/lib/work-types.ts` and `src/lib/smart-model-selector.ts`
- Convoy-related model settings from `src/lib/settings-api.ts`
- References in `src/dashboard/frontend/src/components/Settings/types.ts`

## What to Keep / Refactor

The 3+1 parallel review stays. `review-agent.ts` should be refactored to inline what `startConvoy('code-review', ...)` + `waitForConvoy(...)` did:

1. Spawn 3 tmux sessions in parallel: correctness, security, performance reviewers
2. Poll for completion of all 3 (same polling logic currently in `waitForConvoy`)
3. Run synthesis agent after all 3 complete
4. Parse output from the 4 agents (already done in `parseConvoySynthesis`)

The `agents/code-review-*.md` templates stay unchanged.

The 4 model assignments (convoy:security-reviewer, convoy:correctness-reviewer, convoy:performance-reviewer, convoy:synthesis-agent) should be renamed to something like `review:security`, `review:correctness`, `review:performance`, `review:synthesis` and kept in the model router — just without the "convoy" namespace.

## Configurable Review Agents (from PAN-609)

After inlining, make the reviewer list user-configurable via `config.yaml`:
...
specialists:
  review_agents:
    - name: deep-review
      ...
      focus: [correctness, security, architecture]
    - name: standard-review
      model: claude-sonnet-4-6
      focus: [correctness, style, performance]
    - name: quick-review
      model: claude-haiku-4-5
      focus: [lint, formatting, obvious-bugs]
```

- `name`: unique identifier, used in tmux session naming and logs
- `model`: model ID for this reviewer
- `focus`: optional list of review focus areas passed to the agent prompt
- Backward-compatible: if `review_agents` is absent, fall back to the default 3 (correctness, security, performance)

### Synthesis Rules

- If **any** reviewer says CHANGES_REQUESTED → overall is CHANGES_REQUESTED
- Security issues from any reviewer are always surfaced
- Duplicate findings (same file + same concern) are collapsed
- The synthesized report attributes findings to which reviewer found them
...
interface SpecialistsConfig {
  review_agents?: ReviewAgentConfig[];  // new — configurable list
  review_agent?: SpecialistConfig;      // legacy single-agent fallback
  // ...
}
```
...
Update `docs/USAGE.md` to remove the convoy section. Update `docs/INDEX.md` if it references convoys.
...
```json
{
  "vBRIEFInfo": {
    ...
    "description": "Plan for PAN-540: <issue title>"
  },
  "plan": {
    "id": "pan-540",
    "title": "<issue title>",
    ...
    "references": [
      { "uri": "https://github.com/eltmon/panopticon-cli/issues/540", "label": "PAN-540", "type": "issue" },
      { "uri": "/home/eltmon/Projects/panopticon-cli/workspaces/feature-pan-540/docs/prds/active/pan-540", "label": "pan-540", "type": "prd" }
    ],
    "tags": ["<relevant tags>"],
    "narratives": {
...
- `plan.id` MUST be the issue ID in lowercase (e.g., "pan-540")
