...
## Issue Details
- **ID:** PAN-445
- **Title:** Planning Abort button uses native JS confirm() instead of styled dialog
- **URL:** https://github.com/eltmon/panopticon-cli/issues/445
...
### 1. Native JS confirm() instead of styled dialog

Clicking "Abort" in the Plan dialog triggers a native `window.confirm()` browser dialog instead of using the styled `useConfirm()` dialog from `DialogProvider` that all other confirmation actions use.

**Expected:** Same dark-themed confirmation dialog with styled buttons that the rest of the dashboard uses (Cancel issue, Deep Wipe, Kill Agent all use `useConfirm()` from `DialogProvider.tsx`).

### 2. "planning" label not removed on abort

When aborting a planning session, the "planning" label remains on the GitHub issue. The abort-planning endpoint should remove the "planning" label so the issue returns to its pre-planning visual state on the board.

## Location

- `src/dashboard/frontend/src/components/PlanDialog.tsx` — Abort button click handler
- `src/dashboard/server/routes/issues.ts` — `POST /api/issues/:id/abort-planning` endpoint

## Fix

### Dialog fix
Replace the native `confirm()` call with:
```typescript
const confirm = useConfirm();
const confirmed = await confirm({
  title: 'Abort Planning',
  message: 'Are you sure you want to abort this planning session? The workspace and any progress will be preserved.',
  confirmLabel: 'Abort',
  variant: 'danger',
});
if (!confirmed) return;
...
### Label fix
In the abort-planning endpoint, add label removal after killing the session:
```typescript
...
For Linear issues, revert the state back to the previous state (already partially done via the `revertedState` logic).
...
- `plan.id` MUST be the issue ID in lowercase (e.g., "pan-445")
