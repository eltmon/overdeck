...
_Last sequenced: 2026-07-19T00:05:48Z · model: glm-5.2 · open: 637_
...
| 9 | PAN-2817 | M | critical | ok |  |  | Redrive idle-at-prompt work/review agents — gpt-5.6-sol stops at composer mid-task for hours. |
| 10 | PAN-2888 | M | critical | ok |  |  | Close-out leaves stale residue inflating troubled/failed metrics: orphaned inspect sub-agents + uncleared review_status rows. |
| 11 | PAN-2886 | M | critical | ok |  |  | Placeholder 'pending-work-spawn' model crashes auto-resume → agents stranded troubled forever. |
| 12 | PAN-2880 | L | critical | ok |  |  | Linear listIssues is a 3N+1 request storm — one MYN gather burns the entire 2500/hr budget. |
| 13 | PAN-2887 | M | critical | ok |  |  | Pipeline membership infers 'non-PR landing' from absence of unmerged branch — false post_merge_limbo. |
| 14 | PAN-2599 | L | high | ok |  |  | Integrate PostHog product analytics + telemetry |
| 14 | PAN-2882 | L | critical | ok |  | PAN-2877 | Pipeline membership has no GitLab merged-MR oracle — squash-merged branches read as unmerged. |
| 15 | PAN-2731 | M | critical | ok |  |  | state.json reports costSoFar=0 + frozen lastActivity for actively-committing codex agents → doctrine misclassifies them as ghosts and routes |
| 16 | PAN-2874 | L | critical | ok |  |  | Strike landing can't merge: verify gate demands a vBRIEF checklist strikes never have + feedback wedges on exited strike agents. |
| 17 | PAN-2883 | S | high | ok |  |  | Close-out deploy row fails for every strike-landed issue — PR resolver hardcodes feature/, misses strike/. |
| 18 | PAN-1560 | M | critical | ok |  |  | Re-review after PR head moves doesn't re-post panopticon/review status → PR stranded BLOCKED forever. |
| 19 | PAN-1650 | L | critical | ok |  |  | Split readyForMerge → gatesPassed (derived/event-driven) + shipComplete; auto-dispatch ship when gates pass. |
| 20 | PAN-2567 | M | critical | ok |  |  | Reviewed+green PR stuck — deacon reconciles 'advancing verdict' every patrol, never dispatches test/merge. |
| 21 | PAN-2569 | M | critical | ok |  |  | Planning finalizes (issue→planned) but work agent never auto-spawns — silent stall. |
| 22 | PAN-2650 | L | critical | ok |  |  | Swarm final ready-to-merge slot wedges when memory-governor sheds the integration stack; no recovery path. |
| 23 | PAN-2324 | S | critical | ok |  |  | Close-out label transition fails atomically on missing 'in-planning' label — closed-out label never applies. |
| 24 | PAN-2165 | M | critical | ok |  |  | pan close reports success but leaves issue OPEN / wrong labels (two distinct failures). |
| 25 | PAN-2186 | M | high | ok |  |  | Post-merge lifecycle can leave merged issues in-review and auto-merge rows 'merging' forever. |
| 26 | PAN-2516 | M | high | ok |  |  | Spec plan.status flips left uncommitted in shared primary worktree → spec-vs-record drift + blocks push loop. |
| 27 | PAN-2323 | M | high | ok |  |  | Flywheel respawn after crash/displacement starts a blank session instead of resuming the live one. |
| 28 | PAN-2337 | M | high | ok |  |  | Reload/build atomicity: in-place `npm run build` under a live dashboard breaks every new PTY-supervisor spawn. |
| 29 | PAN-2379 | M | high | ok |  |  | Verify gate dependency install is warn-only + 60s timeout → false verify failures against partial node_modules. |
| 30 | PAN-2593 | S | high | ok |  |  | Dashboard server children inherit bare system PATH → verification gates run Node 18, not 22. |
| 31 | PAN-2170 | M | high | ok |  |  | Workspace Docker init container lacks Python — node-gyp rebuild of better-sqlite3 fails for every new workspace. |
| 32 | PAN-2511 | M | high | ok |  |  | Work agents burn 20+ min on false test failures — sandbox denies spawnSync git (EPERM); looks like a code regression. |
| 33 | PAN-2179 | M | high | ok |  |  | Relaunch can leave a zombie agent — session alive but kickoff never delivered; liveness checks fooled. |
| 34 | PAN-2169 | M | high | ok |  |  | kimi agent silently frozen at 100% ctx (no thrown overflow error) not caught — deacon never respawns. |
| 35 | PAN-2331 | S | high | ok |  |  | Codex rate-limit 'Switch to gpt-5.4-mini?' modal stalls autonomous agents (no dismiss path). |
| 36 | PAN-2521 | S | high | ok |  |  | Launch pipeline agents with the harness rate-limit model-switch reminder disabled. |
| 37 | PAN-2639 | M | high | ok |  |  | codex-resume replays a rotated-out (revoked) refresh token → codex review convoys wedge with no verdict. |
| 38 | PAN-1770 | M | high | ok |  |  | pan-dir auto-commit rebase races live .pan/continues writes — 'rebase failed' recurs, main carries unpushed state. |
| 39 | PAN-2451 | M | high | ok |  |  | Overflow-restart + auto-commit + merge-main strands work agent behind commit-msg gate. |
| 40 | PAN-2193 | M | high | ok |  |  | Held issues (objection/parked/vetoed/needs-handoff) invisible in Command Deck tree. |
| 41 | PAN-2850 | S | high | ok |  |  | npm test fails in clean checkout — pretest removes dashboard bundle that dashboard-cwd-guard test needs. |
| 42 | PAN-1454 | L | critical | ok |  | PAN-1498, PAN-1499, PAN-1500, PAN-1501 | [META] 9 systemic failure patterns from 80-issue audit — substrate work to prevent each. |
| 43 | PAN-807 | L | critical | ok |  | PAN-806 | Epic C: Workspace state sanity on spawn — stop hard-reset + pre-flight checks. |
| 44 | PAN-1666 | XL | critical | ok | ✓ |  | [EPIC] Pipeline Throughput Hardening — throttle deacon, on-demand specialists, slot manager, fly.io scale-out. |
| 45 | PAN-2075 | XL | critical | ok | ✓ |  | [EPIC] Boot Reconciliation + Operator Inbox — informed, substrate-complete (local + Fly), reachable online/CLI/offline. |
| 46 | PAN-2642 | L | high | ok | ✓ |  | [EPIC] Cost strategy: waste detection over budget policing — retire invented limits, progress-aware breaker, honest dollars. |
| 47 | PAN-2059 | L | high | ok | ✓ |  | [EPIC] Backlog pickup gate — operator Plan→Release row + AI Objection (5th state) + Flywheel relevance-vetting. |
| 48 | PAN-2376 | XL | critical | ok |  |  | Epic: CI/CD reliability — flake policy, verification-to-merge convergence, strike/swarm merge paths. |
| 49 | PAN-2377 | L | critical | ok |  |  | first-class 'special orders' runs — operator-supplied order book executed with lane semantics |
| 49 | PAN-2079 | L | high | ok |  | PAN-2075, PAN-1520, PAN-2717, PAN-2492, PAN-104, PAN-1844 | Operator Inbox: durable server-side queue + in-dashboard surface (the notification spine). |
| 50 | PAN-2077 | M | high | ok |  | PAN-2075, PAN-1775 | Substrate-complete reconciliation inventory (local tmux + remote Fly) — one resolver both surfaces consume. |
| 51 | PAN-2078 | M | high | ok |  |  | CLI parity for boot reconciliation: pan boot status + pan resume --all|--select|--freeze|--kill-remote. |
| 52 | PAN-2080 | M | medium | ok |  | PAN-2075, PAN-2079, PAN-43 | Operator Inbox external transports (email/Slack/push/TTS) — offline reach, fast-follow. |
| 53 | PAN-1504 | M | high | ok |  |  | feat(cli): pan hygiene — codify the merge/commit/push state audit as a first-class verb. |
| 54 | PAN-1618 | M | high | ok |  |  | Substrate: work-spawn docker-health gate has no autonomous recovery — proposed work can't auto-start. |
| 55 | PAN-262 | L | high | ok |  |  | Refactor post-merge lifecycle into composable, idempotent operations. |
| 56 | PAN-2558 | L | high | ok |  |  | feat(state-migration): support polyrepo projects — resolve state-host repo via pan_records. |
| 57 | PAN-1889 | M | high | ok |  |  | feat(flywheel): retention/compaction policy for docs/FLYWHEEL-STATE.md — it grows unbounded. |
| 58 | PAN-2189 | L | high | ok |  |  | Decompose src/lib/cloister/deacon.ts (3,394 lines) — supervised handoff only (TENET-10). |
| 59 | PAN-2190 | L | high | ok |  |  | Decompose routes/workspaces/merge-ops.ts (1,925 lines) — new god file from the workspaces.ts split. |
| 60 | PAN-2233 | L | high | ok |  |  | refactor(cloister): decompose merge-agent.ts (1,414 lines) into focused modules. |
| 61 | PAN-1313 | L | high | ok |  |  | Finish the src/lib Effect migration — remove/justify the Promise/sync compatibility bridge. |
| 62 | PAN-1246 | L | high | ok |  |  | Perf: projection-cached VCS driver for diff/checkpoint reads (port of t3code #2586). |
| 63 | PAN-1311 | M | high | ok |  |  | Swarm: fast-track tier — skip slot dispatch for trivial mechanical items. |
| 64 | PAN-2859 | M | high | ok |  |  | Add Kimi K3 (k3 / k3[1m]) to claude-code Kimi provider — parity with kimi-k2.7-code. Strike. |
| 65 | PAN-1766 | S | high | ok |  |  | Work agents hang on Claude Code .claude/** settings-file protection prompt. |
| 66 | PAN-2709 | M | high | ok |  |  | Flywheel orchestrator unreachable as a notification target — feedback dead-ends. |
| 67 | PAN-2868 | S | medium | ok |  |  | Desktop window opens at fixed 1400×900 — persist window state, default first run to maximized. |
| 68 | PAN-2430 | S | high | ok |  |  | Frontend typecheck fails with dozens of pre-existing unused-local errors — blocks verification gate. |
| 69 | PAN-1452 | M | medium | ok |  |  | PAN-1381 follow-up: per-reviewer restart with model override (architecturally impossible post PAN-1048). |
| 70 | PAN-1767 | S | high | ok |  |  | Show merged-but-not-closed-out count in pan status and the dashboard headline. |
| 71 | PAN-2334 | M | high | ok |  |  | chore(process): write a Definition of Ready (DoR) — the bar before planning/pickup. |
...
| 79 | PAN-1253 | S | high | ok |  |  | Flywheel: respect issue dependencies before autopicking work. |
| 80 | PAN-2027 | M | high | ok |  | PAN-1989 | ohmypi: route kimi-k2 through ohmypi harness instead of CLIProxy (eliminates 200k-window deadlock). |
| 81 | PAN-1578 | L | high | ok |  |  | GitHub Copilot CLI as a first-class harness (pipeline peer to Claude Code, Pi, Codex). |
| 82 | PAN-1451 | M | medium | ok |  |  | PAN-1124 follow-up: complete planning-on-main pivot (dropped ACs from scope drift). |
| 83 | PAN-2358 | S | high | ok |  |  | PAN-2145 follow-up: restore PAN-1535 hardening in transformMessageForHarness (rewritten, not moved). |
| 84 | PAN-2706 | M | high | ok |  |  | Ghost test sessions absorb every test dispatch — never-kicked-off session reads as 'already running'. |
| 85 | PAN-2720 | S | medium | ok |  |  | File-size ratchet counts lines, so it rewards line-packing on the god files it means to shrink. |
| 86 | PAN-2259 | M | high | ok |  |  | bug(infra): something burns the full 5k/hr GitHub GraphQL quota — repeatedly breaks pan close. |
| 87 | PAN-2848 | M | high | ok |  |  | Work agent stalls forever on a dead inspection: no re-dispatch, verdict never delivered. |
| 88 | PAN-1142 | M | medium | ok |  |  | Add reasoning effort level to per-role / per-conversation model config. |
| 89 | PAN-2106 | M | high | ok |  |  | pan strike workspace setup leaves broken partial workspace + false 'spawned' success (git-lock race) |
| 90 | PAN-1209 | M | high | ok |  |  | PAN-1052 bead projection disagrees with bd state |
| 91 | PAN-2421 | M | high | ok |  |  | bug(test): dashboard server route tests flake under full-suite verification load |
| 92 | PAN-955 | M | high | ok |  |  | Workspace devcontainer template versioning + re-render on demand |
| 93 | PAN-1198 | M | high | ok |  |  | Workspace init container's bun install doesn't populate container-node-modules named volume |
| 94 | PAN-1497 | M | high | ok |  |  | feat(flywheel): emit TTS announcements on lifecycle events (start, pause, resume, report) |
| 95 | PAN-2188 | M | high | ok |  |  | Flywheel resilience for the codebase-health flood: substrate-first prioritization + tenets spirit-gate |
| 96 | PAN-813 | M | high | ok |  |  | Add regression test for /api/review/:issueId/reset preserving work-agent resolution |
| 97 | PAN-1538 | M | medium | ok |  |  | Unblock Pi source forks — remove API guard, verify transcript parsers |
| 98 | PAN-1558 | M | medium | ok |  |  | Review/specialist agents should run in the workspace Docker container, not inherit host-override |
| 99 | PAN-630 | M | medium | ok |  |  | Multi-tenant workspace isolation with ACLs |
| 100 | PAN-1254 | M | medium | ok |  |  | Tailscale integration: advertise dashboard + workspace endpoints over tailnet (Effect-native) |
| 101 | PAN-1357 | M | medium | ok |  |  | Template conversations: load curated skill bundles into a single conversation |
| 102 | PAN-1424 | M | medium | needs-refinement |  |  | Model pool dispatch + work.* subtype taxonomy (follow-up to PAN-1122) |
| 103 | PAN-1561 | M | medium | ok |  |  | feat: Project-scoped dashboard nav (deck of tabs per project + conversations/tree column + activity feed) |
| 104 | PAN-1913 | M | medium | ok |  |  | Project description: show on click, edit in dashboard, mirror into the project layer (and document what's in .pan and ~/.panopticon) |
| 105 | PAN-1544 | M | medium | ok |  |  | Type cleanup: strip 'ship' from the Role union and its ~10 downstream references |
| 106 | PAN-578 | M | medium | ok |  |  | Security: Comment mediation layer to prevent prompt injection via tracker comments |
| 107 | PAN-1416 | S | medium | ok |  |  | Workspace-spawned dashboards must never claim the canonical dashboard port |
| 108 | PAN-1435 | M | medium | ok |  |  | API keys in ~/.panopticon/config.yaml stored as plaintext |
| 109 | PAN-1915 | M | medium | ok |  |  | enhancement(security): API key at-rest hardening — startup perm check + OS keychain + deprecate plaintext |
| 110 | PAN-538 | S | low | ok |  |  | pan reload freshness guard must also verify the frontend bundle |
| 111 | PAN-1556 | S | low | ok |  |  | Session/activity feed: coalesce review-spawn spam, supersede re-reviews per issue, keep active conversations most-recent |
| 112 | PAN-933 | M | low | ok |  |  | Review poster cannot post to GitLab MRs (only supports GitHub PRs) |
| 113 | PAN-1027 | S | low | ok |  |  | Merge-status drift: deacon auto-detect paths set mergeStatus=merged without postMergeLifecycle, never reset on revert |
| 114 | PAN-1436 | S | low | ok |  |  | PAN-1419 follow-up: stale stopped-agent zombies still pollute dashboard list |
| 115 | PAN-2069 | S | low | ok |  |  | caveman: follow-up gaps — review agent routing, hook execution tests, Settings UI toggle, Experiments view |
| 116 | PAN-304 | S | low | ok |  |  | closeLinearDirect returns stepOk even when state update never happens |
| 117 | PAN-1769 | L | low | ok |  |  | Supervisor echo-confirm false negative on long messages → triple-paste delivery (rewrite ×2 + tmux fallback); resumed-conv message still eat |
| 118 | PAN-2695 | S | low | ok |  |  | Concurrent review dispatches race fresh-spawn vs resume — second dispatch resumes a still-booting parent and kills the synthesis kickoff |
| 119 | PAN-2742 | S | low | ok |  |  | bug(review): synthesis fires 42s after spawn and reports reviewers with reports on disk as 'infrastructure failure' — false CHANGES REQUESTE |
| 120 | PAN-1226 | L | low | ok |  |  | PAN-1148 unified-dashboard redesign — 32 gaps vs PRD and mockups (full audit) |
| 121 | PAN-1433 | S | low | ok |  |  | Conversation agents can leave host main repo in abandoned git rebase state for hours |
| 122 | PAN-1130 | S | low | ok |  |  | Headless review sub-reviewer normal exit misclassified as 'crashed', triggers spurious restart |
| 123 | PAN-1392 | S | low | ok |  |  | pan close: archive-planning:move-prd fails when completed/ PRD exists but workspace PRD also exists |
| 124 | PAN-1438 | S | low | ok |  |  | pan flywheel start launcher process orphans when orchestrator dies externally |
| 125 | PAN-1440 | S | low | ok |  |  | Follow-up to PAN-1158: bd export --refuse-empty guard + dolt-empty root cause |
| 126 | PAN-247 | S | low | ok |  |  | Deacon has no backoff or escalation for repeated specialist startup failures |
| 127 | PAN-245 | S | low | ok |  |  | Ctrl+C aborts planning dialog instead of copying text |
| 128 | PAN-244 | S | low | ok |  |  | Deep-wipe leaves local branch and worktree metadata behind |
| 129 | PAN-1711 | S | low | ok |  |  | Root-cause and fix dashboard event-loop stalls under load |
| 130 | PAN-1525 | L | critical | ok |  |  | Substrate work; improves the foundation required for reliable shipping. |
| 130 | PAN-1795 | S | low | ok |  |  | Codebase map bootstrapped in planning worktree is never promoted to main (PAN-1788 WI-6 wiring gap) |
| 131 | PAN-1824 | S | low | ok |  |  | Fix flaky main CI: fake timers + @slow exclusion for real-timer test family |
| 132 | PAN-1816 | S | low | ok |  |  | Scratch/UAT-lifecycle issues (PAN-18031) enter the real pipeline: kanban, review convoys, agent registry — need an ephemeral flag + auto-cle |
| 133 | PAN-324 | S | low | ok |  |  | Agent detail pane missing Merge/Approve button |
| 134 | PAN-2237 | S | low | ok |  |  | bug(cli): pan plan done swallows vbrief quality lint details |
| 135 | PAN-2495 | S | low | ok |  |  | PAN-2487 ci-green merge skip bypassed CI-green gate — landed red-main change |
| 136 | PAN-681 | S | low | ok |  |  | Feedback routing: wrong issueId written to workspace when verification runs for co-active issues |
| 137 | PAN-2656 | S | low | ok |  |  | bug(test): deacon-swarm unit tests read live ~/.overdeck/config.yaml — 6 tests fail whenever swarm.mode=off |
| 138 | PAN-2700 | S | low | ok |  |  | Test artifact recovery consumes a stale .pan/test/result.json — fresh test dispatch insta-failed with the previous run's verdict |
| 139 | PAN-2691 | S | low | ok |  |  | Auto-planned issues park silently when the post-finalize work spawn is gated (stack-unhealthy 422) — no retry, no needs-you |
| 140 | PAN-2746 | S | low | ok |  |  | bug(review): infra-failure bypass writes reviewStatus='passed' — indistinguishable from a real approval; nearly merged PAN-2710 unreviewed |
| 141 | PAN-2733 | S | low | ok |  |  | bug(dashboard): substrate-bug-poller has never run — BOT_LOGIN is a git author string, not a GitHub user (49,907 failed polls) |
| 142 | PAN-2759 | S | low | ok |  |  | Dead flywheel with an active run was never auto-relaunched after a reboot — sat idle 2h with recovery wired and enabled |
| 143 | PAN-2758 | S | low | ok |  |  | Provider capacity error silently zombies a spawned agent: willRetry=false, turn reported completed, state stays status=running forever |
| 144 | PAN-2747 | S | low | ok |  |  | Flywheel cannot be resumed after a crash/reboot: Resume is disabled and the only offered action aborts the run |
| 145 | PAN-2846 | S | low | ok |  |  | Close-out blocks on a dead agent: postMergeLifecycle pauses the work agent but leaves status=running |
| 146 | PAN-2839 | S | low | ok |  |  | plan→work autoSpawn now 500s with a duplicated workspace prep — nondeterministic half-spawns (post-PAN-2825) |
| 147 | PAN-49 | S | low | ok |  |  | Fix CloisterService tests that require real runtime |
| 148 | PAN-1444 | S | low | ok |  |  | Follow-up to PAN-1416: dashboard port lockfile + pan doctor multi-instance check |
| 149 | PAN-1461 | S | low | ok |  |  | Conversation transcript: in-page search (Ctrl+F) only finds text in currently-rendered virtualized rows |
| 150 | PAN-113 | S | low | ok |  |  | Dashboard 'Start Agent' returns success before verifying agent actually started |
| 151 | PAN-886 | S | low | ok |  |  | pan review request shows 'fetch failed' instead of actual sync-target-branch error |
| 152 | PAN-932 | S | low | ok |  |  | pan done: polyrepo uncommitted changes check + existing MR handling |
| 153 | PAN-1113 | S | low | ok |  |  | Conversations sidebar lets you message review-specialist sessions, which derails them silently |
| 154 | PAN-1129 | M | low | ok |  |  | Review-request route pushes wrong branch name: 'feature/977' instead of 'feature/pan-977' |
| 155 | PAN-1149 | S | low | ok |  |  | v0.9.3 upgraders: stale workhorses.mid: claude-sonnet-4-7 in config.yaml keeps breaking Model Routing saves |
| 156 | PAN-1227 | M | low | needs-refinement |  |  | Substrate: bead can be closed without delivering the work — add per-bead delivery check in pan done |
| 157 | PAN-1240 | S | low | ok |  |  | Ship-complete PRs going CONFLICTING after main moves need auto re-rebase recovery |
| 158 | PAN-1330 | S | low | ok |  |  | CLI cannot address planning-*/specialist-* sessions — pan tell/pan kill hard-code 'agent-' prefix; no 'pan plan abort' |
| 159 | PAN-1386 | S | low | ok |  |  | Flywheel orchestrator never emits status snapshots — dashboard 'flywheel' pane stays blank during an active run |
| 160 | PAN-1530 | S | low | ok |  |  | Investigate: state.json with model='gpt-5.5' (a model that doesn't exist) |
| 161 | PAN-1565 | S | low | ok |  |  | Defensive mitigation: auto-recover conversations poisoned by Claude Code thinking-block resume 400 (upstream #63147) |
| 162 | PAN-1571 | S | low | ok |  |  | Large multi-line pastes (handoff docs) land unsubmitted — paste/submit verification is blind to Claude's collapsed "[Pasted text +N lines]"  |
| 163 | PAN-454 | M | low | ok |  |  | Crash recovery: detect orphaned agents and present recovery UI on dashboard startup |
| 164 | PAN-1624 | S | low | ok |  |  | pan handoff --author external: authored doc is socket_write-ten but never submitted — successor sits at empty welcome screen |
| 165 | PAN-532 | M | low | ok |  |  | Per-project and per-issue model overrides for pipeline roles |
| 166 | PAN-1674 | S | low | ok |  |  | TLDR .venv (~7.5G) is duplicated into every workspace — 236G across 33 worktrees, caused disk-full ENOSPC |
| 167 | PAN-1830 | S | low | ok |  |  | Reviewer stuck on gpt-5.5 rate-limit modal blocks REVIEWER_READY — synthesis waits forever despite report written (PAN-1696) |
| 168 | PAN-1828 | S | low | ok |  |  | Conversation fork/handoff harness defaults ignore source conversation harness — silent claude-code coercion |
| 169 | PAN-334 | S | low | ok |  |  | Dashboard server has no duplicate-process protection — zombie instances cause 502 |
| 170 | PAN-1951 | M | low | ok |  |  | Inspector resumes a warm per-issue session instead of cold-spawning per item |
| 171 | PAN-817 | M | low | ok |  |  | Improve planning dialog layout and content fit |
| 172 | PAN-2202 | S | low | ok |  |  | complete-planning silently skips spec promotion on a dead session's unanswered AskUserQuestion — and finalize reports false success |
| 173 | PAN-2241 | S | low | ok |  |  | complete-planning is not serialized or idempotent per issue (spec tmp-rename 500s, bead delete-recreate thrash) |
| 174 | PAN-2243 | S | low | ok |  |  | pan plan finalize: CLI aborts complete-planning at 90s while the server handler legitimately finishes later (false ✖ Failed) |
| 175 | PAN-2242 | S | low | ok |  |  | Unidentified duplicate caller fires complete-planning in pairs every ~2 minutes (perpetual loop while session survives) |
| 176 | PAN-2467 | S | low | ok |  |  | Multi-repo merge train merges only one repo, strands sibling repos' branches (MIN-857 api half never merged) |
| 177 | PAN-1040 | M | low | ok |  |  | feat(infra): event-driven dispatch for inspect-agent (requiresInspection=true beads) |
| 178 | PAN-2478 | S | low | ok |  |  | CI flake: Playwright browser install fails on packages.microsoft.com apt (NOSPLIT), red-mains legit merges |
| 179 | PAN-2627 | S | low | ok |  |  | bug(tracker): Linear poller is blind after cycle rollover — active-cycle filter returns 0 issues, wiping the whole project from the issue tr |
| 180 | PAN-2659 | S | low | ok |  |  | fs-lock: crash between mkdir(lock) and owner.json write leaves an unreclaimable record lock (successor to #2623) |
| 181 | PAN-2664 | S | low | ok |  |  | bug(sync-main): auto-commit completes unresolved merge with conflict markers |
| 182 | PAN-2689 | S | low | ok |  |  | Review verdicts from sandboxed codex review agents are silently lost — fire-and-forget journal write dies with the CLI process |
| 183 | PAN-2672 | S | low | ok |  |  | Post-/clear siblings render the same original transcript (per-tmux resolution + frozen launcher pin + null claude_session_id) |
| 184 | PAN-2697 | S | low | ok |  |  | First-review codex parents enter discovery mode and the supervisor session no-ops every discovery-ready signal — convoy never launches |
| 185 | PAN-2696 | S | low | ok |  |  | Task views still speak beads vocabulary — completed vBRIEF items shown as 'upcoming', plus phantom 'not synced' label |
| 186 | PAN-2739 | S | low | ok |  |  | bug(cloister): first-completion detection throws every patrol cycle — non-null assertion on getAgentRuntimeStateSync kills the pan-done nudg |
| 187 | PAN-2738 | S | low | ok |  |  | bug(cli): strikes deadlock — 'git rebase origin/main' denied as history rewriting, so they cannot sync, gate, or push |
| 188 | PAN-2734 | S | low | ok |  |  | bug(cloister): merge queue head-of-line zombie — closed PAN-2325 re-triggered on all 294 boots; removeMerge has zero callers |
| 189 | PAN-2775 | S | low | ok |  |  | Agents die in sweeps: boot-correlated false reaps (live flywheel reaped, convoy reaped 5x) + unexplained simultaneous 3-host kill at 04:34Z |
| 190 | PAN-2769 | S | low | ok |  |  | review_status rows are never reconciled when an issue closes — 9 closed issues still advertise reviewing/failed, inflating every operator co |
| 191 | PAN-2792 | S | low | ok |  |  | Orphan-process sweeps killed the dashboard and live conversations via lsof +D over Bun-hardlinked node_modules |
| 192 | PAN-2805 | S | low | ok |  |  | FlywheelPage shows 'No active run' while /api/flywheel/current returns a live run — open-questions reveal lands nowhere |
| 193 | PAN-2828 | S | low | ok |  |  | pan done --strike always refuses squash-merged strikes (--is-ancestor can't see through a squash) |
| 194 | PAN-2824 | S | low | ok |  |  | pan review pending dies when one project's lens gather fails (non-degrading caller; PAN-2820 class) |
| 195 | PAN-1443 | M | low | ok |  |  | Follow-up to PAN-487: migrate 10 stale .vbrief.json files from docs/prds/active/ to completed/ |
| 196 | PAN-863 | M | low | ok |  |  | One-shot sweep of stale feature branches and worktrees predating the reaper |
| 197 | PAN-900 | S | low | ok |  |  | Trust devroot for conversations + atomic .claude.json writes |
| 198 | PAN-1042 | S | low | ok |  |  | cost_events retention: 14 months of granular rows accumulating with ad-hoc partial deletions |
| 199 | PAN-1068 | S | low | ok |  |  | PAN-1048 deferred findings: security, correctness, and model validation gaps |
| 200 | PAN-1128 | S | low | ok |  |  | Channels: spurious 'no MCP server configured with that name' banner at conversation startup |
| 201 | PAN-1150 | S | low | ok |  |  | Settings: "Anthropic is not configured" warning persists in Model Routing after claude /login (Provider tab disagrees) |
| 202 | PAN-1173 | S | low | ok |  |  | pan show <bare-number> derives wrong agent ID for PAN-prefixed issues |
| 203 | PAN-1985 | M | low | ok |  |  | Agent wipe-and-respawn family (work + review): harness/model switch + Complete work reset, with confirmation |
| 204 | PAN-1995 | M | low | ok |  |  | infra: set up smee webhook relay so merge-on-green + post-merge are reactive (not deacon-only) |
| 205 | PAN-1449 | S | low | ok |  |  | PAN-1052 follow-up: memory extraction failing 59% on dogfood project + storage layout deviates from spec |
| 206 | PAN-1446 | M | low | ok |  |  | PAN-1231 follow-up: remove or implement Table + Timeline modes in FleetAgentsView (scope-creep stubs) |
| 207 | PAN-1445 | M | low | ok |  |  | PAN-1389 follow-up: remove or implement Files + Comments tabs in SessionFeedSidebar (scope-creep stubs) |
| 208 | PAN-2335 | M | low | ok |  |  | chore: review the full open backlog for junk/stale/nonsensical issues — produce a categorized document for operator review (FIND ONLY, do NO |
| 209 | PAN-1673 | S | low | ok |  |  | Regression: pi + gpt-5.5 fails with 'No API key for provider: openai-codex' (worked previously) |
| 210 | PAN-1751 | M | low | ok |  |  | feat(settings): harness picker on every Settings → Roles row (plan/work/review/test/ship/strike), not just Flywheel |
| 211 | PAN-2582 | M | low | ok |  |  | feat(swarm): show slot assignments on the vBRIEF DAG + unify swarm/tiered terminology (Lead/Crew or Trunk/Lanes) |
| 212 | PAN-1912 | S | low | ok |  |  | Pi agent transcripts hide tool-call detail; agent panes lack the Tools show/hide toggle |
| 213 | PAN-2005 | M | low | ok |  |  | Backlog Sequencer: Pickup Forecast — visualize Flywheel pickup order (waves, lanes, planning bottleneck) |
| 214 | PAN-38 | M | low | ok |  |  | Support multiple merge agents per repository |
| 215 | PAN-37 | M | low | ok |  |  | Support external PR selection for merge-agent |
| 216 | PAN-924 | M | low | ok |  |  | Spike: evaluate GitNexus for Panopticon integration |
| 217 | PAN-2210 | M | low | ok |  |  | PAN-2203 follow-up: a swarm slot's completion can trigger the issue-level review pipeline |
| 218 | PAN-2212 | M | low | ok |  |  | Swarm slot dispatch has no reserved budget — a busy pipeline starves it to zero |
| 219 | PAN-947 | M | low | ok |  |  | feat: project management actions in unified sidebar |
| 220 | PAN-2244 | S | low | ok |  |  | Recurring [pan-dir/auto-commit] GitError on main — half-staged spec file blocks all pan-dir mirroring (continue mirrors never land) |
| 221 | PAN-2240 | S | low | ok |  |  | bug(agents): pan tell contradicts itself on dead ohmypi sessions — 'session is dead and resume failed: it appears healthy' |
| 222 | PAN-958 | M | low | ok |  |  | Implement vBRIEF issue sync: migrate and reconcile GitHub issues into specification |
| 223 | PAN-2406 | M | low | ok |  |  | close-out gaps: verify-merged rejects record-only deltas; slot/suffixed worktrees never torn down; teardown abort fires after worktree remov |
| 224 | PAN-537 | M | low | ok |  |  | feat: show changed files diff summary after each agent response in activity view |
| 225 | PAN-1037 | M | low | ok |  |  | Retire 'planning-' tmux prefix — fold into agent-PAN-N keyed by phase |
| 226 | PAN-1041 | M | low | ok |  |  | Audit and consolidate REMOTE/LOCAL gates in work-agent prompt template |
| 227 | PAN-2550 | S | low | ok |  |  | bug(test): npm test exits 0 despite root-suite failures — 31 failed tests reported green at the command level |
| 228 | PAN-2547 | S | low | ok |  |  | bug(cli): pan restart --health-timeout parses seconds as milliseconds — '--health-timeout 180' waits 180ms then declares failure |
| 229 | PAN-2546 | S | low | ok |  |  | bug(cli): pan tell is codex-conversation-unaware — declares live codex sessions zombie and refuses delivery |
| 230 | PAN-2554 | S | low | ok |  |  | bug(dashboard): clicking a project doesn't update the browser URL — project view isn't copyable/shareable/bookmarkable |
| 231 | PAN-2563 | S | low | ok |  |  | npm-flavor desktop (npx @overdeck/desktop) lacks node_modules for the server's externalized deps |
| 232 | PAN-646 | M | low | ok |  |  | Canceled issues: add guided Recover workflow |
| 233 | PAN-2580 | S | low | ok |  |  | pan tell cannot deliver to codex (GPT) conversations — runtime stays null, delivery door misclassifies live session as zombie |
| 234 | PAN-709 | M | low | ok |  |  | feat(flywheel): self-improving flywheel — retro agent, skill-change pipeline, audience-scoped skills, Q&A detection, autonomous daemon |
| 235 | PAN-2649 | S | low | ok |  |  | bug(palette): Ctrl+K conversation search indexes Claude transcripts only |
| 236 | PAN-2670 | S | low | ok |  |  | Gate the dashboard-server tsconfig in npm run typecheck — the server graph has no type enforcement (161 pre-existing errors) |
| 237 | PAN-2663 | S | low | ok |  |  | bug(restart): health probe can accept old dashboard after replacement EADDRINUSE |
| 238 | PAN-2686 | S | low | ok |  |  | Policy strip "restart pending" badge never clears after restart-fresh with a new model (record.model is sticky) |
| 239 | PAN-2678 | M | low | ok |  |  | Ops: clean blocked state worktrees, fix auricle git-status failure, restore the Deacon (2026-07-14 review outage) |
| 240 | PAN-2699 | S | low | ok |  |  | npm run build regenerates the committed record-cost-event.js bundle — every workspace build dirties the tree and blocks clean-workspace gate |
| 241 | PAN-2717 | S | low | ok |  |  | bug(dashboard): conversation permission waits missing from Awareness; strengthen alert pulse |
| 242 | PAN-2763 | S | low | ok |  |  | Workspace node_modules is symlinked to the primary repo, breaking test resolution — the pattern CLAUDE.md explicitly forbids |
| 243 | PAN-2749 | S | low | ok |  |  | Resume restores the conversation but not the machinery: timers, monitors and background processes die and are never re-armed |
| 244 | PAN-2761 | S | low | ok |  |  | done.test.ts asserts a hardcoded URL without stubbing env, so it fails in any agent shell with OVERDECK_DASHBOARD_URL set and looks like a r |
| 245 | PAN-1164 | M | low | ok |  |  | Conversation diff summaries update live over WebSocket (drop 5s polling) |
| 246 | PAN-2802 | S | low | ok |  |  | bug(cloister): same-head strike-ready cannot re-arm a needs-you landing |
| 247 | PAN-2820 | S | low | ok |  |  | CRITICAL: main HEAD dashboard build stalls in boot before HTTP listen (running a9e301526b rollback) |
| 248 | PAN-47 | M | low | ok |  |  | PRD files should be committed to feature branch, moved to completed/ on merge |
| 249 | PAN-810 | M | low | ok |  |  | Inspector: diagnostic UI when pipeline phase is unknown |
| 250 | PAN-1432 | M | low | ok |  |  | Merge agent leaves packages/contracts/dist stale — typecheck breaks on every fresh checkout |
| 251 | PAN-833 | M | low | ok |  |  | Agent spawn logs ENOTDIR for .git/pan-credentials in worktrees (GitHub App credential loader) |
| 252 | PAN-277 | M | low | ok |  |  | Session reasoning capture & collaborative PRD refinement |
| 253 | PAN-1577 | M | low | ok |  |  | Move a conversation to a different project (CLI + drag/drop + menu action) |
| 254 | PAN-1657 | M | low | ok |  |  | feat: one-off double-check reviews with a user-specified agent/harness + settings-managed default reviewer |
| 255 | PAN-1655 | M | low | ok |  |  | Skills: scope by audience AND by agent role (conversation/work/review/ship/plan/test), sync accordingly |
| 256 | PAN-1672 | M | low | ok |  |  | GPT-5.5/CLIProxy context-window deadlock: conversations get no overflow recovery + 200k window illusion |
| 257 | PAN-178 | M | low | ok |  |  | PAN-178: Crash recovery with granular task checkpointing |
| 258 | PAN-176 | S | low | ok |  |  | PAN-176: Hook-enforced delegation guardrails for specialist agents |
| 259 | PAN-1840 | M | low | ok |  |  | Add 'pan switch <id>' — change a running agent's model/harness in one command (kill + fresh-start + re-onboard) |
| 260 | PAN-1136 | M | low | ok |  |  | Hook system cleanup: dead inspect-on-bead-close, pan-review-agent inconsistency |
| 261 | PAN-1967 | M | low | ok |  |  | Flywheel must re-validate (re-plan) pre-cutover plans before implementing them |
| 262 | PAN-1965 | M | low | ok |  |  | Project pipeline view: true-state buckets + lens reconciliation (pipeline as exception queue) |
| 263 | PAN-2065 | M | low | ok |  |  | feat(dashboard): unified usage & headroom panel across all provider plans (z.ai, Anthropic, Codex, OpenRouter) |
| 264 | PAN-1627 | M | low | ok |  |  | Substrate: Claude Code's native .claude/** settings-edit protection wedges in-scope work agents (un-overridable by PreToolUse auto-approve h |
| 265 | PAN-548 | M | low | ok |  |  | Command Deck: preserve state across navigation including URL routing for tabs |
| 266 | PAN-607 | M | low | ok |  |  | Evaluate Ultimate Bug Scanner (UBS) for verification gate |
| 267 | PAN-613 | M | low | ok |  |  | Investigate thinking effort levels for agents — reduce signature corruption frequency |
| 268 | PAN-1735 | M | low | ok |  |  | feat(flywheel): adopt externally-completed readyForMerge issues into the pipeline/merge queue |
| 269 | PAN-637 | M | low | ok |  |  | Direct issue kickoff (skip planning) from dashboard UI |
| 270 | PAN-678 | M | low | ok |  |  | pan work issue --auto: headless planning → agent handoff without interactive dialog |
| 271 | PAN-675 | M | low | ok |  |  | Deacon: detect API rate-limit events, surface on dashboard, auto-restart when window resets |
| 272 | PAN-1868 | S | low | ok |  |  | Cost-bleed circuit breaker: progress-aware, always-on guard against runaway agent spend |
| 273 | PAN-1986 | M | low | ok |  |  | restartAgent (change harness/model): wipe stale agent-dir session pointers + refresh conversations row |
| 274 | PAN-77 | M | low | ok |  |  | Cost breakdown modal: show costs by stage and model when clicking cost badge |
| 275 | PAN-903 | M | low | ok |  |  | Detect ~/.claude.json corruption on startup and surface it in the dashboard |
| 276 | PAN-938 | M | low | ok |  |  | Fizzy visual pipeline — Kanban mirror for specialist pipeline |
| 277 | PAN-2201 | S | low | ok |  |  | Close-out label step fails atomically when a hardcoded label (e.g. 'in-planning') is absent from the repo — closed issues keep stale labels |
| 278 | PAN-2211 | M | low | ok |  |  | PAN-2203 follow-up: swarm slot pan done records completion but slot never becomes merge-ready |
| 279 | PAN-2213 | M | low | ok |  |  | Swarm slot allocator picks an orphaned slot index and refuses instead of skipping to the next free one |
| 280 | PAN-480 | S | low | ok |  |  | Pass --effort flag when spawning planning agents via Cloister |
| 281 | PAN-2399 | M | low | ok |  |  | feat(tiered): wire replay_threshold/compaction_reroute into the slot-recovery respawn seam (PAN-2397 W3b) |
| 282 | PAN-2449 | M | low | ok |  |  | start-planning: GITHUB_REPOS env shadows projects.yaml github_repo; unknown IDs fall through to Linear and plan the wrong issue |
| 283 | PAN-2469 | M | low | ok |  |  | feat(swarm): issue-level assembly owner — 'all slots done' must deterministically trigger assemble → verify → review (root cause of PAN-2388 |
| 284 | PAN-2466 | M | low | ok |  |  | bug(records): close-out/record writer clobbers closeOut.usage with EMPTY data — cost history lost on the local side (recurring) |
| 285 | PAN-2507 | M | low | ok |  |  | Preemptive pipeline scheduler: yield idle work agents to unblock review/test/merge dispatch |
| 286 | PAN-603 | M | low | ok |  |  | Plan review loop with configurable reviewer model |
| 287 | PAN-1060 | S | low | ok |  |  | Self-modify permission handling: stop the interrupt loop without weakening the safety guard |
| 288 | PAN-623 | M | low | ok |  |  | Multi-channel workflow triggers: Slack, Discord, Telegram, GitHub webhooks |
| 289 | PAN-634 | M | low | ok |  |  | Documentation cleanup: restructure docs, update installation (npx panctl), refresh stale PRDs |
| 290 | PAN-633 | M | low | ok |  |  | Update Cloister PRD and docs index — stale relative to implementation |
| 291 | PAN-660 | M | low | ok |  |  | Slash menu command catalog drifts: hardcoded array in ComposerPromptEditor needs codegen |
| 292 | PAN-700 | M | low | ok |  |  | Detachable terminal for conversation view — popout into OS window |
| 293 | PAN-713 | M | low | ok |  |  | test: add unit tests for doneCommand and approveCommand |
| 294 | PAN-727 | M | low | ok |  |  | Fix orphaned work-agent start handoff after planning |
| 295 | PAN-1151 | M | low | ok |  |  | Anthropic Enterprise auth: distinguish from consumer subscription for Pi+Anthropic harness gating |
| 296 | PAN-1165 | M | low | ok |  |  | Lightweight review path for small/trivial PRs |
| 297 | PAN-764 | M | low | ok |  |  | Add quota/usage inspector for routed model providers |
| 298 | PAN-769 | M | low | ok |  |  | Track verification/review/test phase churn over time |
| 299 | PAN-775 | L | low | ok |  |  | Redesign workspace inspector panel: sidebar layout is cramped and wrong |
| 300 | PAN-778 | M | low | ok |  |  | Write conflict race: review-agent fails when test-agent write scope not yet released |
| 301 | PAN-802 | M | low | ok |  |  | Resume on conversation session forks instead of resuming |
| 302 | PAN-44 | M | low | ok |  |  | Planning should fetch ALL issue context: comments, attachments, linked issues, discussions |
| 303 | PAN-52 | M | low | ok |  |  | Guidance needed: Running complex multi-container projects with Panopticon worktrees |
| 304 | PAN-51 | M | low | ok |  |  | Documentation: Clarify issue tracker options beyond Linear |
| 305 | PAN-1437 | M | low | ok |  |  | pan flywheel report semantics: split read-only snapshot from run finalization |
| 306 | PAN-243 | M | low | ok |  |  | Audit dashboard actions: ensure all are available via CLI |
| 307 | PAN-255 | M | low | ok |  |  | Agents lack awareness of MCP tools — sync MCP config and inject into prompts |
| 308 | PAN-252 | M | low | ok |  |  | Disable Sync with Main button when workspace is up to date |
| 309 | PAN-1489 | M | low | needs-refinement |  |  | task(flywheel): tune v1.0 readiness criteria after 30 days of telemetry |
| 310 | PAN-1485 | M | low | ok |  |  | Auto-archive stale conversations: pre-archive warning at 7 days, archive at 10 days, configurable |
| 311 | PAN-1473 | L | low | ok |  |  | Dashboard conversation composer: refactor context indicator to mirror t3code (show cumulative + live separately) |
| 312 | PAN-1469 | M | low | ok |  |  | End-to-end review and consolidation of all project documentation |
| 313 | PAN-258 | M | low | ok |  |  | Kanban board: fit all columns without horizontal scrolling |
| 314 | PAN-832 | M | low | ok |  |  | state.json staleness: lastActivity/costSoFar not updated as agent runs; /api/agents drops phase/cost/lastActivity |
| 315 | PAN-1542 | M | low | ok |  |  | Spawn-refusal modal: render the three-button workflow on dirty-workspace 409 |
| 316 | PAN-1545 | M | low | ok |  |  | feat(dashboard): New Terminal button — spawn ad-hoc bash sessions from sidebar / conversation / drawer / palette |
| 317 | PAN-293 | M | low | ok |  |  | Project Living Memory — per-project semantic memory for agents |
| 318 | PAN-294 | M | low | ok |  |  | Surface module initialization errors as system-level, not per-issue |
| 319 | PAN-962 | M | low | ok |  |  | Post-PAN-946: vBRIEF lifecycle follow-up plan |
| 320 | PAN-1656 | M | low | ok |  |  | Skills page: make it a full management surface (browse, review, edit, scope, sync status) |
| 321 | PAN-1676 | M | low | ok |  |  | feat(fly.io): harden remote workspaces + `pan workspace move` local↔remote (scale-out / overflow slots) |
| 322 | PAN-1684 | M | low | ok |  |  | docs(marketing): build full marketing kit + plan (SEO, video list, channels) from MARKETING.md seed |
| 323 | PAN-190 | M | low | ok |  |  | PAN-190: Specialized reviewer prompts (industry best-practice checklists) |
| 324 | PAN-175 | M | low | ok |  |  | PAN-175: Pre-compact auto-save hook for agent sessions |
| 325 | PAN-1126 | M | low | ok |  |  | Integrate TLDR summaries into review context manifest |
| 326 | PAN-1839 | M | low | ok |  |  | Settings → Providers: show each provider's default harness in the collapsed row (no expand needed) |
| 327 | PAN-1837 | M | low | ok |  |  | Support Kimi Code as a first-class harness (Moonshot's own coding CLI) |
| 328 | PAN-1154 | M | low | ok |  |  | pan up does not kill existing port holders — startup races against orphan dashboard servers |
| 329 | PAN-1245 | M | low | ok |  |  | Flywheel gate gets stuck after orchestrator dies (reboot, crash, partial report) |
| 330 | PAN-2004 | M | low | ok |  |  | Resumable Planning node: double-click a planned issue's Planning to resume the planning agent |
| 331 | PAN-241 | L | low | ok |  |  | Mobile redesign initiative: full UX/UI overhaul + implementation plan |
| 332 | PAN-2035 | M | low | ok |  |  | ohmypi: GitHub Copilot subscription provider routing via omp |
| 333 | PAN-2032 | M | low | ok |  |  | ohmypi: local Ollama model as zero-cost preliminary review role |
| 334 | PAN-265 | M | low | ok |  |  | Review skill categorization: all skills available everywhere via personal + workspace |
| 335 | PAN-271 | M | low | ok |  |  | Auto-assign Linear project from project config when creating issues |
| 336 | PAN-1581 | M | low | ok |  |  | Duplicate skills in picker: code-review collides with official plugin; beads/pan-flywheel/pan-handoff doubled across project+user sync |
| 337 | PAN-1592 | M | low | ok |  |  | Composer: make ephemeral composer state reload-durable (pasted images + unsent/failed message text) |
| 338 | PAN-2266 | M | low | ok |  |  | feat: add zcode harness and make it the default for glm-5.2 |
| 339 | PAN-299 | M | low | ok |  |  | Granular session state persistence across context compaction |
| 340 | PAN-297 | M | low | ok |  |  | Workspace templates: pre/post tool hooks for auto-format, typecheck, lint |
| 341 | PAN-2288 | L | low | ok |  |  | tmux managed-server: lossless auto-migration of dirty-founded servers + boot-time ensure call (PAN-1798 follow-up) |
| 342 | PAN-450 | M | low | ok |  |  | Adopt remaining Effect patterns — Schema, Platform, Streams, Logging, Testing |
| 343 | PAN-452 | M | low | ok |  |  | Conversation input bar — mode/permissions/workspace selectors |
| 344 | PAN-466 | M | low | ok |  |  | Add QwenCoder CLI as a supported runtime alongside Claude Code and Codex |
| 345 | PAN-465 | M | low | ok |  |  | Add OpenRouter as a model provider |
| 346 | PAN-463 | M | low | ok |  |  | Add Qwen 3.6+ model support |
| 347 | PAN-531 | M | low | ok |  |  | PAN: Windows Electron support (WSL2 required) |
| 348 | PAN-546 | M | low | ok |  |  | Remove claude-code-router — all providers use direct env var injection |
| 349 | PAN-1668 | M | low | ok |  |  | bug(dashboard): right-click 'restart with <model>' carries model only, never harness — can't move a review off Kimi |
| 350 | PAN-606 | M | low | ok |  |  | Evaluate MCP Agent Mail for inter-agent communication and file reservations |
| 351 | PAN-1691 | M | low | ok |  |  | feat(flywheel): conflict-aware merge train + on-demand UAT candidate — stop the rebase-cascade that strands ready PRs |
| 352 | PAN-1696 | M | low | ok |  |  | Merge train becomes per-project — works without a Flywheel run, multi-project view |
| 353 | PAN-2548 | M | low | ok |  |  | chore(state): close the PAN-2541 legacy-fallback deprecation window — delete dual-path resolution once every project carries the D12 marker |
| 354 | PAN-629 | M | low | ok |  |  | Workspace quotas and resource governance |
| 355 | PAN-1740 | S | low | ok |  |  | Deacon mislabels SIGTERM workspace container restarts as crashes |
| 356 | PAN-1773 | M | low | ok |  |  | Swarm v2 Phase 2: remote slot agents on Fly (B5 follow-up to PAN-1762) |
| 357 | PAN-1750 | M | low | ok |  |  | feat(flywheel): UAT assembly/conflict agent — observability surfaces + configurable harness/model (default gpt-5.5 via Codex) |
| 358 | PAN-654 | M | low | ok |  |  | Project Setup Wizard — Dashboard UI |
| 359 | PAN-649 | M | low | ok |  |  | Render Excalidraw drawings inline in Claude Code conversations |
| 360 | PAN-687 | M | low | ok |  |  | Support OpenCode as alternative coding agent |
| 361 | PAN-2646 | M | low | ok |  |  | feat(swarm): configurable global/project/issue policy UI with default OFF |
| 362 | PAN-2685 | M | low | ok |  |  | Annotated live preview: Codex-style annotate-the-app feedback delivered to agents |
| 363 | PAN-1936 | M | low | ok |  |  | Single source-of-truth reads — one canonical resolver per domain (consolidate the 280+ scattered read endpoints) |
| 364 | PAN-1999 | M | low | ok |  |  | Backlog Sequencer: one sequencer per project (currently a single global runner scoped to PAN) |
| 365 | PAN-2008 | M | low | ok |  |  | feat(ci): store-access guard — fail the build on direct store reads outside a domain resolver (PAN-1936 slice) |
| 366 | PAN-818 | M | low | ok |  |  | Make summary optional when forking conversations |
| 367 | PAN-902 | M | low | ok |  |  | Settings: add 'Run pan sync' button to configuration menu |
| 368 | PAN-901 | M | low | ok |  |  | Settings: add Maintenance panel with Claude Code Organizer + Config Editor quick-launch |
| 369 | PAN-2197 | M | low | ok |  |  | bug(codex): work agents skip `pan done` (manual push instead) — sandbox blocks its GitHub calls; idle agents spuriously 'troubled' |
| 370 | PAN-2195 | M | low | ok |  |  | pan plan finalize re-plan churn: stale superseded spec on main transiently materializes the old plan |
| 371 | PAN-949 | M | low | ok |  |  | feat: add conversation for project from sidebar |
| 372 | PAN-2308 | M | low | ok |  |  | hardening(workspaces): migrate stale generated compose files off PORT=3011 + deacon quarantine for deterministic container boot refusals (fo |
| 373 | PAN-2347 | M | low | ok |  |  | docs: refresh AGENT-STATE-PLANES.md — update, harden, make useful |
| 374 | PAN-459 | M | low | ok |  |  | Planning setup screen with SSE progress streaming |
| 375 | PAN-2390 | M | low | ok |  |  | systemd-oomd killed overdeck-tmux-server.service (all 55 agent processes) under host memory pressure — set ManagedOOMPreference=avoid on the |
| 376 | PAN-2414 | M | low | ok |  |  | bug(cloister): context-overflow recovery is inconsistent — some agents get the PAN-1781 compact-respawn, others hit the PAN-1980 rotation re |
| 377 | PAN-2416 | M | low | ok |  |  | bug(cloister): codex agents can wedge on the Codex CLI first-run/consent screen — spawn must pre-accept non-interactively |
| 378 | PAN-2424 | L | low | ok |  |  | Epic: the Order Book — first-class operator priority queue (markdown-authored, backlog-exempt, load-governed, flywheel-integrated, remote ov |
| 379 | PAN-2442 | M | low | ok |  |  | feat(agents): Agent Client Protocol (ACP) as Overdeck's structured control plane — replace tmux keystrokes, transcript parsers, and prompt-d |
| 380 | PAN-571 | M | low | ok |  |  | Add OpenRouter credits/plan status endpoint and UI |
| 381 | PAN-570 | M | low | ok |  |  | Show PLAN badge on costs when under a subscription/plan |
| 382 | PAN-568 | S | low | ok |  |  | Kanban: Show workspace and tmux session counts in stats |
| 383 | PAN-2484 | M | low | ok |  |  | fix(uat-train): ready set misses merge-eligible issues without flywheel merge verbs — eligibility sweep added; verb-coverage prompt rule add |
| 384 | PAN-589 | M | low | ok |  |  | Review and update commands-skills.md with all available Panopticon skills |
| 385 | PAN-2506 | M | low | ok |  |  | flywheel-primary-root.test.ts fails on macOS: /var vs /private/var symlink not canonicalized |
| 386 | PAN-2514 | M | low | ok |  |  | Claude Code Traffic Inspector — intercept & inspect model API traffic in the dashboard |
| 387 | PAN-604 | M | low | ok |  |  | Hide planning agent from workspace detail pane |
| 388 | PAN-2549 | M | low | ok |  |  | Fly remote workspaces: sync overdeck-state before re-enabling migrated projects |
| 389 | PAN-622 | M | low | ok |  |  | YAML workflow DAGs: custom per-project pipeline definitions |
| 390 | PAN-658 | M | low | ok |  |  | Shared Sessions v0: GitHub-auth'd shared conversation panel with WebRTC transport |
| 391 | PAN-2625 | M | low | ok |  |  | feat(onboarding): auto-run /pan-new-project on project creation + setup banner, checklist, teaching empty states, and a guided demo issue |
| 392 | PAN-702 | M | low | ok |  |  | OpenAI provider: add plan/subscription support and fix unregistered model resolution |
| 393 | PAN-2651 | M | low | ok |  |  | fix(pipeline): simplify lifecycle reconciliation and add a safe post-planning reset |
| 394 | PAN-2668 | M | low | ok |  |  | Verification/review feedback silently queued to stopped-by-user agents — re-drive not applied on delivery |
| 395 | PAN-736 | M | low | ok |  |  | feat: wire per-subagent model overrides from settings to Claude Code spawn env |
| 396 | PAN-735 | M | low | ok |  |  | Settings page: review and configure overridden subagent model files |
| 397 | PAN-2813 | M | low | ok |  |  | Scheduler yield never self-clears: yielded work agents stay paused after the blocking review completes/merges |
| 398 | PAN-774 | M | low | ok |  |  | Unify launch UX and release pipeline for 1.0 — npx panctl, lazy prereqs, cross-platform desktop builds |
| 399 | PAN-772 | M | low | ok |  |  | Unify terminal stack behavior across tmux sessions |
| 400 | PAN-1223 | M | low | ok |  |  | Auto-update for users in the field (npm + desktop binaries) |
| 401 | PAN-786 | M | low | ok |  |  | Post planning Q\&A answers as issue comment |
| 402 | PAN-793 | M | low | ok |  |  | Borrow Deft's explicit scope-lifecycle transitions for Panopticon agent state machine |
| 403 | PAN-43 | M | low | ok |  |  | Add Slack and email notifications for agent events |
| 404 | PAN-55 | M | low | ok |  |  | Track specialist costs with time period filtering |
| 405 | PAN-54 | M | low | ok |  |  | feat: Add pan test:e2e command for full workflow integration test |
| 406 | PAN-1442 | M | low | ok |  |  | Follow-up to PAN-829: voice-sampler.html cleanup in pan-tts repo |
| 407 | PAN-1490 | M | low | ok |  |  | feat(dashboard): show each conversation's current git branch (port t3code BranchToolbar pattern) |
| 408 | PAN-106 | M | low | ok |  |  | Cost prediction/estimation for in-progress work |
| 409 | PAN-1524 | M | low | ok |  |  | Slash command aliases: /handoff → /pan-handoff (and similar short forms) |
| 410 | PAN-853 | M | low | ok |  |  | Evaluate terminal-bench@2.0 custom agent harnesses for Panopticon integration |
| 411 | PAN-908 | M | low | ok |  |  | PAN-908: Make work-agent spawn limits configurable and overridable |
| 412 | PAN-927 | L | low | ok |  |  | Rewrite containerize route: dead code, orphan processes, no pending-op tracking |
| 413 | PAN-943 | M | low | ok |  |  | Add memory file review and management command |
| 414 | PAN-944 | M | low | ok |  |  | Make vBRIEF the durable task graph source of truth |
| 415 | PAN-961 | M | low | ok |  |  | Update documentation for vBRIEF v0.6 lifecycle model |
| 416 | PAN-1623 | M | low | ok |  |  | Codex: surface interactive approval prompts as conversation Q&A (like AskUserQuestion) |
| 417 | PAN-1654 | M | low | ok |  |  | perf(build): run lint:skills from source via tsx, skip CLI dist build (salvaged from PAN-1615 workspace) |
| 418 | PAN-1653 | M | low | ok |  |  | perf(docs-rag): batch local embedding in buildDocsIndex (salvaged from PAN-1617 workspace) |
| 419 | PAN-146 | M | low | ok |  |  | PAN-146: Refine light mode theming across all dashboard pages |
| 420 | PAN-155 | L | low | ok |  |  | PAN-155: Redesign health page with Stitch (system overview, timeline, costs) |
| 421 | PAN-1685 | M | low | ok |  |  | Show model capability icons in conversation dialogs + complete per-model vision (supportsImages) audit |
| 422 | PAN-1066 | M | low | ok |  |  | Complete PAN-1048 R5: retire dispatchParallelReview body and specialists.ts module |
| 423 | PAN-180 | M | low | ok |  |  | PAN-180: Cross-terminal file locking for concurrent agents |
| 424 | PAN-177 | M | low | ok |  |  | PAN-177: Iteration limits with escalation for autonomous agents |
| 425 | PAN-198 | M | low | ok |  |  | Structured audit trail for agent actions |
| 426 | PAN-1776 | M | low | ok |  |  | Hot-updatable message delivery: version-stamped supervisors + server-side delivery logic |
| 427 | PAN-1124 | M | low | ok |  |  | Decouple specs and PRDs from workspaces — write directly to main |
| 428 | PAN-1844 | M | low | ok |  |  | Deep-linkable Command Deck: reflect selected issue/agent in the browser URL + make activity notifications link to the specific view |
| 429 | PAN-1854 | M | low | ok |  |  | Define handoff strategy for large conversations: external vs source authoring + tail-biased read |
| 430 | PAN-1852 | M | low | ok |  |  | Capability-tiered work-agent model selection: difficulty→capability-floor routing from benchmark-anchored eval data |
| 431 | PAN-1853 | M | low | ok |  |  | Surface a transcript-size warning on growing conversations (2 MB warn / 10 MB strong-nudge tiers) |
| 432 | PAN-1133 | M | low | ok |  |  | TLDR: deacon supervision + pan doctor check + GC |
| 433 | PAN-1135 | M | low | ok |  |  | Document the hook system in docs/HOOKS.md |
| 434 | PAN-1916 | M | low | ok |  |  | feat(search): configurable web search providers (Exa, Tavily, Brave, Perplexity) |
| 435 | PAN-1968 | S | low | ok |  |  | Finish local-domain rename: pan.localhost → overdeck.localhost |
| 436 | PAN-1244 | M | low | ok |  |  | pan admin cloister start: CLI crashes with SIGSEGV (exit code 139) after handing off to server |
| 437 | PAN-1991 | L | low | ok |  |  | Issue cockpit redesign — incremental rollout (tracking) |
| 438 | PAN-1325 | M | low | ok |  |  | Artifact storage model is unsafe for polyrepo projects — define a canonical "orchestration repo" |
| 439 | PAN-1356 | M | low | ok |  |  | Extend the memory Observation pipeline to ad-hoc conversations |
| 440 | PAN-227 | M | low | ok |  |  | Phase gate validation — mid-implementation acceptance checks |
| 441 | PAN-228 | M | low | ok |  |  | Shift-left post-edit diagnostics — type check after every edit |
| 442 | PAN-249 | M | low | ok |  |  | Add data-testid attributes across dashboard UI and create Playwright smoke test suite |
| 443 | PAN-2034 | M | low | ok |  |  | ohmypi: end-to-end test that tool-call steps render in Conversation panel |
| 444 | PAN-2033 | M | low | ok |  |  | ohmypi: benchmark FIFO vs paste-buffer message delivery latency |
| 445 | PAN-2031 | M | low | ok |  |  | ohmypi: add Bun 1.3.11 regression test to checkOhmypi doctor gate |
| 446 | PAN-2030 | M | low | ok |  |  | ohmypi: version-pin extension in package.json and pan doctor mismatch warning |
| 447 | PAN-2029 | S | low | ok |  |  | ohmypi: capture kimi thinking_tokens in ohmypi-parser for complete cost accounting |
| 448 | PAN-2028 | M | low | ok |  |  | ohmypi: per-provider cost grouping in cost dashboard |
| 449 | PAN-2026 | M | low | ok |  |  | ohmypi: surface 35+ provider matrix in dashboard model picker |
| 450 | PAN-2025 | M | low | ok |  |  | ohmypi: extend provider credential passthrough for Groq, Cerebras, Fireworks |
| 451 | PAN-2024 | M | low | ok |  |  | ohmypi: frontend Tools-toggle for conversation view |
| 452 | PAN-1480 | M | low | ok |  |  | TLDR: 93% bypass rate — daemon/hook integration broken |
| 453 | PAN-1533 | M | low | ok |  |  | Fork-into-worktree from conversation branch chip |
| 454 | PAN-283 | M | low | ok |  |  | Reset should sync workspace feature branch with latest main |
| 455 | PAN-298 | M | low | ok |  |  | Auto-detect package manager and runtime in workspace setup |
| 456 | PAN-2295 | M | low | needs-refinement |  |  | feat(overdeck): built-in web browser surface (openable like terminal/Claude Code/Codex) + native Agentation integration |
| 457 | PAN-1643 | M | low | ok |  |  | Extend local Ollama support to Codex + Claude Code harnesses and dashboard model picker |
| 458 | PAN-1646 | M | low | ok |  |  | Rabbit-hole drift detection and lift-to-new-conversation |
| 459 | PAN-1640 | M | low | ok |  |  | Re-platform interactive permission allow/deny onto a PreToolUse hook (provider-agnostic) |
| 460 | PAN-1669 | M | low | ok |  |  | bug(dashboard): restart-with-model doesn't emit a live event — issue tree shows stale model until manual refresh |
| 461 | PAN-1683 | M | low | ok |  |  | docs: canonical agent session-prefix registry + reconcile role taxonomy (ROLES.md/AGENT_TYPES_INDEX/CLAUDE.md) — strike keeps falling out of |
| 462 | PAN-1710 | M | low | ok |  |  | bug(ci): 'Clean install + server smoke test' hangs (3 consecutive 20-min timeout kills) on feature/pan-1491 and feature/pan-1641 — server bo |
| 463 | PAN-1728 | M | low | ok |  |  | bug(work): PAN-1700 agent committed .pan/specs/*.vbrief.json mutations — PAN-1124 immutability violated on feature branch |
| 464 | PAN-2557 | M | low | ok |  |  | feat(dashboard): project-level 'Restart All' context action — restart every agent in a project, throttled by the PAN-2500 memory governor |
| 465 | PAN-2556 | M | low | ok |  |  | feat(dashboard): add a per-issue 'Restart agent' action (stop+start active role) — the restartAgent type exists but isn't wired into the iss |
| 466 | PAN-2553 | M | low | ok |  |  | feat(dashboard): project-level CI visibility — surface repo/main-branch workflow runs on the Command Deck with click-through to logs |
| 467 | PAN-2566 | L | low | ok |  |  | Traycer parity epic: gap analysis of capabilities Overdeck lacks |
| 468 | PAN-2565 | M | low | ok |  |  | Multi-agent conversations: N agent sessions in one task surface with agent-to-agent messaging |
| 469 | PAN-1758 | M | low | ok |  |  | Watch: ready-for-merge work must converge despite a continuously moving main |
| 470 | PAN-1774 | M | low | ok |  |  | bug(uat): workspace server container crashloops when dist/dashboard/server.js is missing |
| 471 | PAN-1755 | M | low | ok |  |  | bug(cloister): uat stuck-assembly cap (30m) kills slow-but-alive assemblies and leaves orphaned conflict agents racing the next generation |
| 472 | PAN-2609 | M | low | ok |  |  | Cross-device sync of conversations and tasks via user-owned git remote |
| 473 | PAN-2608 | M | low | ok |  |  | Persistent collaboration roles (owner/editor/viewer) and organizations — gated behind the shared-instance milestone |
...
| 475 | PAN-1878 | M | low | ok |  |  | process: bake 'docs updated' into acceptance criteria / definition-of-done in role + planning prompts |
| 476 | PAN-1895 | M | low | ok |  |  | Spawn work agents from issue workspace slide-out |
| 477 | PAN-1907 | M | low | ok |  |  | Generalize ToS gate: block ALL non-Claude-Code harnesses from Anthropic-subscription models; gray out + non-selectable + validate everywhere |
| 478 | PAN-1906 | M | low | ok |  |  | Enforce harness restrictions with subscription: gray out non-claude-code, validate everywhere |
| 479 | PAN-1910 | S | low | ok |  |  | fast-follow(PAN-1908): collapse issue status to ONE canonical field — labels become a derived projection, not the source of truth |
| 480 | PAN-2718 | S | low | ok |  |  | pan restart needs a first-class no-dialog reconciliation flag — autonomous restarts must not park a dialog on the operator |
| 481 | PAN-1918 | M | low | ok |  |  | bug(ci): full frontend vitest suite runs in no CI path — npm test limited to 3 files; IssueMissionControl.test.tsx open-handle hang stalls t |
| 482 | PAN-1926 | M | low | ok |  |  | feat(strike): --big flag to lift strike's precision-only scope guard (operator-authorized larger strikes) |
| 483 | PAN-1949 | M | low | ok |  |  | Surface inspection sub-runs in the issue tree + a parent Inspection node aggregating all item verdicts |
| 484 | PAN-2890 | S | low | ok |  |  | Stopped/running agent surfaces: no drill-down, no context, inconsistent counts |
| 485 | PAN-1980 | M | low | ok |  |  | Stop session rotation on resume (behind a constant); one pipeline-membership view from all lenses |
| 486 | PAN-1988 | M | low | ok |  |  | Verdict signaling: one host-owned write door; agents journal, host owns the DB cache |
| 487 | PAN-2006 | M | low | ok |  |  | Pipeline semantics lock-down: Definition of Ready, pickup gates (parked/vetoed/blocks-main), unblock override, and Run definition |
| 488 | PAN-2085 | M | low | ok |  |  | Auto-isolate conversations in a lightweight git worktree (Conductor-style workspaces) |
| 489 | PAN-2084 | M | low | ok |  |  | Auto-create lightweight conversation worktrees on project chats |
| 490 | PAN-2083 | M | low | ok |  |  | Composer: a failed first send leaves the text in BOTH the composer box and the retry outbox |
| 491 | PAN-2082 | M | low | ok |  |  | Composer: a single send failure clears ALL in-flight optimistic bubbles (and strips siblings' compaction net) |
| 492 | PAN-2071 | M | low | ok |  |  | docs: add user-facing page for the Hooks system |
| 493 | PAN-2070 | M | low | ok |  |  | docs: add user-facing page for the Flywheel orchestrator |
| 494 | PAN-407 | M | low | ok |  |  | Run Panopticon from a main workspace for development isolation |
| 495 | PAN-2282 | M | low | ok |  |  | Conversation view shows no history for ohmypi-harness conversations — pi transcript surface missing (conv 353) |
| 496 | PAN-2280 | M | low | ok |  |  | Resumed conversations wedge without writing transcripts when dashboard is black-holed — views diverge from terminals (conv 367 et al.) |
| 497 | PAN-438 | M | low | ok |  |  | Migrate remaining REST polling endpoints to Effect RPC |
| 498 | PAN-2356 | M | low | ok |  |  | Overdeck Anywhere P3: relay service — outbound-only daemon, GitHub OAuth, push origin, multi-tenant front door |
| 499 | PAN-2355 | M | low | ok |  |  | Overdeck Anywhere P2: mobile PWA (Needs-You feed, conversation view, pipeline board, Web Push) |
| 500 | PAN-2348 | M | low | ok |  |  | docs: migrate STATE-STORAGE-AUDIT.md content to living docs, then delete |
| 501 | PAN-461 | M | low | ok |  |  | Deep-wipe multi-step progress dialog |
| 502 | PAN-471 | M | low | ok |  |  | Cost reconciler: auto-trigger on agent lifecycle events with debounce |
| 503 | PAN-476 | M | low | ok |  |  | Agent resume with Haiku session summary instead of claude --resume |
| 504 | PAN-468 | M | low | ok |  |  | Agent test conversations pollute production database — need test isolation |
| 505 | PAN-483 | M | low | ok |  |  | Unify Resume Agent UX — all entry points should show message input |
| 506 | PAN-2423 | M | low | ok |  |  | bug(workspace): pan workspace rebuild hardcodes 'overdeck-' compose project prefix — mismatches project templates and verification container |
| 507 | PAN-2408 | M | low | ok |  |  | bug(cli): pan start --auto commits the spec to main AFTER creating the worktree — agent's own workspace lacks its spec, causing wrong-worksp |
| 508 | PAN-2443 | M | low | ok |  |  | feat(costs): OpenTelemetry GenAI semconv — OTLP ingestion layer for cross-harness telemetry (tokens/latency/tools), pinned-snapshot adoption |
| 509 | PAN-543 | M | low | ok |  |  | Add confirmation dialog before applying Optimal Defaults |
| 510 | PAN-554 | M | low | ok |  |  | Add kanban board deeplinks for issue URLs |
| 511 | PAN-565 | M | low | ok |  |  | Handle CTRL-Z to undo accidental conversation archival |
| 512 | PAN-564 | M | low | ok |  |  | Slash menu positioned incorrectly — cut off / off-screen |
| 513 | PAN-576 | M | low | ok |  |  | Global / search should include conversations in addition to workspace features |
| 514 | PAN-2487 | M | low | ok |  |  | feat(ship): CI-green merge skip + Ship & Merge cockpit view (live door log + progress) + active-node spinner |
| 515 | PAN-591 | M | low | ok |  |  | Integrate Karpathy LLM guidelines into all Panopticon CLAUDE.md templates |
| 516 | PAN-2505 | M | low | ok |  |  | lint:circular reports new frontend cycles + stale baseline in chat/conversations components |
| 517 | PAN-2527 | M | low | ok |  |  | Harness selector should restrict OpenAI models to Claude Code only |
| 518 | PAN-2526 | L | low | ok |  |  | Refactor deacon.ts below file-size baseline |
| 519 | PAN-624 | M | low | ok |  |  | Loop nodes: iterative agent execution with conditional termination |
| 520 | PAN-2560 | M | low | ok |  |  | resolveStateReadHomeSync (state-read-home.ts) resolves state dir by path basename, not registry key — migrated projects silently fall back t |
| 521 | PAN-674 | M | low | ok |  |  | docs: add glossary of Panopticon domain terms |
| 522 | PAN-663 | M | low | ok |  |  | Workspace frontend containers not auto-started for panopticon-cli self-hosted workspaces |
| 523 | PAN-2622 | M | low | ok |  |  | cloister.toml materializes ALL defaults into the user file — default changes in code never reach existing installs |
| 524 | PAN-2630 | M | low | ok |  |  | pan binary not on PATH for operator shells or spawned work agents; pan doctor can't be run to diagnose it |
| 525 | PAN-2629 | M | low | ok |  |  | pan start kickoff delivery never lands: "Claude Code did not become ready within 30s" (both attempts), agent sits idle at empty prompt |
| 526 | PAN-2628 | M | low | ok |  |  | pan close aborts at close-issue:transition: "No tracker available and cannot determine issue type" for GitHub-tracker project |
| 527 | PAN-2626 | M | low | ok |  |  | feat(conversations): allow composer model switching within the same model family (e.g. Sonnet → Fable) |
| 528 | PAN-701 | M | low | ok |  |  | Quick-Create conversation via keystroke using Conversations-page default model |
| 529 | PAN-2662 | M | low | ok |  |  | Add project context-menu actions scoped to issues currently in the pipeline |
| 530 | PAN-2660 | M | low | ok |  |  | Add safe Reset to planned action to the issue actions menu |
| 531 | PAN-730 | M | low | ok |  |  | Add provider account telemetry for credits, balances, and usage |
| 532 | PAN-2680 | M | low | ok |  |  | pan close: Docker teardown silently skips a running stack in multi-repo projects (MYN), aborting close-out |
| 533 | PAN-2679 | M | low | ok |  |  | conv-lookup skill: resolve transcripts for codex and pi harness conversations |
| 534 | PAN-743 | M | low | ok |  |  | Add consistent new conversation icon actions in Command Deck |
| 535 | PAN-738 | M | low | ok |  |  | Add right-click fork option to conversation list |
| 536 | PAN-2754 | M | low | ok |  |  | bug(swarm): `always` is inert — it behaves exactly like `auto`, contradicting the documented spec |
| 537 | PAN-2755 | M | low | ok |  |  | bug(review): per-issue review-model override never reached convoy sub-reviewers on the discovery-fork path |
| 538 | PAN-747 | S | low | ok |  |  | Conversation list items lack accessible labels in accessibility tree |
| 539 | PAN-749 | M | low | ok |  |  | Research and borrow best features from gstack |
| 540 | PAN-750 | L | low | ok |  |  | PAN-XXX: Complete Metrics Page Redesign — Real Data, Charts, Time Filtering, and TLDR Analytics |
| 541 | PAN-751 | M | low | ok |  |  | PAN-XXX: Historical Metrics Data Persistence — Beyond the 30-Day JSONL Window |
| 542 | PAN-2810 | M | low | ok |  |  | Workspace 'vitest --changed' gate diverges from CI: App.test.tsx fails locally on missing selectPendingInputSubjects mock |
| 543 | PAN-2809 | M | low | ok |  |  | Live-terminal Playwright UAT blocked in containerized workspaces (node-pty musl/glibc mismatch + Vite/Traefik WS Origin 403) |
| 544 | PAN-2806 | M | low | ok |  |  | bug(cloister): strike merge trigger registry splits across dashboard chunks |
| 545 | PAN-2796 | M | low | ok |  |  | fix(cloister): idle nudge must not advance after failed mandatory inspection |
| 546 | PAN-752 | M | low | ok |  |  | Add Gemini OAuth support, remove O3/O4-mini, disable GPT-5.4-Pro |
| 547 | PAN-762 | M | low | ok |  |  | Settings: warn when model overrides target disabled providers |
| 548 | PAN-765 | M | low | ok |  |  | Preserve trailing zeros in cost displays |
| 549 | PAN-771 | M | low | ok |  |  | Investigate Vercel Sandbox execution backend support |
| 550 | PAN-773 | M | low | ok |  |  | Design prompt-style overlays with model hierarchy and scoped toggles |
| 551 | PAN-777 | M | low | ok |  |  | Inter-agent communication skill: send messages to conversation-mode agents |
| 552 | PAN-790 | L | low | ok |  |  | PAN-789: Eliminate remaining TanStack Query polling — complete push-first migration |
| 553 | PAN-791 | M | low | ok |  |  | Skill mapping: Deft Directive v0.20.0-rc.3 ↔ Panopticon CLI |
| 554 | PAN-797 | M | low | ok |  |  | Cost display: cache write tokens not shown separately; investigate Claude Code discrepancy |
| 555 | PAN-898 | M | low | ok |  |  | Dashboard polling and WebSocket efficiency: remaining audit findings |
| 556 | PAN-984 | M | low | ok |  |  | Evaluate context-mode MCP server as session continuity + search layer |
| 557 | PAN-1051 | M | low | ok |  |  | feat: Subspace-inspired alternate theme with Inter + JetBrains Mono |
| 558 | PAN-1049 | M | low | ok |  |  | Spike: evaluate Tauri v2 desktop shell |
| 559 | PAN-1065 | M | low | ok |  |  | Validate issueId at every shell-string interpolation site (defense in depth) |
| 560 | PAN-1064 | M | low | ok |  |  | Harden launcher generation against shell-quote injection (model and arg quoting) |
| 561 | PAN-1063 | M | low | ok |  |  | Harden tts_daemon.py: bearer auth, CORS, body size cap, concurrency bound |
| 562 | PAN-1117 | M | low | ok |  |  | Memory: pinned docs (long-form doc chunking + retrieval) |
| 563 | PAN-1116 | M | low | ok |  |  | Memory: cross-project search mode |
| 564 | PAN-1121 | M | low | ok |  |  | Context bloat: agents receive oversized prompts that exceed tool limits and force immediate compaction |
| 565 | PAN-1123 | M | low | ok |  |  | Channels delivery: surface failures, add fallback toggle, route conversations through channels |
| 566 | PAN-1153 | M | low | ok |  |  | Vite TRAEFIK_ENABLED conflates 'Traefik on' with 'inside container' — breaks pan dev proxy |
| 567 | PAN-1152 | M | low | ok |  |  | Remove PANOPTICON_DEV env-var persistence — derive Traefik mode from the running command |
| 568 | PAN-1166 | M | low | ok |  |  | Re-introduce /ws/terminal auth gate with a working bootstrap path |
| 569 | PAN-1208 | M | low | ok |  |  | Polyrepo: support non-feature 'main' workspaces alongside feature-* |
| 570 | PAN-1222 | M | low | ok |  |  | Project-templated DB lifecycle: auxiliary databases + seed refresh from prod |
| 571 | PAN-1242 | M | low | ok |  |  | Create a new issue directly from a kanban column |
| 572 | PAN-1481 | M | low | ok |  |  | Add cost-event telemetry for Caveman token savings |
| 573 | PAN-1483 | M | low | ok |  |  | Distinguish general-use skills from Panopticon-only dev skills in pan sync |
| 574 | PAN-1482 | M | low | ok |  |  | Token spend report should aggregate data from repo, not just local machine |
| 575 | PAN-1479 | M | low | ok |  |  | RTK: Add telemetry to measure token savings from bash output compression |
| 576 | PAN-1474 | M | low | ok |  |  | Add ACKNOWLEDGEMENTS doc — credit borrowed code from open-source projects (MIT/Apache 2.0) |
| 577 | PAN-1553 | M | low | ok |  |  | Investigate Claude Code Fast mode support (and fast-tier pricing) |
| 578 | PAN-1552 | M | low | ok |  |  | Dashboard conversation-message 500 cause is unloggable: serve mode never writes dashboard.log |
| 579 | PAN-1550 | M | low | ok |  |  | feat: FilesPane + BrowserPane — file browser and embedded web view implementation details |
| 580 | PAN-1572 | M | low | ok |  |  | Settings permission-mode can desync from resolved config — agents silently use --dangerously-skip-permissions despite 'Auto' |
| 581 | PAN-1641 | M | low | ok |  |  | Run agents on local GPU models via a managed Ollama sidecar |
| 582 | PAN-1667 | M | low | ok |  |  | feat(dashboard): unify Agents + Resources into one issue-centric holistic view |
| 583 | PAN-1720 | M | low | ok |  |  | bug(test): cloister auto-resume tests fail under full parallel run, pass in isolation — test pollution reddening main |
| 584 | PAN-1761 | M | low | ok |  |  | bug(dashboard): conversations endpoints fetched via relative /api path — 403 inside workspace/UAT containers (session cookie is on the api-* |
| 585 | PAN-1754 | M | low | ok |  |  | feat(settings): surface + edit the host claude CLI default model (~/.claude/settings.json) from the Settings page |
| 586 | PAN-1748 | M | low | ok |  |  | feat(cloister): reuse uat-assembly conflict resolutions across generations (rerere or resolution replay) |
| 587 | PAN-1775 | M | low | ok |  |  | Remote (Fly.io) work agents appear as real session rows in the issue tree |
| 588 | PAN-1782 | M | low | ok |  |  | Handoff forks stall at "Injecting…" then die on double 300s summary timeout — decouple precompaction from the handoff author model |
| 589 | PAN-1914 | M | low | ok |  |  | Follow-up: move /api/health/agents off agent-directory scans |
| 590 | PAN-1937 | M | low | ok |  |  | feat: data export — portable bundle (conversations + favorites core; decoupled optional cost ledger) + user-facing Export my data |
| 591 | PAN-1958 | M | low | ok |  |  | Source-tagged programmatic delivery into pi conversation agents (extension sendUserMessage + input.source) |
| 592 | PAN-1983 | L | low | ok |  |  | Remove all panopticon.db-supporting code (legacy SQLite layer + db↔db migration + seed-from-legacy) |
| 593 | PAN-1990 | M | low | ok |  |  | First-class workspaces and projects with per-workspace memory |
| 594 | PAN-1984 | M | low | ok |  |  | Migrate or delete the 18 dead panopticon.db modules referenced by ~30 test files (#1983 follow-up) |
| 595 | PAN-2002 | M | low | ok |  |  | [HUMAN-ONLY] Sign & notarize the macOS desktop build (Apple Developer ID) |
| 596 | PAN-2046 | M | low | ok |  |  | Conversation view does not surface terminal command responses |
| 597 | PAN-2066 | M | low | ok |  |  | OKF v2 — knowledge viewer: inkeep open-knowledge coinstall (progressive), /okf open, dashboard Knowledge page |
| 598 | PAN-2091 | M | low | ok |  |  | chore(dashboard): delete dead IssueCockpitBody cockpit subtree (8 files, superseded by IssueMissionControl) |
| 599 | PAN-2074 | M | low | ok |  |  | research: evaluate ponytail (DietrichGebert/ponytail) for prompt compression and consider building in-house |
| 600 | PAN-2073 | M | low | ok |  |  | docs: add user-facing page for the Desktop App |
| 601 | PAN-2068 | M | low | ok |  |  | docs: add user-facing page for Caveman (agent output compression) |
| 602 | PAN-2067 | M | low | ok |  |  | docs: add user-facing page for RTK (Bash output compression) |
| 603 | PAN-2287 | M | low | ok |  |  | bug(supervisor): every supervisor.log line written twice — log() appendFile + launcher stdout redirect target the same file |
| 604 | PAN-2352 | M | low | ok |  |  | Overdeck Anywhere P1a: remote dashboard access via Cloudflare Tunnel + Access |
| 605 | PAN-2354 | M | low | ok |  |  | Overdeck Anywhere P1c: needs-you push notification bridge (ntfy first, Web Push later) |
| 606 | PAN-2353 | M | low | ok |  |  | Overdeck Anywhere P1b: Hermes external-agent bridge (scoped API + Fly 6PN) |
| 607 | PAN-2351 | M | low | ok |  |  | Overdeck Anywhere P0: scoped access tokens + WS/SSE heartbeats (security prerequisites) |
| 608 | PAN-2350 | L | low | ok |  |  | Epic: Overdeck Anywhere — remote access, Hermes bridge, mobile, and the shared relay backbone |
| 609 | PAN-2346 | M | low | ok |  |  | docs: refresh AGENT_TYPES_INDEX.md — update, harden, make useful |
| 610 | PAN-2345 | M | low | ok |  |  | docs: refresh pan-done.md — update, harden, make useful |
| 611 | PAN-2344 | M | low | ok |  |  | docs: refresh KANBAN-MODEL.md — update, harden, make useful |
| 612 | PAN-2343 | M | low | ok |  |  | docs: refresh MISSION-CONTROL.md — update, harden, make useful |
| 613 | PAN-2381 | M | low | ok |  |  | bug(dashboard): three event types missing from DomainEvent schema union poison the RPC stream — permanent "Reconnecting…" loop |
| 614 | PAN-2394 | M | low | ok |  |  | Incident: conv-* agent-dir cleanup destroyed ohmypi/codex conversation transcripts ("no saved history") |
| 615 | PAN-2392 | M | low | ok |  |  | feat(dashboard): Standing Crew cost panel — per-member roster with cost, tokens, verdicts, escalations (mockup included) |
| 616 | PAN-2395 | M | low | ok |  |  | bug(config): one invalid tiered_execution enum poisons every config read — live conversations falsely marked ended, resume/new-conversation  |
| 617 | PAN-2428 | S | low | ok |  |  | bug(workspace): MYN workspace Traefik routing broken post-rebrand — legacy 'panopticon' network + missing traefik.docker.network label make  |
| 618 | PAN-2422 | M | low | ok |  |  | bug(infra): rebuilding dist under a live server breaks lazy chunk imports — 'Cannot find module dist/dashboard/<chunk>.js' |
| 619 | PAN-2409 | M | low | ok |  |  | feat(cloister): enforce the workspace boundary — work agents must not edit the primary checkout (PAN-2204 class, reproduced 3x on 2026-07-06 |
| 620 | PAN-2444 | M | low | ok |  |  | feat(agents): optional SageOx re-integration — session-reasoning capture for OSS projects (per-project opt-in, v0.11-era ox) |
| 621 | PAN-2454 | M | low | ok |  |  | bug(infra): ratchet audit fails per-commit on push ranges whose NET baseline delta is zero — strands finished branches |
| 622 | PAN-2465 | M | low | ok |  |  | bug(done): pan done's PR lookup fails at MYN polyrepo root — 'no git remotes found' makes completion exit nonzero |
| 623 | PAN-2492 | M | low | ok |  |  | bug(needs-you): pane-detected waits (rate-limit/session-resume) surface as 'needs you' but cannot be answered from the dashboard — only the  |
| 624 | PAN-2493 | M | low | ok |  |  | feat(parity): align the cockpit Agents-lane and sidebar issue-tree feature sets (two-way gaps) |
| 625 | PAN-2491 | M | low | ok |  |  | Migrate @xenova/transformers to @huggingface/transformers to eliminate silent npx install failures from sharp 0.32 postinstall |
| 626 | PAN-2489 | M | low | ok |  |  | bug(tree): strike agents are invisible in the project issue tree — needs-you pings with no node to click |
| 627 | PAN-2501 | M | low | ok |  |  | bug(dashboard): deleteResourceVenvEffect's HttpRouter.schemaParams call fails typecheck under the root tsconfig (masked by src/dashboard/**  |
| 628 | PAN-2504 | M | low | ok |  |  | Auto-relaunch npx @overdeck/core under a compatible Node 22+ instead of failing on old Node |
| 629 | PAN-2533 | M | low | ok |  |  | UAT workspace magic-link login 502: Traefik picks unreachable panopticon IP for multi-homed fe/api |
| 630 | PAN-2572 | M | low | ok |  |  | Noisy EBADENGINE + deprecation warnings on npx/npm install make a healthy install look broken |
| 631 | PAN-2600 | M | low | ok |  |  | Retire the Codex TUI path after app-server burn-in (no-loss audit gate) — follow-up to PAN-2597 |
| 632 | PAN-2635 | M | low | ok |  |  | chore(server): pay down the 152-error src/dashboard/server typecheck debt |
| 633 | PAN-2652 | M | low | ok |  |  | Conversation view diverges from Terminal: Claude Code backgrounding forks the session file in-process, invisible to all session-id resolutio |
| 634 | PAN-2645 | M | low | ok |  |  | Add opt-in Observation-first conversation view |
| 635 | PAN-2667 | M | low | ok |  |  | Reimplement the task-progress admission signal in resource discovery (PAN-2648 follow-up) |
| 636 | PAN-2767 | M | low | ok |  |  | Expose Codex app-server conversation controls in the dashboard |
| 637 | PAN-2836 | M | low | ok |  |  | okf: in-repo placement presets (okf/, docs/okf/) and /okf migrate to switch placements later |
...
The headline 'troubled: 14-16' / 'failed verification: N' counts are chronically inflated by residue on CLOSED issues: orphaned `inspect-<id>-workspace-<hash>` sub-agents that `pan untroubled` can't reach, and `review_status.stuck=1` rows that close-out only clears for issues IT closes. Operators repeatedly react to a scary number that represents nothing actionable. This corrupts the dashboard read model — the one surface the operator trusts to decide where to look.
...
`resolvePipelineMembership` concludes 'branch already in main' whenever no unmerged convention branch exists — but that's indistinguishable from 'branch never existed'. So planned-but-never-branched open issues (PAN-1525, 2802, 2838, 2859, 2876) sit as permanent false-limbo rows cluttering the rail, and `pan close` on them correctly fails (nothing merged) so they can't drain. Fix requires POSITIVE evidence of a merge.
...
blocks-main. The flywheel's own doctrine directs agents to trust `state.json lastActivity` for liveness — but for codex work agents that field freezes at spawn and costSoFar stays 0 even while the agent is committing every 30s. On PAN-2710 the operator came within one command of `pan kill`-ing a healthy, productive agent because the recommended signal said 'ghost'. When the doctrine-recommended signal actively misleads toward killing good work, that's a main-blocker.
...
Part of PAN-2075, fast-follow after the inbox spine. Once the inbox exists, reach the operator out-of-band when they aren't looking at the dashboard — a held fleet or runaway remote agent must be able to ping via email/Slack/push/TTS. Absorbs PAN-43.
...
K3 shipped 2026-07-16 (2.8T MoE, 1M context) and is in high demand. Kimi's coding endpoint already serves it under `k3`/`k3[1m]`; the claude-code Kimi provider (`getKimiAnthropicBaseUrl`) already selects that endpoint, so this is config + model-entry parity with how `kimi-k2.7-code` is wired. Operator-marked strike.
...
The flywheel picks by P0/P1/P2 and filters by author/labels but never checks whether an issue is blocked by another open issue — 'Depends-on' declarations are decorative. Without dependency awareness it starts downstream work too early (agents write promise-based code that immediately needs re-migration) and picks blocked work over ready work.

### PAN-2027 (rank 80)

kimi-k2 via CLIProxy advertises a false ~200k context window; long sessions deadlock silently (PAN-1865). omp has native DashScope auth, so kimi-k2 can run natively via ohmypi — eliminating the deadlock class. Update `resolveHarness` defaults so kimi models default to ohmypi instead of pi/CLIProxy.

### PAN-1578 (rank 81)

Copilot CLI defaults to Claude Sonnet 4.5 — a Copilot-subscription user could run a first-party GitHub agent loop on Claude without an Anthropic subscription or CLIProxy. Native AGENTS.md + SKILL.md support (reads `~/.copilot/skills/` and the open `~/.agents/skills/` tree Overdeck already writes). Harness expansion; pending gating-risk evaluation.
...
```json
{
  "version": 1,
  "project": "overdeck",
  "generatedAt": "2026-07-19T00:05:48Z",
  "model": "glm-5.2",
  "pass": "incremental",
  "openCount": 637,
  "nodes": [
    ...
    },
    {
      ...
      "condition": "ok",
      "dependsOn": [],
      "why": "Close-out leaves stale residue inflating troubled/failed metrics: orphaned inspect sub-agents + uncleared review_status rows.",
      "rationale": "The headline 'troubled: 14-16' / 'failed verification: N' counts are chronically inflated by residue on CLOSED issues: orphaned `inspect-<id>-workspace-<hash>` sub-agents that `pan untroubled` can't reach, and `review_status.stuck=1` rows that close-out only clears for issues IT closes. Operators repeatedly react to a scary number that represents nothing actionable. This corrupts the dashboard read model — the one surface the operator trusts to decide where to look.",
      "gate": "auto",
      "planning": "auto"
    },
    {
    ...
    },
    {
      ...
      "condition": "ok",
      "dependsOn": [],
      "why": "Pipeline membership infers 'non-PR landing' from absence of unmerged branch — false post_merge_limbo.",
      "rationale": "`resolvePipelineMembership` concludes 'branch already in main' whenever no unmerged convention branch exists — but that's indistinguishable from 'branch never existed'. So planned-but-never-branched open issues (PAN-1525, 2802, 2838, 2859, 2876) sit as permanent false-limbo rows cluttering the rail, and `pan close` on them correctly fails (nothing merged) so they can't drain. Fix requires POSITIVE evidence of a merge.",
      "gate": "auto",
      "planning": "auto"
    },
    {
    ...
    },
    {
      ...
      "condition": "ok",
      "dependsOn": [],
      "why": "state.json reports costSoFar=0 + frozen lastActivity for actively-committing codex agents → doctrine misclassifies them as ghosts and routes",
      "rationale": "blocks-main. The flywheel's own doctrine directs agents to trust `state.json lastActivity` for liveness — but for codex work agents that field freezes at spawn and costSoFar stays 0 even while the agent is committing every 30s. On PAN-2710 the operator came within one command of `pan kill`-ing a healthy, productive agent because the recommended signal said 'ghost'. When the doctrine-recommended signal actively misleads toward killing good work, that's a main-blocker.",
      "gate": "auto",
      "planning": "auto"
    },
    {
    ...
    },
    {
      ...
      "condition": "ok",
      "dependsOn": [],
      "why": "Add Kimi K3 (k3 / k3[1m]) to claude-code Kimi provider — parity with kimi-k2.7-code. Strike.",
      "rationale": "K3 shipped 2026-07-16 (2.8T MoE, 1M context) and is in high demand. Kimi's coding endpoint already serves it under `k3`/`k3[1m]`; the claude-code Kimi provider (`getKimiAnthropicBaseUrl`) already selects that endpoint, so this is config + model-entry parity with how `kimi-k2.7-code` is wired. Operator-marked strike.",
      "gate": "auto",
      "planning": "auto"
    },
    {
    ...
    },
    {
      ...
      "condition": "ok",
      "dependsOn": [],
      "why": "Stopped/running agent surfaces: no drill-down, no context, inconsistent counts",
      "gate": "auto",
      "planning": "auto"
    },
    {
