...
_Last sequenced: 2026-07-12T22:32:37.933Z · model: glm-5.2 · open: 599_


| rank | issue | size | importance | condition | epic | depends-on | why |
|------|-------|------|------------|-----------|------|------------|-----|
| 1 | PAN-2567 | M | critical | ok |  | PAN-1650 | Reviewed+green PR never merges under main churn — advancing verdict reconciled forever |
| 2 | PAN-2593 | XS | critical | ok |  |  | Server children inherit bare PATH — verification gates run under system Node 18, not Node 22 |
| 3 | PAN-2569 | M | critical | ok |  |  | Planning finalizes (issue→planned) but work agent never auto-spawns — silent handoff failure |
| 4 | PAN-2379 | S | critical | ok |  |  | Verify-gate dependency install is warn-only + 60s timeout → false failures vs empty node_modules |
| 5 | PAN-2259 | M | critical | needs-refinement |  |  | Something burns the full 5k/hr GitHub GraphQL quota — breaks pan close, gh edits, orchestration |
| 6 | PAN-806 | L | critical | ok | ✓ |  | Substrate work; improves the foundation required for reliable shipping. |
| 7 | PAN-2337 | M | critical | ok |  |  | In-place npm run build under live dashboard breaks new PTY-supervisor spawns until restart |
| 8 | PAN-2331 | S | critical | ok |  |  | Codex rate-limit "Switch to mini?" modal stalls autonomous agents — no auto-dismiss |
| 9 | PAN-2285 | M | critical | ok |  |  | Per-agent codex-home auth.json rots — seed-once copy forks OAuth tokens, 401 wedge loop |
| 10 | PAN-2511 | M | high | ok |  |  | Work agents burn 20+ min on false test failures — sandbox denies spawnSync git (EPERM) |
| 11 | PAN-2179 | M | high | ok |  |  | Relaunch can leave a zombie agent — session alive but kickoff never delivered |
| 12 | PAN-2372 | S | medium | ok |  |  | Swarm slot finishes its beads but never runs pan done — deacon can't converge it; permanent stall in the default nudge mode. |
...
| 14 | PAN-2516 | S | high | ok |  |  | Spec plan.status flips left uncommitted in shared primary worktree → drift, blocks flywheel push |
| 15 | PAN-2333 | M | high | ok |  | PAN-2331 | Handle codex weekly-quota exhaustion — resource alert + downshift policy, not frozen modal |
| 16 | PAN-578 | L | high | needs-refinement |  |  | Security: comment mediation layer to stop prompt injection via tracker comments |
| 17 | PAN-1435 | M | high | ok |  |  | API keys in ~/.panopticon/config.yaml stored as plaintext — any local process can read them |
| 18 | PAN-1915 | L | high | ok |  | PAN-1435 | API key at-rest hardening — startup perm check + OS keychain + deprecate plaintext |
...
| 21 | PAN-2558 | L | high | ok |  |  | Polyrepo state-migration: MyN state tracked in NO git repo — latent data-loss |
...
| 23 | PAN-2168 | L | medium | ok |  |  | Hardens the pipeline paths that ship all other work. |
| 24 | PAN-1650 | L | high | ok |  |  | Split readyForMerge → gatesPassed (event-driven) + shipComplete; auto-dispatch ship on green |
...
| 27 | PAN-2079 | L | high | ok |  |  | Operator Inbox: durable server-side queue + in-dashboard surface (notification spine) |
...
| 29 | PAN-2540 | S | high | ok |  |  | Inspect route /beads/:beadId/inspect skips bd-bead-id → vBRIEF-item resolution |
| 30 | PAN-2521 | XS | high | ok |  | PAN-2331 | Launch pipeline agents with harness rate-limit model-switch reminder disabled |
| 31 | PAN-2451 | M | high | ok |  |  | Work agent stranded behind commit-msg gate after overflow-restart + auto-commit + merge-main |
| 32 | PAN-2186 | M | high | ok |  |  | Post-merge lifecycle can leave merged issues in-review and auto-merge rows stuck |
| 33 | PAN-2075 | XL | high | ok | ✓ | PAN-2079, PAN-2077, PAN-2078, PAN-2080 | [EPIC] Boot Reconciliation + Operator Inbox — informed, substrate-complete, reachable |
| 34 | PAN-2376 | XL | high | needs-refinement | ✓ |  | [EPIC] CI/CD reliability — flake policy, verify→merge convergence, strike/swarm, deploy hygiene |
| 35 | PAN-1666 | XL | high | ok | ✓ | PAN-1556 | [EPIC] Pipeline Throughput Hardening — many work agents safely, on-demand specialists, slots |
| 36 | PAN-807 | L | critical | needs-refinement | ✓ |  | Epic C: workspace state sanity on spawn |
| 37 | PAN-804 | XL | critical | needs-refinement | ✓ |  | Epic D: archaeological audit & pre-1.0 cleanup |
| 38 | PAN-2188 | M | high | ok |  |  | Flywheel resilience for codebase-health flood — substrate-first + tenets spirit-gate |
| 39 | PAN-2430 | M | high | ok |  |  | Frontend typecheck fails with dozens of pre-existing unused-local errors |
| 40 | PAN-2421 | S | high | ok |  | PAN-2511 | Dashboard server route tests flake under full-suite verification load |
| 41 | PAN-2334 | M | high | ok |  |  | Write a Definition of Ready — the bar an issue must clear before planning/pickup |
| 42 | PAN-2358 | S | high | ok |  |  | Restore PAN-1535 hardening in transformMessageForHarness (lost in conversations.ts split) |
| 43 | PAN-2323 | M | high | ok |  |  | Flywheel respawn after crash starts a blank session instead of resuming the live one |
| 44 | PAN-2324 | S | high | ok |  |  | Close-out label transition fails atomically on missing in-planning label — stale labels remain |
| 45 | PAN-2165 | M | high | ok |  |  | pan close: close-issue phase reports success but leaves issue OPEN / wrong labels |
| 46 | PAN-2106 | M | high | ok |  |  | pan strike workspace setup leaves broken partial workspace + false spawned success |
| 47 | PAN-2170 | S | high | ok |  |  | Docker init container lacks Python — node-gyp rebuild of better-sqlite3 fails (forces --host) |
| 48 | PAN-2169 | M | high | ok |  |  | kimi agent silently frozen at 100% ctx (no overflow error) — not caught by overflow patterns |
| 49 | PAN-2189 | L | high | ok |  |  | Decompose src/lib/cloister/deacon.ts (3394 lines) — pipeline machinery, supervised handoff |
| 50 | PAN-2232 | L | high | ok |  |  | Decompose specialists.ts (1749 lines) into focused modules |
| 51 | PAN-2233 | L | high | ok |  |  | Decompose merge-agent.ts (1414 lines) — preserve postMergeLifecycle idempotency + Docker cleanup |
| 52 | PAN-2190 | L | high | ok |  |  | Decompose routes/workspaces/merge-ops.ts (1925 lines) — new god file from the workspaces split |
| 53 | PAN-2077 | L | high | ok |  |  | Substrate-complete reconciliation inventory (local tmux + remote Fly) — one resolver |
| 54 | PAN-2078 | M | high | ok |  | PAN-2077 | CLI parity for boot reconciliation — pan boot status + pan resume --all|--select|--freeze |
| 55 | PAN-2229 | L | high | ok |  |  | Hardens the pipeline paths that ship all other work. |
| 56 | PAN-1454 | M | high | ok |  |  | [META] 9 systemic failure patterns from 80-issue audit — substrate work vs closed-not-shipped |
...
| 59 | PAN-2027 | M | high | ok |  |  | ohmypi: route kimi-k2 through ohmypi instead of CLIProxy — eliminates 200k-window illusion |
| 60 | PAN-262 | L | high | ok |  | PAN-806 | Refactor post-merge lifecycle into composable, idempotent operations |
| 61 | PAN-1560 | M | high | ok |  |  | Re-review after a PR head moves doesn't re-post review status → PR stranded BLOCKED |
| 62 | PAN-2597 | L | high | ok |  |  | Adopt codex app-server (JSON-RPC over stdio) as Codex transport — retire TUI keystroke injection |
| 63 | PAN-1770 | S | high | ok |  |  | pan-dir auto-commit rebase races live .pan/continues writes — rebase failed every busy cycle |
| 64 | PAN-1618 | M | high | ok |  |  | Work-spawn docker-health gate has no autonomous recovery — proposed work can't auto-start |
| 65 | PAN-2193 | M | medium | ok |  |  | Held issues (objection/parked/vetoed/needs-handoff) invisible in Command Deck tree |
| 66 | PAN-1766 | M | high | ok |  |  | Work agents hang on Claude Code settings-file protection editing .claude/** — un-overridable hook |
| 67 | PAN-1217 | M | medium | ok |  |  | Requirements reviewer: classify each AC as in-PR-scope vs whole-feature; only !-block in-PR |
| 68 | PAN-1219 | M | medium | ok |  |  | Promote across-cycle review state (cycle SHA, prior findings) to first-class data |
| 69 | PAN-1196 | L | medium | ok |  |  | Architecture: workhorse routing by bead difficulty + subject-matter (single + swarm) |
| 70 | PAN-2468 | M | medium | ok |  |  | OKF knowledge skill v1 — Karpathy-loop wiki + okf-embeddings vector extension (/okf). |
| 71 | PAN-1313 | L | high | ok |  |  | Finish src/lib Effect migration — remove or justify legacy Promise/sync surfaces |
| 72 | PAN-1556 | M | medium | ok |  |  | Coalesce re-reviews so on-demand specialists are not bulk-spawned (PAN-1666 workstream B) |
| 73 | PAN-1126 | S | medium | ok |  |  | Integrate TLDR summaries into the review context manifest |
| 74 | PAN-1130 | S | medium | ok |  |  | Headless review sub-reviewer normal exit misclassified as crashed — spurious restart |
| 75 | PAN-1830 | S | high | ok |  |  | Reviewer stuck on gpt-5.5 rate-limit modal blocks REVIEWER_READY — synthesis waits forever |
...
| 81 | PAN-1451 | M | medium | ok |  |  | PAN-1124 follow-up: complete planning-on-main pivot (dropped ACs from scope drift) |
...
| 87 | PAN-1198 | M | high | ok |  |  | Workspace init container's bun install doesn't populate container-node-modules named volume |
| 88 | PAN-1497 | L | high | ok |  |  | feat(flywheel): emit TTS announcements on lifecycle events (start, pause, resume, report) |
| 89 | PAN-1218 | M | high | ok |  |  | Bead inspect: drop Check 3 (compile/lint), restrict to foundation beads, add end-of-batch mode |
| 90 | PAN-2080 | M | high | ok |  |  | Operator Inbox external transports (email/Slack/push/TTS) — offline reach (fast-follow, absorbs #43) |
| 91 | PAN-2377 | L | high | ok |  |  | feat(flywheel): first-class 'special orders' runs — operator-supplied order book executed with lane semantics |
| 92 | PAN-1578 | M | high | ok |  |  | GitHub Copilot CLI as a first-class harness (pipeline peer to Claude Code, Pi, Codex) |
| 93 | PAN-1538 | M | high | ok |  |  | Unblock Pi source forks — remove API guard, verify transcript parsers |
| 94 | PAN-1142 | M | high | ok |  |  | Add reasoning effort level to per-role / per-conversation model config |
| 95 | PAN-1311 | M | high | needs-refinement |  |  | Swarm: fast-track tier — skip slot dispatch for trivial mechanical items |
| 96 | PAN-2542 | L | medium | ok |  |  | feat(models): add GPT-5.6 (Sol/Terra/Luna) family, make gpt-5.6-sol the new OpenAI default |
| 97 | PAN-1424 | M | high | needs-refinement |  |  | Model pool dispatch + work.* subtype taxonomy (follow-up to PAN-1122) |
| 98 | PAN-630 | M | high | ok |  |  | Multi-tenant workspace isolation with ACLs |
| 99 | PAN-1246 | M | medium | ok |  |  | Perf: projection-cached VCS driver for diff/checkpoint reads (port of t3code #2586) |
| 100 | PAN-2568 | M | medium | ok |  |  | Summary fork delivery race: large summary lands in codex composer but is never submitted; conversation looks dead |
| 101 | PAN-1561 | M | medium | ok |  |  | feat: Project-scoped dashboard nav (deck of tabs per project + conversations/tree column + activity feed) |
| 102 | PAN-1558 | M | medium | needs-refinement |  |  | Review/specialist agents should run in the workspace Docker container, not inherit host-override |
| 103 | PAN-1357 | M | medium | ok |  |  | Template conversations: load curated skill bundles into a single conversation |
| 104 | PAN-1254 | L | medium | ok |  |  | Tailscale integration: advertise dashboard + workspace endpoints over tailnet (Effect-native) |
| 105 | PAN-1253 | M | medium | ok |  |  | Flywheel: respect issue dependencies before autopicking work |
| 106 | PAN-2059 | XL | medium | ok | ✓ |  | [EPIC] Backlog pickup gate — operator Plan→Release row + AI Objection (5th state) + Flywheel relevance-vetting |
| 107 | PAN-1711 | M | medium | ok |  |  | Dashboard event loop stalls 15-25s under load — watchdog force-restarted it 3x in 45 min |
| 108 | PAN-1544 | M | medium | ok |  |  | Type cleanup: strip 'ship' from the Role union and its ~10 downstream references |
| 109 | PAN-1027 | M | medium | ok |  |  | Merge-status drift: deacon auto-detect paths set mergeStatus=merged without postMergeLifecycle, never reset on revert |
| 110 | PAN-113 | M | medium | ok |  |  | Dashboard 'Start Agent' returns success before verifying agent actually started |
| 111 | PAN-454 | M | medium | ok |  |  | Crash recovery: detect orphaned agents and present recovery UI on dashboard startup |
| 112 | PAN-1416 | M | medium | ok |  |  | Workspace-spawned dashboard servers can bind the main pan.localhost port and hijack the canonical dashboard |
...
| 114 | PAN-2478 | M | medium | ok |  |  | CI flake: Playwright browser install fails on packages.microsoft.com apt (NOSPLIT), red-mains legit merges |
| 115 | PAN-1436 | M | medium | needs-refinement |  |  | PAN-1419 follow-up: stale stopped-agent zombies still pollute dashboard list |
| 116 | PAN-334 | M | medium | ok |  |  | Dashboard server has no duplicate-process protection — zombie instances cause 502 |
| 117 | PAN-324 | M | medium | ok |  |  | Agent detail pane missing Merge/Approve button |
| 119 | PAN-1782 | M | medium | ok |  |  | Handoff forks stall at "Injecting…" then die on double 300s summary timeout — decouple precompaction from the handoff author model |
| 120 | PAN-1060 | M | medium | ok |  |  | Self-modify permission handling: stop the interrupt loop without weakening the safety guard |
| 121 | PAN-2044 | M | medium | ok |  |  | UI: import conversations from old panopticon.db into overdeck.db (Settings → Experimental) |
| 122 | PAN-681 | M | medium | ok |  |  | Feedback routing: wrong issueId written to workspace when verification runs for co-active issues |
| 123 | PAN-38 | M | medium | ok |  |  | Support multiple merge agents per repository |
| 124 | PAN-37 | M | medium | ok |  |  | Support external PR selection for merge-agent |
| 125 | PAN-1966 | M | medium | ok |  |  | Single authoritative pipeline-membership resolver — one function for "what's in the pipeline" (collapse the 5 divergent views) |
| 126 | PAN-538 | M | medium | ok |  |  | npm run build sometimes skips Vite frontend rebuild |
| 127 | PAN-1610 | M | medium | ok |  |  | Consistent issue actions across all surfaces (Command Deck cockpit, Pipeline rows, Board cards, IssueDrawer) |
...
| 128 | PAN-1438 | M | medium | ok |  |  | pan flywheel start launcher process orphans when orchestrator dies externally |
| 129 | PAN-1227 | M | medium | needs-refinement |  |  | Substrate: bead can be closed without delivering the work — add per-bead delivery check in pan done |
| 130 | PAN-1525 | L | high | ok |  |  | Substrate work; improves the foundation required for reliable shipping. |
| 131 | PAN-1113 | M | medium | ok |  |  | Conversations sidebar lets you message review-specialist sessions, which derails them silently |
| 132 | PAN-1068 | M | medium | ok |  |  | PAN-1048 deferred findings: security, correctness, and model validation gaps |
| 133 | PAN-769 | M | medium | ok |  |  | Track verification/review/test phase churn over time |
| 134 | PAN-255 | M | medium | ok |  |  | Agents lack awareness of MCP tools — sync MCP config and inject into prompts |
| 135 | PAN-49 | S | medium | ok |  |  | Fix CloisterService tests that require real runtime |
| 136 | PAN-1913 | M | medium | ok |  |  | Project description: show on click, edit in dashboard, mirror into the project layer (and document what's in .pan and ~/.panopticon) |
| 137 | PAN-1897 | S | medium | ok |  |  | bug(cli): pan start workspace-prep hangs/times out (>120s) on re-entry — blocks PAN-1711, PAN-1827 (no spawn, no error) |
| 138 | PAN-1828 | M | medium | ok |  |  | Conversation fork/handoff harness defaults ignore source conversation harness — silent claude-code coercion |
| 139 | PAN-1816 | M | medium | ok |  |  | Scratch/UAT-lifecycle issues (PAN-18031) enter the real pipeline: kanban, review convoys, agent registry — need an ephemeral flag +... |
| 140 | PAN-1673 | S | medium | ok |  |  | Regression: pi + gpt-5.5 fails with 'No API key for provider: openai-codex' (worked previously) |
| 141 | PAN-1624 | M | medium | ok |  |  | pan handoff --author external: authored doc is socket_write-ten but never submitted — successor sits at empty welcome screen |
| 142 | PAN-1245 | M | medium | ok |  |  | Flywheel gate gets stuck after orchestrator dies (reboot, crash, partial report) |
| 143 | PAN-1154 | M | medium | ok |  |  | pan up does not kill existing port holders — startup races against orphan dashboard servers |
| 144 | PAN-2603 | L | medium | ok |  |  | feat(docs-rag): complete (or retire) the docs RAG integration — PAN-1203 shipped lib-only |
| 145 | PAN-607 | M | medium | ok |  |  | Evaluate Ultimate Bug Scanner (UBS) for verification gate |
| 146 | PAN-675 | M | medium | ok |  |  | Deacon: detect API rate-limit events, surface on dashboard, auto-restart when window resets |
| 147 | PAN-629 | M | medium | ok |  |  | Workspace quotas and resource governance |
| 148 | PAN-247 | M | medium | ok |  |  | Deacon has no backoff or escalation for repeated specialist startup failures |
| 149 | PAN-245 | M | medium | ok |  |  | Ctrl+C aborts planning dialog instead of copying text |
| 150 | PAN-244 | M | medium | needs-refinement |  |  | Deep-wipe leaves local branch and worktree metadata behind |
| 151 | PAN-47 | M | medium | needs-refinement |  |  | PRD files should be committed to feature branch, moved to completed/ on merge |
| 152 | PAN-2580 | M | medium | ok |  |  | pan tell cannot deliver to codex (GPT) conversations — runtime stays null, delivery door misclassifies live session as zombie |
| 153 | PAN-2546 | S | medium | ok |  |  | bug(cli): pan tell is codex-conversation-unaware — declares live codex sessions zombie and refuses delivery |
| 154 | PAN-2495 | M | medium | ok |  |  | PAN-2487 ci-green merge skip bypassed CI-green gate — landed red-main change |
| 155 | PAN-2467 | M | medium | ok |  |  | Multi-repo merge train merges only one repo, strands sibling repos' branches (MIN-857 api half never merged) |
| 156 | PAN-2242 | M | medium | ok |  |  | Unidentified duplicate caller fires complete-planning in pairs every ~2 minutes (perpetual loop while session survives) |
| 157 | PAN-2202 | M | medium | ok |  |  | complete-planning silently skips spec promotion on a dead session's unanswered AskUserQuestion — and finalize reports false success |
| 158 | PAN-1696 | M | medium | ok |  |  | feat: decouple merge-train from the Flywheel — per-project pipeline feature + multi-project view |
| 159 | PAN-1824 | L | medium | ok |  |  | Flaky main CI: real-timer integration tests time out (~5s) on loaded runners — fork recovery, rollout-JSONL, heartbeat, conversatio... |
| 160 | PAN-1774 | S | medium | ok |  |  | bug(uat): workspace server container crashloops when dist/dashboard/server.js is missing |
| 161 | PAN-1755 | S | medium | ok |  |  | bug(cloister): uat stuck-assembly cap (30m) kills slow-but-alive assemblies and leaves orphaned conflict agents racing the next gen... |
| 162 | PAN-1735 | L | medium | ok |  |  | feat(flywheel): adopt externally-completed readyForMerge issues into the pipeline/merge queue |
| 163 | PAN-1627 | M | medium | ok |  |  | Substrate: Claude Code's native .claude/** settings-edit protection wedges in-scope work agents (un-overridable by PreToolUse auto-... |
| 164 | PAN-1572 | M | medium | ok |  |  | Settings permission-mode can desync from resolved config — agents silently use --dangerously-skip-permissions despite 'Auto' |
| 165 | PAN-1571 | M | medium | ok |  |  | Large multi-line pastes (handoff docs) land unsubmitted — paste/submit verification is blind to Claude's collapsed "[Pasted text +N... |
| 166 | PAN-1461 | M | medium | ok |  |  | Conversation transcript: in-page search (Ctrl+F) only finds text in currently-rendered virtualized rows |
| 167 | PAN-1444 | M | medium | needs-refinement |  |  | Follow-up to PAN-1416: dashboard port lockfile + pan doctor multi-instance check |
| 168 | PAN-1433 | M | medium | ok |  |  | Conversation agents can leave host main repo in abandoned git rebase state for hours |
| 169 | PAN-1226 | L | medium | ok |  |  | PAN-1148 unified-dashboard redesign — 32 gaps vs PRD and mockups (full audit) |
| 170 | PAN-608 | M | medium | ok |  |  | Integrate Destructive Command Guard (dcg) with configurable settings |
| 171 | PAN-958 | L | medium | ok |  |  | Implement vBRIEF issue sync: migrate and reconcile GitHub issues into specification |
| 172 | PAN-938 | M | medium | ok |  |  | Fizzy visual pipeline — Kanban mirror for specialist pipeline |
| 173 | PAN-546 | M | medium | ok |  |  | Remove claude-code-router — all providers use direct env var injection |
| 174 | PAN-452 | M | medium | ok |  |  | Conversation input bar — mode/permissions/workspace selectors |
| 175 | PAN-304 | M | medium | ok |  |  | closeLinearDirect returns stepOk even when state update never happens |
| 176 | PAN-178 | M | medium | ok |  |  | PAN-178: Crash recovery with granular task checkpointing |
| 177 | PAN-2507 | M | medium | ok |  |  | Preemptive pipeline scheduler: yield idle work agents to unblock review/test/merge dispatch |
| 178 | PAN-2491 | M | medium | ok |  |  | Migrate @xenova/transformers to @huggingface/transformers to eliminate silent npx install failures from sharp 0.32 postinstall |
| 179 | PAN-2469 | L | medium | ok |  |  | feat(swarm): issue-level assembly owner — 'all slots done' must deterministically trigger assemble → verify → review (root cause of... |
| 180 | PAN-2416 | S | medium | ok |  |  | bug(cloister): codex agents can wedge on the Codex CLI first-run/consent screen — spawn must pre-accept non-interactively |
| 181 | PAN-2406 | S | medium | ok |  |  | close-out gaps: verify-merged rejects record-only deltas; slot/suffixed worktrees never torn down; teardown abort fires after workt... |
| 182 | PAN-2069 | M | medium | needs-refinement |  |  | caveman: follow-up gaps — review agent routing, hook execution tests, Settings UI toggle, Experiments view |
| 183 | PAN-1951 | M | medium | needs-refinement |  |  | Inspector agent should resume a warm session instead of cold-spawning a new one per bead |
| 184 | PAN-1769 | M | medium | ok |  |  | Supervisor echo-confirm false negative on long messages → triple-paste delivery (rewrite ×2 + tmux fallback); resumed-conv message ... |
| 185 | PAN-1101 | M | medium | ok |  |  | Permission safety hardening: CI guard, single emission chokepoint, property tests, runtime tripwire |
| 186 | PAN-1545 | L | medium | ok |  |  | feat(dashboard): New Terminal button — spawn ad-hoc bash sessions from sidebar / conversation / drawer / palette |
| 187 | PAN-1542 | M | medium | ok |  |  | Spawn-refusal modal: render the three-button workflow on dirty-workspace 409 |
| 188 | PAN-1432 | M | medium | ok |  |  | Merge agent leaves packages/contracts/dist stale — typecheck breaks on every fresh checkout |
| 189 | PAN-1151 | M | medium | ok |  |  | Anthropic Enterprise auth: distinguish from consumer subscription for Pi+Anthropic harness gating |
| 190 | PAN-1234 | M | medium | needs-refinement |  |  | Bug fix with direct operator or pipeline reliability impact. |
| 191 | PAN-1232 | M | medium | needs-refinement |  |  | Bug fix with direct operator or pipeline reliability impact. |
| 192 | PAN-947 | M | medium | ok |  |  | feat: project management actions in unified sidebar |
| 193 | PAN-532 | M | medium | ok |  |  | Per-project and per-issue model overrides for workflow agent model selection |
| 194 | PAN-783 | L | medium | ok |  |  | Agents Page Redesign — Unified Multi-View Experience |
| 195 | PAN-77 | M | medium | ok |  |  | Cost breakdown modal: show costs by stage and model when clicking cost badge |
| 196 | PAN-658 | M | medium | ok |  |  | Shared Sessions v0: GitHub-auth'd shared conversation panel with WebRTC transport |
| 197 | PAN-2351 | M | medium | ok |  |  | Overdeck Anywhere P0: scoped access tokens + WS/SSE heartbeats (security prerequisites) |
| 198 | PAN-1995 | M | medium | ok |  |  | infra: set up smee webhook relay so merge-on-green + post-merge are reactive (not deacon-only) |
| 199 | PAN-603 | M | medium | ok |  |  | Plan review loop with configurable reviewer model |
| 200 | PAN-1965 | M | medium | ok |  |  | Project pipeline view: true-state buckets + lens reconciliation (pipeline as exception queue) |
| 201 | PAN-1854 | M | medium | ok |  |  | Define handoff strategy for large conversations: external vs source authoring + tail-biased read |
| 202 | PAN-1672 | M | medium | ok |  |  | GPT-5.5/CLIProxy context-window deadlock: conversations get no overflow recovery + 200k window illusion |
| 203 | PAN-1577 | M | medium | ok |  |  | Move a conversation to a different project (CLI + drag/drop + menu action) |
| 204 | PAN-1102 | L | medium | ok |  |  | feat(dashboard): real-time notification + interactive prompts when agent awaits user input |
| 205 | PAN-1488 | M | medium | ok |  |  | chore(repo): add required_pull_request_reviews to main branch protection |
| 206 | PAN-243 | M | medium | ok |  |  | Audit dashboard actions: ensure all are available via CLI |
| 207 | PAN-471 | M | medium | ok |  |  | Cost reconciler: auto-trigger on agent lifecycle events with debounce |
| 208 | PAN-1164 | M | medium | ok |  |  | Push diff summary updates over /ws/rpc instead of 5s polling |
| 209 | PAN-1041 | M | medium | ok |  |  | Audit and consolidate REMOTE/LOCAL gates in work-agent prompt template |
| 210 | PAN-1040 | L | medium | ok |  |  | feat(infra): event-driven dispatch for inspect-agent (requiresInspection=true beads) |
| 211 | PAN-932 | M | medium | ok |  |  | pan done: polyrepo uncommitted changes check + existing MR handling |
| 212 | PAN-933 | M | medium | ok |  |  | Review poster cannot post to GitLab MRs (only supports GitHub PRs) |
| 213 | PAN-900 | M | medium | ok |  |  | Trust devroot for conversations + atomic .claude.json writes |
| 214 | PAN-886 | M | medium | ok |  |  | pan review request shows 'fetch failed' instead of actual sync-target-branch error |
| 215 | PAN-826 | XL | medium | ok |  |  | Conversation/terminal integration refactor: instant-start + parser correctness + T3Code structural alignment |
| 216 | PAN-817 | M | medium | ok |  |  | Improve planning dialog layout and content fit |
| 217 | PAN-709 | L | medium | ok |  |  | feat(flywheel): self-improving flywheel — retro agent, skill-change pipeline, audience-scoped skills, Q&A detection, autonomous daemon |
| 218 | PAN-736 | M | medium | ok |  |  | feat: wire per-subagent model overrides from settings to Claude Code spawn env |
| 219 | PAN-727 | S | medium | ok |  |  | Fix orphaned work-agent start handoff after planning |
| 220 | PAN-660 | M | medium | ok |  |  | Slash menu command catalog drifts: hardcoded array in ComposerPromptEditor needs codegen |
| 221 | PAN-624 | M | medium | ok |  |  | Loop nodes: iterative agent execution with conditional termination |
| 222 | PAN-622 | M | medium | ok |  |  | YAML workflow DAGs: custom per-project pipeline definitions |
| 223 | PAN-480 | M | medium | ok |  |  | Pass --effort flag when spawning planning agents via Cloister |
| 224 | PAN-258 | M | medium | ok |  |  | Kanban board: fit all columns without horizontal scrolling |
| 225 | PAN-252 | M | medium | ok |  |  | Disable Sync with Main button when workspace is up to date |
| 226 | PAN-774 | M | medium | ok |  |  | Unify launch UX and release pipeline for 1.0 — npx panctl, lazy prereqs, cross-platform desktop builds |
| 227 | PAN-1386 | M | medium | ok |  |  | Flywheel orchestrator never emits status snapshots — dashboard 'flywheel' pane stays blank during an active run |
| 228 | PAN-1530 | M | medium | needs-refinement |  |  | Investigate: state.json with model='gpt-5.5' (a model that doesn't exist) |
| 229 | PAN-927 | M | medium | ok |  |  | Rewrite containerize route: dead code, orphan processes, no pending-op tracking |
| 230 | PAN-1240 | M | medium | ok |  |  | Ship-complete PRs going CONFLICTING after main moves need auto re-rebase recovery |
| 231 | PAN-797 | M | medium | needs-refinement |  |  | Cost display: cache write tokens not shown separately; investigate Claude Code discrepancy |
| 232 | PAN-1565 | M | medium | ok |  |  | Defensive mitigation: auto-recover conversations poisoned by Claude Code thinking-block resume 400 (upstream #63147) |
| 233 | PAN-1473 | XL | medium | ok |  |  | Dashboard conversation composer: refactor context indicator to mirror t3code (show cumulative + live separately) |
| 234 | PAN-1173 | S | medium | ok |  |  | pan show <bare-number> derives wrong agent ID for PAN-prefixed issues |
| 235 | PAN-1449 | M | medium | needs-refinement |  |  | PAN-1052 follow-up: memory extraction failing 59% on dogfood project + storage layout deviates from spec |
| 236 | PAN-1446 | L | medium | needs-refinement |  |  | PAN-1231 follow-up: remove or implement Table + Timeline modes in FleetAgentsView (scope-creep stubs) |
| 237 | PAN-1445 | L | medium | needs-refinement |  |  | PAN-1389 follow-up: remove or implement Files + Comments tabs in SessionFeedSidebar (scope-creep stubs) |
| 238 | PAN-1440 | M | medium | needs-refinement |  |  | Follow-up to PAN-1158: bd export --refuse-empty guard + dolt-empty root cause |
| 239 | PAN-1392 | M | medium | ok |  |  | pan close: archive-planning:move-prd fails when completed/ PRD exists but workspace PRD also exists |
| 240 | PAN-1330 | S | medium | ok |  |  | CLI cannot address planning-*/specialist-* sessions — pan tell/pan kill hard-code 'agent-' prefix; no 'pan plan abort' |
| 241 | PAN-1150 | M | medium | ok |  |  | Settings: "Anthropic is not configured" warning persists in Model Routing after claude /login (Provider tab disagrees) |
| 242 | PAN-1149 | M | medium | ok |  |  | v0.9.3 upgraders: stale workhorses.mid: claude-sonnet-4-7 in config.yaml keeps breaking Model Routing saves |
| 243 | PAN-1129 | M | medium | ok |  |  | Review-request route pushes wrong branch name: 'feature/977' instead of 'feature/pan-977' |
| 244 | PAN-1128 | M | medium | ok |  |  | Channels: spurious 'no MCP server configured with that name' banner at conversation startup |
| 245 | PAN-1063 | M | medium | ok |  |  | Harden tts_daemon.py: bearer auth, CORS, body size cap, concurrency bound |
| 246 | PAN-908 | M | medium | ok |  |  | PAN-908: Make work-agent spawn limits configurable and overridable |
| 247 | PAN-833 | M | medium | ok |  |  | Agent spawn logs ENOTDIR for .git/pan-credentials in worktrees (GitHub App credential loader) |
| 248 | PAN-810 | M | medium | ok |  |  | Inspector: diagnostic UI when pipeline phase is unknown |
| 249 | PAN-778 | M | medium | ok |  |  | Write conflict race: review-agent fails when test-agent write scope not yet released |
| 250 | PAN-764 | M | medium | ok |  |  | Add quota/usage inspector for routed model providers |
| 251 | PAN-752 | M | medium | ok |  |  | Add Gemini OAuth support, remove O3/O4-mini, disable GPT-5.4-Pro |
| 252 | PAN-293 | M | medium | ok |  |  | Project Living Memory — per-project semantic memory for agents |
| 253 | PAN-294 | M | medium | ok |  |  | Surface module initialization errors as system-level, not per-issue |
| 254 | PAN-277 | M | medium | ok |  |  | Session reasoning capture & collaborative PRD refinement |
| 255 | PAN-1166 | M | medium | ok |  |  | Re-introduce /ws/terminal auth gate with a working bootstrap path |
| 256 | PAN-2037 | M | medium | ok |  |  | UI: prominent 'Start work agent' CTA on all issue surfaces when agent is stopped |
| 257 | PAN-1674 | M | medium | ok |  |  | TLDR .venv (~7.5G) is duplicated into every workspace — 236G across 33 worktrees, caused disk-full ENOSPC |
| 258 | PAN-1912 | M | medium | ok |  |  | Pi agent transcripts hide tool-call detail; agent panes lack the Tools show/hide toggle |
| 259 | PAN-1795 | M | medium | ok |  |  | Codebase map bootstrapped in planning worktree is never promoted to main (PAN-1788 WI-6 wiring gap) |
| 260 | PAN-654 | M | medium | ok |  |  | Project Setup Wizard — Dashboard UI |
| 261 | PAN-1481 | M | medium | ok |  |  | Add cost-event telemetry for Caveman token savings |
| 262 | PAN-1482 | M | medium | needs-refinement |  |  | Token spend report should aggregate data from repo, not just local machine |
| 263 | PAN-1479 | M | medium | ok |  |  | RTK: Add telemetry to measure token savings from bash output compression |
| 264 | PAN-1356 | M | medium | ok |  |  | Extend the memory Observation pipeline to ad-hoc conversations |
| 265 | PAN-1244 | M | medium | ok |  |  | pan admin cloister start: CLI crashes with SIGSEGV (exit code 139) after handing off to server |
| 266 | PAN-1064 | M | medium | ok |  |  | Harden launcher generation against shell-quote injection (model and arg quoting) |
| 267 | PAN-537 | M | medium | ok |  |  | feat: show changed files diff summary after each agent response in activity view |
| 268 | PAN-700 | M | medium | ok |  |  | Detachable terminal for conversation view — popout into OS window |
| 269 | PAN-713 | M | medium | ok |  |  | test: add unit tests for doneCommand and approveCommand |
| 270 | PAN-687 | M | medium | ok |  |  | Support OpenCode as alternative coding agent |
| 271 | PAN-678 | M | medium | ok |  |  | pan work issue --auto: headless planning → agent handoff without interactive dialog |
| 272 | PAN-649 | M | medium | ok |  |  | Render Excalidraw drawings inline in Claude Code conversations |
| 273 | PAN-646 | M | medium | ok |  |  | Canceled issues: add guided Recover workflow |
| 274 | PAN-637 | M | medium | ok |  |  | Direct issue kickoff (skip planning) from dashboard UI |
| 275 | PAN-613 | M | medium | needs-refinement |  |  | Investigate thinking effort levels for agents — reduce signature corruption frequency |
| 276 | PAN-606 | M | medium | ok |  |  | Evaluate MCP Agent Mail for inter-agent communication and file reservations |
| 277 | PAN-548 | M | medium | ok |  |  | Command Deck: preserve state across navigation including URL routing for tabs |
| 278 | PAN-531 | M | medium | ok |  |  | PAN: Windows Electron support (WSL2 required) |
| 279 | PAN-466 | M | medium | ok |  |  | Add QwenCoder CLI as a supported runtime alongside Claude Code and Codex |
| 280 | PAN-465 | M | medium | ok |  |  | Add OpenRouter as a model provider |
| 281 | PAN-463 | M | medium | ok |  |  | Add Qwen 3.6+ model support |
| 282 | PAN-450 | M | medium | ok |  |  | Adopt remaining Effect patterns — Schema, Platform, Streams, Logging, Testing |
| 283 | PAN-55 | M | medium | ok |  |  | Track specialist costs with time period filtering |
| 284 | PAN-106 | M | medium | ok |  |  | Cost prediction/estimation for in-progress work |
| 285 | PAN-104 | M | medium | ok |  |  | Cost alerts/notifications when spending exceeds thresholds |
| 286 | PAN-54 | L | medium | ok |  |  | feat: Add pan test:e2e command for full workflow integration test |
| 287 | PAN-44 | M | medium | needs-refinement |  |  | Planning should fetch ALL issue context: comments, attachments, linked issues, discussions |
| 288 | PAN-43 | M | medium | ok |  |  | Add Slack and email notifications for agent events |
| 289 | PAN-2563 | M | medium | ok |  |  | npm-flavor desktop (npx @overdeck/desktop) lacks node_modules for the server's externalized deps |
| 290 | PAN-2554 | S | medium | ok |  |  | bug(dashboard): clicking a project doesn't update the browser URL — project view isn't copyable/shareable/bookmarkable |
| 291 | PAN-2550 | S | medium | ok |  |  | bug(test): npm test exits 0 despite root-suite failures — 31 failed tests reported green at the command level |
| 292 | PAN-2547 | S | medium | ok |  |  | bug(cli): pan restart --health-timeout parses seconds as milliseconds — '--health-timeout 180' waits 180ms then declares failure |
| 293 | PAN-2319 | S | medium | ok |  |  | bug(cost-monitor): 'COST LIMIT REACHED for undefined' spams every cycle — fix undefined daily_total subject, throttle log, consolid... |
| 294 | PAN-2245 | M | medium | ok |  |  | vBRIEF bead materialization: first bd create after clear deterministically exceeds the 30s floor under bd contention, leaving a par... |
| 295 | PAN-2244 | M | medium | ok |  |  | Recurring [pan-dir/auto-commit] GitError on main — half-staged spec file blocks all pan-dir mirroring (continue mirrors never land) |
| 296 | PAN-2243 | M | medium | ok |  |  | pan plan finalize: CLI aborts complete-planning at 90s while the server handler legitimately finishes later (false ✖ Failed) |
| 297 | PAN-2241 | M | medium | ok |  |  | complete-planning is not serialized or idempotent per issue (spec tmp-rename 500s, bead delete-recreate thrash) |
| 298 | PAN-2240 | S | medium | ok |  |  | bug(agents): pan tell contradicts itself on dead ohmypi sessions — 'session is dead and resume failed: it appears healthy' |
| 299 | PAN-2237 | S | medium | ok |  |  | bug(cli): pan plan done swallows vbrief quality lint details |
| 300 | PAN-2031 | S | medium | ok |  |  | ohmypi: add Bun 1.3.11 regression test to checkOhmypi doctor gate |
| 301 | PAN-2029 | M | medium | ok |  |  | ohmypi: capture kimi thinking_tokens in ohmypi-parser for complete cost accounting |
| 302 | PAN-2006 | M | medium | ok |  |  | Pipeline semantics lock-down: Definition of Ready, pickup gates (parked/vetoed/blocks-main), unblock override, and Run definition |
| 303 | PAN-190 | M | medium | ok |  |  | PAN-190: Specialized reviewer prompts (industry best-practice checklists) |
| 304 | PAN-1042 | M | medium | ok |  |  | cost_events retention: 14 months of granular rows accumulating with ad-hoc partial deletions |
| 305 | PAN-1985 | M | medium | ok |  |  | Agent wipe-and-respawn family (work + review): harness/model switch + Complete work reset, with confirmation |
| 306 | PAN-1980 | M | medium | ok |  |  | Stop session rotation on resume (behind a constant); one pipeline-membership view from all lenses |
| 307 | PAN-1758 | S | medium | ok |  |  | bug(cloister): ship lane cannot converge on a continuously-moving main — 37 re-dispatches for one issue; readyForMerge only ever fl... |
| 308 | PAN-1926 | L | medium | ok |  |  | feat(strike): --big flag to lift strike's precision-only scope guard (operator-authorized larger strikes) |
| 309 | PAN-1918 | S | medium | ok |  |  | bug(ci): full frontend vitest suite runs in no CI path — npm test limited to 3 files; IssueMissionControl.test.tsx open-handle hang... |
| 310 | PAN-1895 | M | medium | ok |  |  | Spawn work agents from issue workspace slide-out |
| 311 | PAN-1740 | M | medium | ok |  |  | Deacon mislabels SIGTERM workspace container restarts as crashes |
| 312 | PAN-1710 | S | medium | ok |  |  | bug(ci): 'Clean install + server smoke test' hangs (3 consecutive 20-min timeout kills) on feature/pan-1491 and feature/pan-1641 — ... |
| 313 | PAN-1691 | L | medium | ok |  |  | feat(flywheel): conflict-aware merge train + on-demand UAT candidate — stop the rebase-cascade that strands ready PRs |
| 314 | PAN-1646 | M | medium | ok |  |  | Rabbit-hole drift detection and lift-to-new-conversation |
| 315 | PAN-863 | M | medium | ok |  |  | Workspace + branch hygiene sweep (124 feature/* branches, 28 worktrees) |
| 316 | PAN-1122 | M | medium | ok |  |  | Trim OpenAI model catalog to 5 supported models |
| 317 | PAN-1037 | S | medium | ok |  |  | Retire 'planning-' tmux prefix — fold into agent-PAN-N keyed by phase |
| 318 | PAN-949 | M | medium | ok |  |  | feat: add conversation for project from sidebar |
| 319 | PAN-903 | M | medium | ok |  |  | Detect ~/.claude.json corruption on startup and surface it in the dashboard |
| 320 | PAN-902 | M | medium | ok |  |  | Settings: add 'Run pan sync' button to configuration menu |
| 321 | PAN-901 | M | medium | ok |  |  | Settings: add Maintenance panel with Claude Code Organizer + Config Editor quick-launch |
| 322 | PAN-818 | M | medium | ok |  |  | Make summary optional when forking conversations |
| 323 | PAN-802 | M | medium | ok |  |  | Resume on conversation session forks instead of resuming |
| 324 | PAN-227 | L | medium | ok |  |  | Phase gate validation — mid-implementation acceptance checks |
| 325 | PAN-265 | M | medium | ok |  |  | Review skill categorization: all skills available everywhere via personal + workspace |
| 326 | PAN-249 | M | medium | ok |  |  | Add data-testid attributes across dashboard UI and create Playwright smoke test suite |
| 327 | PAN-241 | L | medium | ok |  |  | Mobile redesign initiative: full UX/UI overhaul + implementation plan |
| 328 | PAN-228 | M | medium | ok |  |  | Shift-left post-edit diagnostics — type check after every edit |
| 329 | PAN-198 | M | medium | ok |  |  | Structured audit trail for agent actions |
| 330 | PAN-180 | M | medium | ok |  |  | PAN-180: Cross-terminal file locking for concurrent agents |
| 331 | PAN-177 | M | medium | ok |  |  | PAN-177: Iteration limits with escalation for autonomous agents |
| 332 | PAN-176 | M | medium | ok |  |  | PAN-176: Hook-enforced delegation guardrails for specialist agents |
| 333 | PAN-175 | M | medium | ok |  |  | PAN-175: Pre-compact auto-save hook for agent sessions |
| 334 | PAN-155 | L | medium | ok |  |  | PAN-155: Redesign health page with Stitch (system overview, timeline, costs) |
| 335 | PAN-146 | M | medium | ok |  |  | PAN-146: Refine light mode theming across all dashboard pages |
| 336 | PAN-2572 | M | medium | ok |  |  | Noisy EBADENGINE + deprecation warnings on npx/npm install make a healthy install look broken |
| 337 | PAN-2560 | M | medium | ok |  |  | resolveStateReadHomeSync (state-read-home.ts) resolves state dir by path basename, not registry key — migrated projects silently fa... |
| 338 | PAN-2492 | S | medium | ok |  |  | bug(needs-you): pane-detected waits (rate-limit/session-resume) surface as 'needs you' but cannot be answered from the dashboard — ... |
| 339 | PAN-2487 | L | medium | ok |  |  | feat(ship): CI-green merge skip + Ship & Merge cockpit view (live door log + progress) + active-node spinner |
| 340 | PAN-2484 | S | medium | ok |  |  | fix(uat-train): ready set misses merge-eligible issues without flywheel merge verbs — eligibility sweep added; verb-coverage prompt... |
| 341 | PAN-2423 | S | medium | ok |  |  | bug(workspace): pan workspace rebuild hardcodes 'overdeck-' compose project prefix — mismatches project templates and verification ... |
| 342 | PAN-2443 | L | medium | ok |  |  | feat(costs): OpenTelemetry GenAI semconv — OTLP ingestion layer for cross-harness telemetry (tokens/latency/tools), pinned-snapshot... |
...
| 344 | PAN-2414 | S | medium | ok |  |  | bug(cloister): context-overflow recovery is inconsistent — some agents get the PAN-1781 compact-respawn, others hit the PAN-1980 ro... |
| 345 | PAN-2424 | XL | medium | ok |  |  | Epic: the Order Book — first-class operator priority queue (markdown-authored, backlog-exempt, load-governed, flywheel-integrated, ... |
| 346 | PAN-2399 | L | medium | ok |  |  | feat(tiered): wire replay_threshold/compaction_reroute into the slot-recovery respawn seam (PAN-2397 W3b) |
| 347 | PAN-2392 | L | medium | ok |  |  | feat(dashboard): Standing Crew cost panel — per-member roster with cost, tokens, verdicts, escalations (mockup included) |
| 348 | PAN-2381 | S | medium | ok |  |  | bug(dashboard): three event types missing from DomainEvent schema union poison the RPC stream — permanent "Reconnecting…" loop |
| 349 | PAN-2356 | M | medium | ok |  |  | Overdeck Anywhere P3: relay service — outbound-only daemon, GitHub OAuth, push origin, multi-tenant front door |
| 350 | PAN-2355 | M | medium | ok |  |  | Overdeck Anywhere P2: mobile PWA (Needs-You feed, conversation view, pipeline board, Web Push) |
| 351 | PAN-1165 | M | medium | ok |  |  | Lightweight review path for small/trivial PRs |
| 352 | PAN-2295 | L | medium | needs-refinement |  |  | feat(overdeck): built-in web browser surface (openable like terminal/Claude Code/Codex) + native Agentation integration |
| 353 | PAN-2280 | M | medium | ok |  |  | Resumed conversations wedge without writing transcripts when dashboard is black-holed — views diverge from terminals (conv 367 et al.) |
| 354 | PAN-2213 | M | medium | ok |  |  | Swarm slot allocator picks an orphaned slot index and refuses instead of skipping to the next free one |
| 355 | PAN-2212 | M | medium | ok |  |  | Swarm slot dispatch has no reserved budget — a busy pipeline starves it to zero |
| 356 | PAN-2211 | M | medium | needs-refinement |  |  | PAN-2203 follow-up: swarm slot pan done records completion but slot never becomes merge-ready |
| 357 | PAN-2210 | M | medium | needs-refinement |  |  | PAN-2203 follow-up: a swarm slot's completion can trigger the issue-level review pipeline |
| 358 | PAN-2195 | M | medium | ok |  |  | pan plan finalize re-plan churn: stale superseded spec on main transiently materializes the old plan |
| 359 | PAN-1868 | M | medium | ok |  |  | Cost-bleed circuit breaker: progress-aware, always-on guard against runaway agent spend |
| 360 | PAN-1896 | M | medium | ok |  |  | Reduce approval friction for GitHub CLI operations in managed sessions |
| 361 | PAN-1640 | M | medium | ok |  |  | Re-platform interactive permission allow/deny onto a PreToolUse hook (provider-agnostic) |
| 362 | PAN-1524 | M | medium | ok |  |  | Slash command aliases: /handoff → /pan-handoff (and similar short forms) |
| 363 | PAN-1490 | L | medium | ok |  |  | feat(dashboard): show each conversation's current git branch (port t3code BranchToolbar pattern) |
| 364 | PAN-1489 | M | medium | needs-refinement |  |  | task(flywheel): tune v1.0 readiness criteria after 30 days of telemetry |
| 365 | PAN-1485 | M | medium | ok |  |  | Auto-archive stale conversations: pre-archive warning at 7 days, archive at 10 days, configurable |
| 366 | PAN-1442 | M | medium | needs-refinement |  |  | Follow-up to PAN-829: voice-sampler.html cleanup in pan-tts repo |
| 367 | PAN-1437 | M | medium | ok |  |  | pan flywheel report semantics: split read-only snapshot from run finalization |
| 368 | PAN-1223 | M | medium | needs-refinement |  |  | Auto-update for users in the field (npm + desktop binaries) |
| 369 | PAN-634 | M | medium | ok |  |  | Documentation cleanup: restructure docs, update installation (npx panctl), refresh stale PRDs |
| 370 | PAN-299 | M | medium | ok |  |  | Granular session state persistence across context compaction |
| 371 | PAN-298 | M | medium | ok |  |  | Auto-detect package manager and runtime in workspace setup |
| 372 | PAN-297 | M | medium | ok |  |  | Workspace templates: pre/post tool hooks for auto-format, typecheck, lint |
| 373 | PAN-283 | M | medium | needs-refinement |  |  | Reset should sync workspace feature branch with latest main |
| 374 | PAN-271 | M | medium | ok |  |  | Auto-assign Linear project from project config when creating issues |
| 375 | PAN-51 | M | medium | ok |  |  | Documentation: Clarify issue tracker options beyond Linear |
| 376 | PAN-2065 | L | medium | ok |  |  | feat(dashboard): unified usage & headroom panel across all provider plans (z.ai, Anthropic, Codex, OpenRouter) |
| 377 | PAN-1991 | L | medium | ok |  |  | Issue cockpit redesign — incremental rollout (tracking) |
| 378 | PAN-2004 | M | medium | ok |  |  | Resumable Planning node: double-click a planned issue's Planning to resume the planning agent |
| 379 | PAN-1968 | M | medium | ok |  |  | Finish local-domain rename: pan.localhost → overdeck.localhost |
| 380 | PAN-1967 | L | medium | ok |  |  | Flywheel must re-validate (re-plan) pre-cutover plans before implementing them |
| 381 | PAN-1776 | L | medium | ok |  |  | feat(supervisor): hot-updatable delivery path — version-stamped supervisors, rolling refresh, and dumb-shim primitives with server-... |
| 382 | PAN-1916 | L | medium | ok |  |  | feat(search): configurable web search providers (Exa, Tavily, Brave, Perplexity) |
| 383 | PAN-1852 | M | medium | ok |  |  | Capability-tiered work-agent model selection: difficulty→capability-floor routing from benchmark-anchored eval data |
| 384 | PAN-1853 | M | medium | ok |  |  | Surface a transcript-size warning on growing conversations (2 MB warn / 10 MB strong-nudge tiers) |
| 385 | PAN-1844 | M | medium | ok |  |  | Deep-linkable Command Deck: reflect selected issue/agent in the browser URL + make activity notifications link to the specific view |
| 386 | PAN-1840 | M | medium | ok |  |  | Add 'pan switch <id>' — change a running agent's model/harness in one command (kill + fresh-start + re-onboard) |
| 387 | PAN-1839 | M | medium | ok |  |  | Settings → Providers: show each provider's default harness in the collapsed row (no expand needed) |
| 388 | PAN-1837 | M | medium | ok |  |  | Support Kimi Code as a first-class harness (Moonshot's own coding CLI) |
| 389 | PAN-1676 | L | medium | ok |  |  | feat(fly.io): harden remote workspaces + `pan workspace move` local↔remote (scale-out / overflow slots) |
| 390 | PAN-1671 | L | medium | ok |  |  | feat(dashboard): surface pending ExitPlanMode plan as a popup modal (reuse PlanCard + /plan-action) |
| 391 | PAN-1657 | M | medium | ok |  |  | feat: one-off double-check reviews with a user-specified agent/harness + settings-managed default reviewer |
| 392 | PAN-1656 | M | medium | ok |  |  | Skills page: make it a full management surface (browse, review, edit, scope, sync status) |
| 393 | PAN-1655 | M | medium | ok |  |  | Skills: scope by audience AND by agent role (conversation/work/review/ship/plan/test), sync accordingly |
| 394 | PAN-1654 | M | medium | ok |  |  | perf(build): run lint:skills from source via tsx, skip CLI dist build (salvaged from PAN-1615 workspace) |
| 395 | PAN-1623 | M | medium | ok |  |  | Codex: surface interactive approval prompts as conversation Q&A (like AskUserQuestion) |
| 396 | PAN-735 | M | medium | ok |  |  | Settings page: review and configure overridden subagent model files |
| 397 | PAN-730 | M | medium | ok |  |  | Add provider account telemetry for credits, balances, and usage |
| 398 | PAN-702 | S | medium | ok |  |  | OpenAI provider: add plan/subscription support and fix unregistered model resolution |
| 399 | PAN-701 | M | medium | ok |  |  | Quick-Create conversation via keystroke using Conversations-page default model |
| 400 | PAN-623 | M | medium | ok |  |  | Multi-channel workflow triggers: Slack, Discord, Telegram, GitHub webhooks |
| 401 | PAN-604 | M | medium | ok |  |  | Hide planning agent from workspace detail pane |
| 402 | PAN-576 | M | medium | needs-refinement |  |  | Global / search should include conversations in addition to workspace features |
| 403 | PAN-571 | M | medium | ok |  |  | Add OpenRouter credits/plan status endpoint and UI |
| 404 | PAN-570 | M | medium | ok |  |  | Show PLAN badge on costs when under a subscription/plan |
| 405 | PAN-568 | M | medium | ok |  |  | Kanban: Show workspace and tmux session counts in stats |
| 406 | PAN-565 | M | medium | ok |  |  | Handle CTRL-Z to undo accidental conversation archival |
| 407 | PAN-564 | M | medium | ok |  |  | Slash menu positioned incorrectly — cut off / off-screen |
| 408 | PAN-554 | M | medium | ok |  |  | Add kanban board deeplinks for issue URLs |
| 409 | PAN-543 | M | medium | ok |  |  | Add confirmation dialog before applying Optimal Defaults |
| 410 | PAN-483 | M | medium | needs-refinement |  |  | Unify Resume Agent UX — all entry points should show message input |
| 411 | PAN-476 | M | medium | ok |  |  | Agent resume with Haiku session summary instead of claude --resume |
| 412 | PAN-468 | M | medium | ok |  |  | Agent test conversations pollute production database — need test isolation |
| 413 | PAN-461 | M | medium | ok |  |  | Deep-wipe multi-step progress dialog |
| 414 | PAN-459 | M | medium | ok |  |  | Planning setup screen with SSE progress streaming |
| 415 | PAN-438 | M | medium | ok |  |  | Migrate remaining REST polling endpoints to Effect RPC |
| 416 | PAN-2599 | M | medium | ok |  |  | Integrate PostHog for product analytics and telemetry |
| 417 | PAN-2582 | L | medium | ok |  |  | feat(swarm): show slot assignments on the vBRIEF DAG + unify swarm/tiered terminology (Lead/Crew or Trunk/Lanes) |
| 418 | PAN-2596 | L | medium | ok |  |  | feat(dashboard): generic file attachment in conversation composer (+ button and drag-and-drop) |
| 419 | PAN-2566 | XL | medium | ok |  |  | Traycer parity epic: gap analysis of capabilities Overdeck lacks |
| 420 | PAN-2565 | M | medium | ok |  |  | Multi-agent conversations: N agent sessions in one task surface with agent-to-agent messaging |
| 421 | PAN-2557 | L | medium | ok |  |  | feat(dashboard): project-level 'Restart All' context action — restart every agent in a project, throttled by the PAN-2500 memory go... |
| 422 | PAN-1987 | M | medium | ok |  |  | Routine backlog item; rank reflects current shipping leverage. |
| 423 | PAN-2556 | L | medium | ok |  |  | feat(dashboard): add a per-issue 'Restart agent' action (stop+start active role) — the restartAgent type exists but isn't wired int... |
| 424 | PAN-2553 | L | medium | ok |  |  | feat(dashboard): project-level CI visibility — surface repo/main-branch workflow runs on the Command Deck with click-through to logs |
| 425 | PAN-2551 | L | medium | ok |  |  | chore(cli): state-migration hardening round 2 — auto-rebuild beads DB + encode the live cutover's resume scenarios as tests |
| 426 | PAN-2548 | M | medium | ok |  |  | chore(state): close the PAN-2541 legacy-fallback deprecation window — delete dual-path resolution once every project carries the D1... |
| 427 | PAN-2526 | XL | medium | ok |  |  | Refactor deacon.ts below file-size baseline |
| 428 | PAN-2335 | M | medium | ok |  |  | chore: review the full open backlog for junk/stale/nonsensical issues — produce a categorized document for operator review (FIND ON... |
| 429 | PAN-2288 | L | medium | needs-refinement |  |  | tmux managed-server: lossless auto-migration of dirty-founded servers + boot-time ensure call (PAN-1798 follow-up) |
| 430 | PAN-2266 | M | medium | ok |  |  | feat: add zcode harness and make it the default for glm-5.2 |
| 431 | PAN-832 | M | medium | ok |  |  | state.json staleness: lastActivity/costSoFar not updated as agent runs; /api/agents drops phase/cost/lastActivity |
| 432 | PAN-1443 | M | medium | needs-refinement |  |  | Follow-up to PAN-487: migrate 10 stale .vbrief.json files from docs/prds/active/ to completed/ |
| 433 | PAN-1065 | M | medium | ok |  |  | Validate issueId at every shell-string interpolation site (defense in depth) |
| 434 | PAN-1051 | M | medium | ok |  |  | feat: Subspace-inspired alternate theme with Inter + JetBrains Mono |
| 435 | PAN-1049 | M | medium | ok |  |  | Spike: evaluate Tauri v2 desktop shell |
| 436 | PAN-984 | M | medium | ok |  |  | Evaluate context-mode MCP server as session continuity + search layer |
| 437 | PAN-962 | M | medium | needs-refinement |  |  | Post-PAN-946: vBRIEF lifecycle follow-up plan |
| 438 | PAN-944 | M | medium | ok |  |  | Make vBRIEF the durable task graph source of truth |
| 439 | PAN-943 | M | medium | ok |  |  | Add memory file review and management command |
| 440 | PAN-771 | M | medium | needs-refinement |  |  | Investigate Vercel Sandbox execution backend support |
| 441 | PAN-749 | M | medium | ok |  |  | Research and borrow best features from gstack |
| 442 | PAN-898 | M | medium | ok |  |  | Dashboard polling and WebSocket efficiency: remaining audit findings |
| 443 | PAN-790 | L | medium | ok |  |  | PAN-789: Eliminate remaining TanStack Query polling — complete push-first migration |
| 444 | PAN-786 | M | medium | ok |  |  | Post planning Q\&A answers as issue comment |
| 445 | PAN-777 | M | medium | ok |  |  | Inter-agent communication skill: send messages to conversation-mode agents |
| 446 | PAN-775 | L | medium | ok |  |  | Redesign workspace inspector panel: sidebar layout is cramped and wrong |
| 447 | PAN-773 | M | medium | ok |  |  | Design prompt-style overlays with model hierarchy and scoped toggles |
| 448 | PAN-772 | M | medium | ok |  |  | Unify terminal stack behavior across tmux sessions |
| 449 | PAN-765 | M | medium | ok |  |  | Preserve trailing zeros in cost displays |
...
| 451 | PAN-762 | M | medium | ok |  |  | Settings: warn when model overrides target disabled providers |
| 452 | PAN-750 | L | medium | ok |  |  | PAN-XXX: Complete Metrics Page Redesign — Real Data, Charts, Time Filtering, and TLDR Analytics |
| 453 | PAN-751 | M | medium | ok |  |  | PAN-XXX: Historical Metrics Data Persistence — Beyond the 30-Day JSONL Window |
| 454 | PAN-747 | M | medium | ok |  |  | Conversation list items lack accessible labels in accessibility tree |
| 455 | PAN-738 | M | medium | ok |  |  | Add right-click fork option to conversation list |
| 456 | PAN-1592 | M | medium | ok |  |  | Composer: make ephemeral composer state reload-durable (pasted images + unsent/failed message text) |
| 457 | PAN-1242 | M | medium | needs-refinement |  |  | Board view follow-up — + New issue column footer button (deferred from PAN-1229) |
| 458 | PAN-1683 | S | medium | ok |  |  | docs: canonical agent session-prefix registry + reconcile role taxonomy (ROLES.md/AGENT_TYPES_INDEX/CLAUDE.md) — strike keeps falli... |
| 459 | PAN-1653 | M | medium | ok |  |  | perf(docs-rag): batch local embedding in buildDocsIndex (salvaged from PAN-1617 workspace) |
| 460 | PAN-1581 | M | medium | ok |  |  | Duplicate skills in picker: code-review collides with official plugin; beads/pan-flywheel/pan-handoff doubled across project+user sync |
| 461 | PAN-1553 | M | medium | needs-refinement |  |  | Investigate Claude Code Fast mode support (and fast-tier pricing) |
| 462 | PAN-1552 | M | medium | ok |  |  | Dashboard conversation-message 500 cause is unloggable: serve mode never writes dashboard.log |
| 463 | PAN-1550 | L | medium | ok |  |  | feat: FilesPane + BrowserPane — file browser and embedded web view implementation details |
| 464 | PAN-1533 | M | medium | ok |  |  | Fork-into-worktree from conversation branch chip |
| 465 | PAN-1480 | L | medium | ok |  |  | TLDR: 93% bypass rate — daemon/hook integration broken |
| 466 | PAN-1124 | M | medium | ok |  |  | Decouple specs and PRDs from workspaces — write directly to main |
| 467 | PAN-1325 | M | medium | ok |  |  | Artifact storage model is unsafe for polyrepo projects — define a canonical "orchestration repo" |
| 468 | PAN-1222 | M | medium | ok |  |  | Project-templated DB lifecycle: auxiliary databases + seed refresh from prod |
| 469 | PAN-1208 | M | medium | ok |  |  | Polyrepo: support non-feature 'main' workspaces alongside feature-* |
| 470 | PAN-1121 | M | medium | ok |  |  | Context bloat: agents receive oversized prompts that exceed tool limits and force immediate compaction |
| 471 | PAN-1153 | M | medium | ok |  |  | Vite TRAEFIK_ENABLED conflates 'Traefik on' with 'inside container' — breaks pan dev proxy |
| 472 | PAN-1136 | M | medium | ok |  |  | Hook system cleanup: dead inspect-on-bead-close, pan-review-agent inconsistency |
| 473 | PAN-1133 | M | medium | ok |  |  | TLDR: deacon supervision + pan doctor check + GC |
| 474 | PAN-1123 | M | medium | ok |  |  | Channels delivery: surface failures, add fallback toggle, route conversations through channels |
| 475 | PAN-1116 | M | medium | ok |  |  | Memory: cross-project search mode |
| 476 | PAN-1990 | M | medium | ok |  |  | First-class workspaces and projects with per-workspace memory |
| 477 | PAN-1773 | M | medium | needs-refinement |  |  | Swarm v2 Phase 2: remote slot agents on Fly (B5 follow-up to PAN-1762) |
| 478 | PAN-2091 | M | medium | ok |  |  | chore(dashboard): delete dead IssueCockpitBody cockpit subtree (8 files, superseded by IssueMissionControl) |
| 479 | PAN-1958 | M | medium | ok |  |  | Source-tagged programmatic delivery into pi conversation agents (extension sendUserMessage + input.source) |
| 480 | PAN-2085 | M | medium | ok |  |  | Auto-isolate conversations in a lightweight git worktree (Conductor-style workspaces) |
| 481 | PAN-2084 | M | medium | ok |  |  | Auto-create lightweight conversation worktrees on project chats |
| 482 | PAN-2083 | M | medium | ok |  |  | Composer: a failed first send leaves the text in BOTH the composer box and the retry outbox |
| 483 | PAN-2082 | M | medium | ok |  |  | Composer: a single send failure clears ALL in-flight optimistic bubbles (and strips siblings' compaction net) |
| 484 | PAN-1775 | L | medium | ok |  |  | feat(dashboard): remote (fly.io) work agents need a real session row in the issue tree — chip-only visibility reads as 'no agent' |
| 485 | PAN-2074 | M | medium | needs-refinement |  |  | research: evaluate ponytail (DietrichGebert/ponytail) for prompt compression and consider building in-house |
| 486 | PAN-2066 | M | medium | ok |  |  | OKF knowledge skill — deferred v2 capabilities (search, viz, lease writes, MCP, semantic auditor) |
| 487 | PAN-2045 | M | medium | ok |  |  | perf(test): frontend vitest (jsdom) is the test-gate bottleneck — ~5min vs ~72s root; move to happy-dom / tune pool |
| 488 | PAN-2046 | M | medium | ok |  |  | Conversation view does not surface terminal command responses |
| 489 | PAN-2035 | M | medium | ok |  |  | ohmypi: GitHub Copilot subscription provider routing via omp |
| 490 | PAN-2034 | M | medium | ok |  |  | ohmypi: end-to-end test that tool-call steps render in Conversation panel |
| 491 | PAN-2033 | M | medium | ok |  |  | ohmypi: benchmark FIFO vs paste-buffer message delivery latency |
| 492 | PAN-2032 | M | medium | ok |  |  | ohmypi: local Ollama model as zero-cost preliminary review role |
| 493 | PAN-2030 | M | medium | ok |  |  | ohmypi: version-pin extension in package.json and pan doctor mismatch warning |
| 494 | PAN-2028 | M | medium | ok |  |  | ohmypi: per-provider cost grouping in cost dashboard |
| 495 | PAN-2026 | M | medium | ok |  |  | ohmypi: surface 35+ provider matrix in dashboard model picker |
| 496 | PAN-2025 | M | medium | ok |  |  | ohmypi: extend provider credential passthrough for Groq, Cerebras, Fireworks |
| 497 | PAN-2024 | M | medium | ok |  |  | ohmypi: frontend Tools-toggle for conversation view |
| 498 | PAN-2008 | L | medium | ok |  |  | feat(ci): store-access guard — fail the build on direct store reads outside a domain resolver (PAN-1936 slice) |
| 499 | PAN-2005 | M | medium | ok |  |  | Backlog Sequencer: Pickup Forecast — visualize Flywheel pickup order (waves, lanes, planning bottleneck) |
| 500 | PAN-2002 | M | medium | ok |  |  | [HUMAN-ONLY] Sign & notarize the macOS desktop build (Apple Developer ID) |
| 501 | PAN-1999 | M | medium | ok |  |  | Backlog Sequencer: one sequencer per project (currently a single global runner scoped to PAN) |
| 502 | PAN-1668 | S | medium | ok |  |  | bug(dashboard): right-click 'restart with <model>' carries model only, never harness — can't move a review off Kimi |
| 503 | PAN-1986 | M | medium | ok |  |  | restartAgent (change harness/model): wipe stale agent-dir session pointers + refresh conversations row |
| 504 | PAN-1988 | M | medium | ok |  |  | Verdict signaling: one host-owned write door; agents journal, host owns the DB cache |
| 505 | PAN-399 | M | medium | ok |  |  | Hardens the pipeline paths that ship all other work. |
| 506 | PAN-1641 | M | medium | ok |  |  | Local model support via Ollama sidecar (Gemma 4 12B) for the Pi harness |
| 507 | PAN-1937 | M | medium | ok |  |  | feat: data export — portable bundle (conversations + favorites core; decoupled optional cost ledger) + user-facing Export my data |
| 508 | PAN-1949 | M | medium | ok |  |  | Surface inspection sub-runs in the issue tree + a parent Inspection node aggregating all bead verdicts |
| 509 | PAN-1936 | M | medium | ok |  |  | Single source-of-truth reads — one canonical resolver per domain (consolidate the 280+ scattered read endpoints) |
| 510 | PAN-1914 | M | medium | needs-refinement |  |  | Follow-up: move /api/health/agents off agent-directory scans |
| 511 | PAN-1910 | M | medium | ok |  |  | fast-follow(PAN-1908): collapse issue status to ONE canonical field — labels become a derived projection, not the source of truth |
| 512 | PAN-1907 | M | medium | ok |  |  | Generalize ToS gate: block ALL non-Claude-Code harnesses from Anthropic-subscription models; gray out + non-selectable + validate e... |
| 513 | PAN-1906 | M | medium | ok |  |  | Enforce harness restrictions with subscription: gray out non-claude-code, validate everywhere |
| 514 | PAN-1720 | S | medium | ok |  |  | bug(test): cloister auto-resume tests fail under full parallel run, pass in isolation — test pollution reddening main |
| 515 | PAN-1846 | S | medium | ok |  |  | bug(cloister): unbounded log growth — deacon.log 687MB / dashboard.log 91MB, no rotation; per-agent skip line logged every 60s patrol |
| 516 | PAN-1761 | S | medium | ok |  |  | bug(dashboard): conversations endpoints fetched via relative /api path — 403 inside workspace/UAT containers (session cookie is on ... |
| 517 | PAN-1750 | L | medium | ok |  |  | feat(flywheel): UAT assembly/conflict agent — observability surfaces + configurable harness/model (default gpt-5.5 via Codex) |
| 518 | PAN-1754 | L | medium | ok |  |  | feat(settings): surface + edit the host claude CLI default model (~/.claude/settings.json) from the Settings page |
| 519 | PAN-1751 | L | medium | ok |  |  | feat(settings): harness picker on every Settings → Roles row (plan/work/review/test/ship/strike), not just Flywheel |
| 520 | PAN-1748 | L | medium | ok |  |  | feat(cloister): reuse uat-assembly conflict resolutions across generations (rerere or resolution replay) |
| 521 | PAN-1728 | S | medium | ok |  |  | bug(work): PAN-1700 agent committed .pan/specs/*.vbrief.json mutations — PAN-1124 immutability violated on feature branch |
| 522 | PAN-1643 | M | medium | ok |  |  | Extend local Ollama support to Codex + Claude Code harnesses and dashboard model picker |
| 523 | PAN-1669 | S | medium | ok |  |  | bug(dashboard): restart-with-model doesn't emit a live event — issue tree shows stale model until manual refresh |
| 524 | PAN-1667 | L | medium | ok |  |  | feat(dashboard): unify Agents + Resources into one issue-centric holistic view |
| 525 | PAN-1469 | M | medium | ok |  |  | End-to-end review and consolidation of all project documentation |
| 526 | PAN-961 | M | medium | ok |  |  | Update documentation for vBRIEF v0.6 lifecycle model |
| 527 | PAN-924 | L | medium | ok |  |  | Spike: evaluate GitNexus for Panopticon integration |
| 528 | PAN-2600 | M | medium | needs-refinement |  |  | Retire the Codex TUI path after app-server burn-in (no-loss audit gate) — follow-up to PAN-2597 |
| 529 | PAN-2499 | L | medium | ok |  |  | feat(dashboard): unify the three issue views into one progressive-density Issue View (rail · cockpit · console) |
| 530 | PAN-2549 | M | medium | ok |  |  | Fly remote workspaces: sync overdeck-state before re-enabling migrated projects |
| 531 | PAN-2535 | M | medium | ok |  |  | POST /api/agents returns unhandled 500 (not 422) when `bd list` exits non-zero |
| 532 | PAN-2527 | M | medium | needs-refinement |  |  | Harness selector should restrict OpenAI models to Claude Code only |
| 533 | PAN-2514 | M | medium | ok |  |  | Claude Code Traffic Inspector — intercept & inspect model API traffic in the dashboard |
| 534 | PAN-2506 | M | medium | ok |  |  | flywheel-primary-root.test.ts fails on macOS: /var vs /private/var symlink not canonicalized |
| 535 | PAN-2505 | M | medium | ok |  |  | lint:circular reports new frontend cycles + stale baseline in chat/conversations components |
| 536 | PAN-2504 | M | medium | ok |  |  | Auto-relaunch npx @overdeck/core under a compatible Node 22+ instead of failing on old Node |
| 537 | PAN-2501 | S | medium | ok |  |  | bug(dashboard): deleteResourceVenvEffect's HttpRouter.schemaParams call fails typecheck under the root tsconfig (masked by src/dash... |
| 538 | PAN-2493 | L | medium | ok |  |  | feat(parity): align the cockpit Agents-lane and sidebar issue-tree feature sets (two-way gaps) |
| 539 | PAN-2489 | S | medium | ok |  |  | bug(tree): strike agents are invisible in the project issue tree — needs-you pings with no node to click |
| 540 | PAN-2466 | S | medium | ok |  |  | bug(records): close-out/record writer clobbers closeOut.usage with EMPTY data — cost history lost on the local side (recurring) |
| 541 | PAN-2465 | S | medium | ok |  |  | bug(done): pan done's PR lookup fails at MYN polyrepo root — 'no git remotes found' makes completion exit nonzero |
| 542 | PAN-2454 | S | medium | ok |  |  | bug(infra): ratchet audit fails per-commit on push ranges whose NET baseline delta is zero — strands finished branches |
| 543 | PAN-2445 | S | medium | ok |  |  | bug(cloister): deacon lifecycle patrol auto-dispatches PLANNING for stale 'planning'-state issues — off-book, and staffed from role... |
| 544 | PAN-2449 | M | medium | ok |  |  | start-planning: GITHUB_REPOS env shadows projects.yaml github_repo; unknown IDs fall through to Linear and plan the wrong issue |
| 545 | PAN-2444 | L | medium | ok |  |  | feat(agents): optional SageOx re-integration — session-reasoning capture for OSS projects (per-project opt-in, v0.11-era ox) |
| 546 | PAN-2442 | L | medium | ok |  |  | feat(agents): Agent Client Protocol (ACP) as Overdeck's structured control plane — replace tmux keystrokes, transcript parsers, and... |
| 547 | PAN-2422 | S | medium | ok |  |  | bug(infra): rebuilding dist under a live server breaks lazy chunk imports — 'Cannot find module dist/dashboard/<chunk>.js' |
| 548 | PAN-2409 | L | medium | ok |  |  | feat(cloister): enforce the workspace boundary — work agents must not edit the primary checkout (PAN-2204 class, reproduced 3x on 2... |
| 549 | PAN-2408 | S | medium | ok |  |  | bug(cli): pan start --auto commits the spec to main AFTER creating the worktree — agent's own workspace lacks its spec, causing wro... |
| 550 | PAN-2395 | S | medium | ok |  |  | bug(config): one invalid tiered_execution enum poisons every config read — live conversations falsely marked ended, resume/new-conv... |
| 551 | PAN-2394 | M | medium | ok |  |  | Incident: conv-* agent-dir cleanup destroyed ohmypi/codex conversation transcripts ("no saved history") |
| 552 | PAN-2390 | M | medium | ok |  |  | systemd-oomd killed overdeck-tmux-server.service (all 55 agent processes) under host memory pressure — set ManagedOOMPreference=avo... |
| 553 | PAN-2354 | M | medium | ok |  |  | Overdeck Anywhere P1c: needs-you push notification bridge (ntfy first, Web Push later) |
| 554 | PAN-2353 | M | medium | ok |  |  | Overdeck Anywhere P1b: Hermes external-agent bridge (scoped API + Fly 6PN) |
| 555 | PAN-2352 | M | medium | ok |  |  | Overdeck Anywhere P1a: remote dashboard access via Cloudflare Tunnel + Access |
| 556 | PAN-2350 | XL | medium | ok |  |  | Epic: Overdeck Anywhere — remote access, Hermes bridge, mobile, and the shared relay backbone |
| 557 | PAN-2308 | M | medium | needs-refinement |  |  | hardening(workspaces): migrate stale generated compose files off PORT=3011 + deacon quarantine for deterministic container boot ref... |
| 558 | PAN-2287 | S | medium | ok |  |  | bug(supervisor): every supervisor.log line written twice — log() appendFile + launcher stdout redirect target the same file |
| 559 | PAN-2282 | M | medium | ok |  |  | Conversation view shows no history for ohmypi-harness conversations — pi transcript surface missing (conv 353) |
| 560 | PAN-2252 | M | medium | ok |  |  | Dashboard port has no identity check — workspace peer server squatted :3011 for 6 minutes and passed all health checks |
| 561 | PAN-2201 | M | medium | ok |  |  | Close-out label step fails atomically when a hardcoded label (e.g. 'in-planning') is absent from the repo — closed issues keep stal... |
| 562 | PAN-2197 | S | medium | ok |  |  | bug(codex): work agents skip `pan done` (manual push instead) — sandbox blocks its GitHub calls; idle agents spuriously 'troubled' |
| 563 | PAN-1684 | M | medium | ok |  |  | docs(marketing): build full marketing kit + plan (SEO, video list, channels) from MARKETING.md seed |
| 564 | PAN-1474 | M | medium | ok |  |  | Add ACKNOWLEDGEMENTS doc — credit borrowed code from open-source projects (MIT/Apache 2.0) |
| 565 | PAN-1135 | M | medium | ok |  |  | Document the hook system in docs/HOOKS.md |
| 566 | PAN-1117 | M | medium | ok |  |  | Memory: pinned docs (long-form doc chunking + retrieval) |
| 567 | PAN-633 | L | medium | ok |  |  | Update Cloister PRD and docs index — stale relative to implementation |
| 568 | PAN-1878 | M | low | ok |  |  | process: bake 'docs updated' into acceptance criteria / definition-of-done in role + planning prompts |
| 569 | PAN-52 | M | low | ok |  |  | Guidance needed: Running complex multi-container projects with Panopticon worktrees |
| 570 | PAN-2348 | M | low | ok |  |  | docs: migrate STATE-STORAGE-AUDIT.md content to living docs, then delete |
| 571 | PAN-2347 | M | low | ok |  |  | docs: refresh AGENT-STATE-PLANES.md — update, harden, make useful |
| 572 | PAN-2346 | M | low | ok |  |  | docs: refresh AGENT_TYPES_INDEX.md — update, harden, make useful |
| 573 | PAN-2345 | M | low | ok |  |  | docs: refresh pan-done.md — update, harden, make useful |
| 574 | PAN-2344 | M | low | ok |  |  | docs: refresh KANBAN-MODEL.md — update, harden, make useful |
| 575 | PAN-2343 | M | low | ok |  |  | docs: refresh MISSION-CONTROL.md — update, harden, make useful |
| 576 | PAN-2073 | M | low | ok |  |  | docs: add user-facing page for the Desktop App |
| 577 | PAN-2072 | M | low | ok |  |  | docs: add user-facing page for Beads (task tracking) |
| 578 | PAN-2071 | M | low | ok |  |  | docs: add user-facing page for the Hooks system |
| 579 | PAN-2070 | M | low | ok |  |  | docs: add user-facing page for the Flywheel orchestrator |
| 580 | PAN-2068 | M | low | ok |  |  | docs: add user-facing page for Caveman (agent output compression) |
| 581 | PAN-2067 | M | low | ok |  |  | docs: add user-facing page for RTK (Bash output compression) |
| 582 | PAN-1685 | M | low | ok |  |  | Show model capability icons in conversation dialogs + complete per-model vision (supportsImages) audit |
| 583 | PAN-591 | M | low | ok |  |  | Integrate Karpathy LLM guidelines into all Panopticon CLAUDE.md templates |
| 584 | PAN-663 | M | low | ok |  |  | Workspace frontend containers not auto-started for panopticon-cli self-hosted workspaces |
| 585 | PAN-589 | M | low | ok |  |  | Review and update commands-skills.md with all available Panopticon skills |
| 586 | PAN-407 | M | low | ok |  |  | Run Panopticon from a main workspace for development isolation |
| 587 | PAN-853 | L | low | ok |  |  | Evaluate terminal-bench@2.0 custom agent harnesses for Panopticon integration |
| 588 | PAN-791 | M | low | ok |  |  | Skill mapping: Deft Directive v0.20.0-rc.3 ↔ Panopticon CLI |
| 589 | PAN-793 | M | low | ok |  |  | Borrow Deft's explicit scope-lifecycle transitions for Panopticon agent state machine |
| 590 | PAN-743 | XS | low | ok |  |  | Add consistent new conversation icon actions in Command Deck |
| 591 | PAN-1483 | M | low | ok |  |  | Distinguish general-use skills from Panopticon-only dev skills in pan sync |
| 592 | PAN-1152 | M | low | ok |  |  | Remove PANOPTICON_DEV env-var persistence — derive Traefik mode from the running command |
| 593 | PAN-1984 | M | low | needs-refinement |  |  | Migrate or delete the 18 dead panopticon.db modules referenced by ~30 test files (#1983 follow-up) |
| 594 | PAN-1983 | L | low | ok |  |  | Remove all panopticon.db-supporting code (legacy SQLite layer + db↔db migration + seed-from-legacy) |
| 595 | PAN-2533 | M | low | ok |  |  | UAT workspace magic-link login 502: Traefik picks unreachable panopticon IP for multi-homed fe/api |
| 596 | PAN-2428 | S | low | ok |  |  | bug(workspace): MYN workspace Traefik routing broken post-rebrand — legacy 'panopticon' network + missing traefik.docker.network la... |
| 597 | PAN-674 | M | low | ok |  |  | docs: add glossary of Panopticon domain terms |
...
```json
{
  "version": 1,
  "project": "overdeck",
  "generatedAt": "2026-07-12T22:32:37.933Z",
  "model": "glm-5.2",
  "pass": "incremental",
  "openCount": 599,
  "nodes": [
