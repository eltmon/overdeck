...
<ThemedImage
  light="/images/dashboard/resources-light.png"
  dark="/images/dashboard/resources-dark.png"
  alt="Resources page showing system RAM usage broken down by agents, conversations, and processes"
/>
...
<ThemedImage
  ...
  alt="Awaiting Merge page showing feature branches from completed workspaces ready for human merge"
/>
...
- **vBRIEF tasks** - Trackable plan items with dependencies for the implementation
- **Feature PRD** - Copied to `docs/prds/active/{issue}-plan.md` for documentation

**What works:**
1. **Context recovery** - If an agent's context compacts, the vBRIEF and durable issue record preserve the planning decisions
2. **Audit trail** - See decisions made in STATE.md
3. **vBRIEF tasks** - Plan items and their status overlays are persisted through the canonical task writer
