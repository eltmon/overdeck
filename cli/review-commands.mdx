...
<Note>
  The old convoy command no longer exists, and neither does the `run`
  subcommand of `pan review`. The parallel reviewers that the convoy used to
  launch are now the `full` review mode (see [Review modes](#review-modes)).
</Note>
