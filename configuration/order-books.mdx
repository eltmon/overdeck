...
Both start commands open the one `conv-flywheel` conversation with the book named in its `/pan-flywheel` prompt (`pan flywheel start --orders <id>` is the direct form). A flywheel that is already running keeps running; a paused one must be resumed (`pan flywheel resume`) or replaced (`pan flywheel start --fresh`).
