...
Managed Codex sessions use an isolated `CODEX_HOME`. Overdeck passes its
rendered context as Codex developer instructions instead of creating an
`AGENTS.md` copy; user-authored native files remain under Codex's discovery.
...
- Proxied models use a dedicated Claude Code context policy rather than copying
  ...
  Terra, and Luna default to a `272000`-token pin for both
  ...
  `gpt-6-astra` has no `[372k]` variant — the 372K pin was measured on
  `gpt-5.6-sol` and has not been re-measured for astra.
...
- First-party OpenAI agent loop for the GPT model family (GPT-6 Astra,
  GPT-5.6 Sol/Terra/Luna, GPT-5.5, GPT-5.4, GPT-5.3-Codex, …) with
...
- Native `AGENTS.md` remains user-owned. Overdeck supplies its own global and
  workspace context through the launch/app-server developer-instructions field.
...
cost parsing (`src/lib/cost-parsers/kimi-parser.ts`). Because the native CLI
does not expose a launch-only system-prompt flag, Overdeck sends its managed
launch context once as the first user message after that session id is
captured. Hard delimiters put managed context first and preserve the original
operator task afterward. A per-session receipt under
`~/.overdeck/agents/<agentId>/` prevents resume, recovery, and follow-up paths
from sending the context again.
...
### Managed harness state is private to Overdeck

Local managed launches use per-launch configuration snapshots under
`~/.overdeck/agents/<id>/` and shared refresh-safe credentials under
`~/.overdeck/credentials/`. Native user homes (`~/.claude`, `~/.codex`,
`~/.agents`, `~/.omp`, and `~/.pi`) are read-only import/fallback sources during
ordinary install, sync, spawn, resume, and recovery. Repository instruction
files remain in place and are discovered by the harness; Overdeck's own layers
travel through launch flags/developer instructions and are recorded in the
agent's `context-receipt.json`.

The one exception is remote Fly VM provisioning. Those VM user homes are
created and owned by Overdeck and are used only through Overdeck-managed remote
sessions, so provisioning may populate harness-native paths inside that remote
VM. This exception does not apply to the operator's host or project repos.

The native Kimi CLI exposes no supported launch-only system-prompt flag.
Overdeck therefore uses the explicit first-user-message envelope described
above; it never mirrors managed context into `AGENTS.md` or another native
instruction file. If the session id or required context artifact cannot be
resolved, the managed delivery fails closed instead of starting context-free.
