...
| `opencode` | [OpenCode](https://opencode.ai) (`opencode`) | OpenCode Go and Zen | Persistent ACP sessions, native credentials, and model discovery; verified with 1.18.29. |
...
recorded session file is readable and the reopened session reports the same
session id. A missing file or an id mismatch stops the process and reports a
recovery error rather than silently starting a fresh session. Prime schedules, heartbeats,
...
The integration was verified against OpenCode **1.18.29**. Install or update the
