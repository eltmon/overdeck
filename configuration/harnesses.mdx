...
- Proxied models use a dedicated Claude Code context policy rather than copying
  ...
  The GPT-6 models (`gpt-6-astra`, `gpt-6-sol`, `gpt-6-luna`) have no `[372k]`
  variant — the 372K pin was measured on `gpt-5.6-sol` and has not been
...
- `gpt-5.6-sol` is the default OpenAI model surfaced by this harness. `gpt-6-astra`
  ...
  GPT-5.6 Sol and Luna at half their API price; they are opt-in too. The Codex
  CLI lists them in its model catalog from `0.156.1`; older CLIs forward the id
  to the backend unchanged. Early rollout reports show `gpt-6-luna` refused on
  some ChatGPT sign-in accounts
