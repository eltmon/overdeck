...
<Info>
**Supported today:** `claude-code` (default), `ohmypi`, `codex`, `acp`, `kimi-code`, `opencode`, and `muse`. For the wider
...
| --- | --- | --- |
| Visibility | In the Agents Directory, under its parent and its issue | None, unless something registers it with Overdeck |
| Issue link | Always linked to an issue | None |
| Transcript | On the agent transcript route, like any agent | Only in the plugin's own files |
...
| Sandbox | Git writes blocked with `--read-only`; no file-system or network sandbox | The plugin's file-system and network sandbox |
