...
| `kimi-code` | [Kimi Code CLI](https://www.kimi.com/code/docs/en/kimi-code-cli/guides/getting-started.html) (`kimi`) | Kimi-only work (opt-in) | Drives Kimi's own native terminal UI directly — no ACP host, no Anthropic-compatibility endpoint. Kimi models only. |
...
endpoint (`ANTHROPIC_BASE_URL` + `KIMI_API_KEY`, still supported and still the
built-in default — see [When to pick which](#when-to-pick-which)): that route
...
Opt a Kimi provider into the native harness in `~/.overdeck/config.yaml`:
...
Kimi remains on its built-in `claude-code` default until you set this — the
native harness is opt-in while its end-to-end path is proven out, the same
posture `acp` shipped under.
