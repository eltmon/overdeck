...
- When Claude Code launches a non-Anthropic model, Overdeck also exports
  `CLAUDE_CODE_AUTO_COMPACT_WINDOW` from the model registry's context window
  (for example, Kimi K2.7 Code uses `262144`) so Claude Code compacts before the
  proxied provider's real token limit is exceeded. Anthropic models keep Claude
  Code's native default.
...
- First-party OpenAI agent loop for the GPT model family (GPT-5.5, GPT-5.4,
  GPT-5.3-Codex, …) with ChatGPT-subscription or API-key auth.
...
- **Harness list:** when the current model is blocked for a harness, the harness
  option shows the reason inline and, if clicked, auto-flips the model to the
  first compatible model instead of switching to the blocked combination.
...
conversation panel, and Settings per-role harness selectors.
