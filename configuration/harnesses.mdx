...
| `storage/codex.ts` | `$CODEX_HOME/sessions/**/rollout-*.jsonl` and the per-conversation `CODEX_HOME` (`codexHome`, `codexSessionsRoot`, `findRolloutPath`) |
...
at the top of `scripts/lint-harness-storage.sh` with a reason.
