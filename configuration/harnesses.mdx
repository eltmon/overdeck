...
- Adds RPC mode (`pi --mode rpc`) so Overdeck can write structured commands to
  Pi's stdin via a named pipe (`mkfifo` at `~/.overdeck/agents/<agentId>/rpc.in`).
...
After installing Pi, run `pan sync` once. Overdeck writes
...
Overdeck bridges configured API keys into Pi's environment at launch time,
...
2. Tail the tmux session: `tmux -L overdeck attach -t agent-<id>`. Pi prints
...
`~/.overdeck/heartbeats/<id>.json` — Pi writes there on every
`tool_execution_end`.
...
Pi's harness uses a per-agent `mkfifo` (`~/.overdeck/agents/<id>/rpc.in`) for
