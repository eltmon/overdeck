...
- **Ledger** — the classic dense monitoring style: DM Sans throughout, a flat medium-weight cap on every piece of text, panel-first layout. Ledger is now the **frozen legacy theme** — it keeps working exactly as it does today and continues to receive bug fixes, but it does not gain new visual patterns going forward.
...
New dashboard surfaces are written against the Broadsheet vocabulary directly — no per-component "if the theme is X" logic. Every Broadsheet-only style resolves to a matching, already-shipped Ledger style when Broadsheet isn't active, so a surface built today renders correctly under either theme without extra work: it just looks a little denser and smaller under Ledger. See `design/style-guide/STYLE-GUIDE.md` for the full component-level specification.
