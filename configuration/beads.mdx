...
Use `pan beads list [ISSUE]`, `pan beads ready [ISSUE]`, and
`pan beads show <BEAD>` for reads. Use
`pan beads claim|update|close|create|dep|delete` for mutations. Direct `bd`
commands are rejected in agent environments, so every read reports resolver
failures as stale instead of misreading them as an empty result.
