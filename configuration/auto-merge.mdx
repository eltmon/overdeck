...
auto-merge — Panopticon merging one eligible PR at a time without a human click.
The separate `merge_train.enabled` flag enables [UAT batch
trains](/features/merge-batches), where ready features are assembled into one
tested batch you promote together per project. They are independent flags:
auto-merge ships individual PRs on a cooldown; batch trains
assemble-and-promote a tested bundle.
