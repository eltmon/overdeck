...
each project whose merge train is on, takes the PRs the forge reports as
approved, green, and mergeable, and schedules each one that passes the merge
gate (`evaluateIssueMergeGate`) and is opted in to auto-merge (its `auto-merge`
/ `hold-for-uat` label, then the project default, then this toggle). It
schedules through the same door as `POST /api/merge-train/auto-merge/schedule`.

The scheduler schedules each issue once. It leaves alone an issue whose latest
auto-merge is still pending or merging, or was cancelled, blocked, or failed:
a cancel sticks, and a blocked or failed entry waits for the operator, who can
re-schedule it through the endpoint. Only a merged entry makes room for a new
one.
