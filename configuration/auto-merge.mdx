...
1. the issue is opted in to auto-merge: its `auto-merge` / `hold-for-uat`
   label, then the project default, then this toggle (no forge read);
2. the merge gate (`evaluateIssueMergeGate`) says the PR is ready. Approval is
   a forge review or a trusted `overdeck-verdict` marker comment, so a repo
   without branch protection still qualifies;
...
is left alone. A cancelled entry, or a failed one (the merge was attempted,
including the retry circuit breaker), holds for that PR head only: a new push
re-arms the issue. A blocked entry (the executor refused it before merging,
for example because checks went red or the head moved during the cooldown)
re-arms as soon as the merge gate passes again. The insert re-checks the latest
entry, so a cancel that lands while the scheduler is reading the forge sticks.
