...
2. the merge gate (`evaluateIssueMergeGate`) says the PR is ready. Approval is
   ...
   as. A review from any other account never counts, and each reviewer's
...
- a PR that needs a rebase is rebased by the server only, and only when the
  ...
  rebases exactly that commit, pushes the result by sha with a lease on the
  approved head, and pins the merge to the result. It never hands the rebase
  to the work agent, whose push would merge unreviewed;
...
- The PR is approved on its exact head commit (a trusted reviewer's standing
  ...
  it; on GitLab, a named approver in `approved_by`), and no review or marker
  requests changes
