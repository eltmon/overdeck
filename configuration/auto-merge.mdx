...
2. the merge gate (`evaluateIssueMergeGate`) says the PR is ready. Approval is
   a GitHub review approving the exact PR head commit, or a trusted
   `overdeck-verdict` marker comment whose `sha=` is the PR head, so a repo
   without branch protection still qualifies. An approval of an older commit,
   or a marker without `sha=`, never does;
...
- The PR is approved on its exact head commit (a GitHub review of that commit,
  or a verdict marker whose `sha=` names it; a GitLab MR's own approval), and
  its latest review does not request changes
