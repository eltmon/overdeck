...
The clone detects the GitHub/GitLab remote, default branch, and proposes an issue prefix based on the repository name. It then creates the main workspace, allowing you to start working immediately.
...
- `github_repo` / `gitlab_repo` — the remote URL and slug
- `tracker` — the issue tracker type (GitHub, GitLab, or Linear when configured)
- `workspace.default_branch` — the repository's default branch
- `issue_prefix` — automatically proposed (first 10 characters of the repo name, or customizable)
- Main workspace creation — the initial workspace is created automatically

### Dashboard

Register projects through the dashboard at `/projects/new`:

- **Clone repository** — Clone from GitHub/GitLab or any git remote
- **Add existing** — Register a local repository
- **New project** — Initialize a new repository in Overdeck

All three options create the main workspace and detect configuration automatically.
