...
`false` omits the rule from every rendered surface — the managed regions, the
Pi/Codex render artifacts, and the dashboard Context-page preview. Any absent
key (or `true`) leaves the rule on. The map is read at render time, so after a
change, run `pan sync`; the new render reaches new sessions only.
