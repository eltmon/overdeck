...
- Workspace branch metadata is cached until `.git/HEAD` changes, branch inventory uses two bulk git reads per project, closed spec-only issues skip per-number state lookups, and merged-PR history is queried only for open issues. Periodic convergence therefore does not respawn git/GraphQL work for unchanged historical workspaces.
