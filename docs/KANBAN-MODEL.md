...
8. **Clear operator-gate residue** — Clears `stoppedByUser`, `paused` and `troubled` on the issue's agent rows so they stop reappearing as parked, and resets the process-local post-merge guard. (Non-blocking.) There is no review-status row to clear: the closed, labelled tracker issue is the terminal marker, and review verdicts are PR reviews.
