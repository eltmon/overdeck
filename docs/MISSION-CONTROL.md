...
State labels are computed from multiple signals: tmux session presence, heartbeat freshness (10-minute threshold), review/test status from SQLite (`review_status` in `panopticon.db`), and the `agents` table in `panopticon.db`. Agent state files are no longer read for runtime status; they are kept only as a rollback/rebuild source.
