...
`runtimes/muse.ts` and `runtimes/kimi-code.ts` still hardcode `useSupervisor: true` (their
`spawnAgent` moves to `launchAgentPane` in PAN-3936), but that is no longer a launch failure: those harnesses are launched
