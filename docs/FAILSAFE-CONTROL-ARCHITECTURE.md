...
1. ~~**Default grace length** (proposed 30s) and whether the default expiry action is **Resume** (recommended) or **Hold**.~~ **Resolved:** grace defaults to 120s (`DEFAULT_BOOT_RECONCILIATION_GRACE_SECS`). [PAN-3184](https://github.com/eltmon/overdeck/issues/3184) classifies expiry from live state: an empty candidate set commits `resume_all` because a vacuous hold must never latch; a non-empty set commits `resume_all` after a clean shutdown recorded by `dashboard.last_clean_shutdown_at` within `CLEAN_SHUTDOWN_FRESHNESS_MS` (30 minutes), and commits `hold_all` when the marker is absent or stale, which identifies a crash or unknown boot. PAN-3052 still governs the operator copy: a surface must never promise auto-resume for a crash or unknown boot.
