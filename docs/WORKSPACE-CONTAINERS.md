...
- **Review status reaches the live UI, not just the REST endpoint.** `upsertReviewStatusSync()` is a raw DB upsert; on its own it never appends the `review.status_changed` event the dashboard's in-memory read model (and from there the frontend Issue store) needs to learn `prUrl` exists. The container's own reconciler (`review-status-reconcile-service.ts`) can't backstop this — it only runs in `primary` dashboard mode, and workspace containers always run `peer` (`OVERDECK_DISABLE_DEACON=1`). `seedUatFixturesLocal()` appends the event itself, the same way the reconciler does.
- **Every seeded agent row carries its `branch`.** `agents.branch` is a real column (`drizzle/overdeck/0000_overdeck_init.sql`) that `AGENT_COLUMNS_FOR_DB` (`src/lib/overdeck/agent-state-sync.ts`) never wired into its SELECT/INSERT column list before PAN-3362's UAT cycle 2 fix — `GET /api/agents` reported `branch: null`/`git: null` for every agent, fixture or real, regardless of what was seeded or set. That fix is general, not fixture-specific.
