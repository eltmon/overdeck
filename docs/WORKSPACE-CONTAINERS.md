...
- **Every seeded agent row carries its `branch`, end to end.** `agents.branch` is a real column (`drizzle/overdeck/0000_overdeck_init.sql`) that `AGENT_COLUMNS_FOR_DB` (`src/lib/overdeck/agent-state-sync.ts`) never wired into its SELECT/INSERT column list — dropped for every agent, fixture or real. `GET /api/workspaces/:issueId` also falls back to a persisted work-agent branch (`getPersistedBranchFallback()`) when there's no real `.git` checkout to shell `git` against, surfaced by the drawer's Workspace section.
