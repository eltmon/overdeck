...
| Review synthesis | Combines lane findings into the verdict (`full` mode). The review parent writes it, so it runs on the `review` role's model | `src/lib/cloister/review-agent.ts:569` → `src/lib/agents/provider-env.ts:311` | `claude-opus-4-8` | `roles.review.model`. `roles.review.sub.synthesis` is retired (#4131): config load drops it (`RETIRED_SUB_ROLES` in `src/lib/config-yaml/roles.ts`) and the Roles panel no longer shows it | session cost parser tags by harness/model | Yes (Roles panel, as `review`) |
...
| Cloister handoff | Respawns an agent on a new model when a handoff trigger fires (`checkHandoffTriggers`) | `src/lib/cloister/service-health.ts:281` → `src/lib/agents/provider-env.ts:311` | The trigger's `suggestedModel`; with none (a `test_failure` trigger without `to_model`, a `task_complete` trigger without `to_specialist`), the agent's role model | The trigger config in `cloister.toml`, else `roles.<role>.model` (PAN-4160; previously a `sonnet` literal). When routing cannot resolve, the handoff is skipped and a failed `handoff_completed` event carries the error | session cost parser tags by harness/model | Yes (Roles panel, for the role model) |

**Claude CLI model flag:** `getClaudeModelFlag()` (`src/lib/settings.ts:290`) maps known `claude-*` IDs to the `claude --model` value. An ID missing from its table passes through unchanged; before PAN-4160 it silently launched as `sonnet`.
...
Operator-initiated planning through `pan plan`, dashboard planning, or `pan start` auto-planning uses the ordinary `roles.plan.model` resolution above. When the dashboard relaunches a finished planner to deliver a message (`POST /api/planning/:issueId/message`, `src/dashboard/server/routes/misc/planning.ts`) and that resolution throws, the route returns a 500 "No default model configured for the plan role" before it touches anything; it no longer falls back to a literal `claude-sonnet-5` (PAN-4160). A fresh autonomous planning dispatch from the reactive lifecycle scheduler uses a separate, fail-closed chain: the model recorded on the current plan agent, then the legacy planning agent, then the optional scalar `roles.plan.autonomousModel`. If none resolves, the scheduler refuses the spawn and records a needs-you trip; it never falls through to `roles.plan.model` or its `workhorse:expensive` default.
...
| Provider-disabled substitute | When an enrichment model's provider is disabled, `applyFallback()` swaps in an Anthropic equivalent from `FALLBACK_MAP`; a model with no entry there (e.g. any OpenRouter `org/model`) gets this configured model | `src/lib/config-yaml/defaults.ts:61` → `src/lib/model-fallback.ts:465` (read in `src/lib/conversations/enrichment/enrich-session.ts:381`) | `claude-sonnet-5` | `models.provider_fallback_model`. Previously a private `DEFAULT_FALLBACK` literal in `model-fallback.ts` (#4160) | Recorded under the calling feature's tag (`background:conversationEnrichment`) | No |
...
| `summaryFork` / fork summary | One-shot fork summary seeding a summary or handoff fork | `src/lib/config-yaml/defaults.ts:82` → `src/lib/conversations/summary-fork.ts:514` (the fork route picks the summary harness from the same key, `src/lib/overdeck/conversation-forks.ts:823`) | `claude-sonnet-5` | The request's `summaryModel`, else `conversations.fork_summary_model` (PAN-4160; replaced two disagreeing literals, `claude-sonnet-5` and `claude-sonnet-4-6`). An empty value fails the LLM summary with a "No fork summary model configured" error, and the fork falls back to the heuristic summary | `background:summaryFork` (when JSON envelope has usage) | Yes (Background AI) |
| Command Deck status review | Analyzes an issue's PRD, discussions, transcripts and diff into a status review (`POST /api/command-deck/planning/:issueId/status-review`) | `src/lib/config-yaml/defaults.ts:56` → `src/lib/status-review-model.ts` → `src/dashboard/server/routes/command-deck.ts:969` | `claude-sonnet-5` | `models.status_review_model` (PAN-4160; previously a `claude-sonnet-5` literal behind a dead `models.overrides['status-review']` lookup, see #4131). An empty value fails the request with "No status review model configured" | **None** | No |
...
| `summaryFork` fork summary (no override) | `claude-sonnet-5` | `$9.00` | `generateSummaryForFork()` uses `conversations.fork_summary_model` when no model is passed; the chunked smart-compaction default is Haiku. The one-shot summary serializes the whole conversation, so the model needs a 1M context | Set `conversations.fork_summary_model` to a cheaper 1M-context model if one is acceptable |
