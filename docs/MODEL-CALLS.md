...
| `strike` role | Precision fix opened as a PR against main (skips review pipeline) | `src/lib/config-yaml.ts:413` → `src/lib/config-yaml.ts:381` | `claude-opus-4-8` | `roles.strike.model` (defaults to `workhorse:expensive`) | session cost parser tags by harness/model | Yes (Roles panel) |
