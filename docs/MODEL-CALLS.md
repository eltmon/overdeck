...
|---|---|---|---|---|---|---|
| `plan` role | Planning agent (xBRIEF/PRD authoring) | `src/lib/config-yaml.ts:369` → `src/lib/config-yaml.ts:381` | `claude-opus-4-8` | `roles.plan.model` (defaults to `workhorse:expensive`) | session cost parser tags by harness/model | Yes (Roles panel) |
...
| `review.requirements` sub-role | Acceptance-criteria / xBRIEF coverage | `src/lib/config-yaml.ts:404` → `src/lib/config-yaml.ts:381` | `claude-sonnet-4-6` | `roles.review.sub.requirements.model` (defaults to `workhorse:mid`) | session cost parser tags by harness/model | Yes (Roles panel) |
...
| `summaryFork` / handoff author | External handoff document author | `src/lib/conversations/summary-fork.ts:214` | `claude-sonnet-4-6` | `options.handoffAuthorModel`, else `conversations.handoff_author_model` (PAN-3860; was an unconfigurable literal) | `background:summaryFork` (when JSON envelope has usage) | Yes (Background AI) |
...
|---|---|---|---|---|
| `summaryFork` handoff author | `claude-sonnet-4-6` | `$9.00` | Background handoff-authoring job defaults to Sonnet; the similar compaction task uses Haiku (`$4.00`). The default is unchanged by PAN-3860 — only its plumbing moved off a private literal — so this suggestion still stands | Set `conversations.handoff_author_model: claude-haiku-4-5` if the cheaper model produces acceptable handoff docs |
