...
| `Notification` | When Claude shows a notification | `notification-hook` | `agent.waiting_started` |
| `PreCompact` | Before context compaction begins | `pre-compact-hook` | `agent.activity_changed` (working/compact) |
...
Three hooks live in each agent definition's YAML frontmatter:
...
| `pre-compact-hook` | PreCompact | Emits `activity: working` (compact indicator) |
...
To bridge the partial equivalents into the domain event system, the Pi extension writes an `events.jsonl` file alongside `heartbeat.json`. A background dashboard poller consumes these events and appends them to the event store, giving Pi agents basic activity tracking (`activity_changed`, `model_set`) even though the six gaps remain.
