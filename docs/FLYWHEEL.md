...
| In-flight issues | The union of the project's `workspaces/feature-*` directories (slot workspaces excluded) and the ids the loop named in its newest tick, minus anything merged or closed. Their derived issue state comes from `loadIssueStatesForProject`, the journal line from the last entry of each `<workspace>/.overdeck/pipeline.jsonl`. **Closed issues drop because the deriver now passes real tracker rows in:** on the dashboard from the `IssueDataService` cache, in the CLI from `readIssueFromTracker`, four issues at a time. Without them every issue read as `open: null` — unknown — and a closed issue whose feature branch lingered derived `working` forever. | `deriveFlywheelStatus()` |
| Title | The tracker row's title, from the same read. | `deriveFlywheelStatus()` |
| Tracker unknown | No configured tracker answered for the issue, so its state is a guess. A neutral `tracker unknown` badge on the page; `(tracker unknown)` in place of the title in the CLI. | `deriveFlywheelStatus()` |
| No agent | The row is `working` and `liveAgents` is 0 — nothing is driving it. The pane inventory is read once per status and shared with `loadIssueStatesForProject`. | `deriveFlywheelStatus()`, `listPanesWithBackend()` |
| Loop in-flight | `inTick`: the loop named this id in its newest tick marker. The header counts these (`N in flight (loop)`) when `inFlightSource` is `tick`, and falls back to `N feature workspaces` when there is no conversation or no tick. | `deriveFlywheelStatus()` |
| Live agents | `status.agents`: every live pane (not `exited`, not `done`) of an in-flight issue, by issue then role, with its role, harness, model, and pane state. | `deriveFlywheelStatus()` |
| Headline strip | PRs merged in the last 30 days and bugs filed in that window that have closed, both from `GET /api/flywheel/stats` (shared with the Stats tab); the running book's landed/total; features queued on the merge train. A failed or malformed read shows `—`, never a zero. | `FlywheelHeadlineStrip` |
| UAT batches | The merge train's queues and generations, `GET /api/merge-train/queues` and `/generations`, as a rail card over `MergeTrainView`. | `FlywheelUatBatchesCard` |
| Report tab | `<planHome>/.pan/flywheel/report.md`, the same file the CLI prints. | `FlywheelReportPane` |
| Needs-you indicator | The app-header count: the loop's tick `needs-you` line plus every in-flight row with `attention: needs-you`. Hidden at zero. | `NeedsYouIndicator` |
| Elapsed | `running <duration>` in the page header, from the flywheel conversation's `createdAt`. | `formatDuration()` |
...
| --- | --- | --- | --- |
| `pan flywheel start [--orders <id>] [--fresh] [--model] [--harness] [--cwd]` | `POST /api/flywheel/start` `{ model?, harness?, orders?, fresh? }` | Start, Start fresh | `startFlywheel`: creates `conv-flywheel` and sends `/pan-flywheel [book]`. Refuses a running flywheel (409). Refuses a paused one unless `--fresh` (409): `paused flywheel exists — pan flywheel resume to continue, pan flywheel start --fresh to start over`. A start that fails after writing the conversation row archives that row and stops any half-started session, so the next start is not refused as paused (an archived row reads as `idle`). A start refused with `FlywheelOrphanSession` (409) reveals **Start fresh** in the idle toolbar; its confirm names the leftover session and it posts `{ fresh: true }`. |
...
| `pan flywheel resume` | `POST /api/flywheel/resume` | Resume | `resumeFlywheel`: `handleConversationResume` without the resume contract, then sends `/pan-flywheel` again — unless the harness was still alive and the handler only re-attached, in which case nothing is sent. |
| `pan flywheel abort` | `POST /api/flywheel/abort` | Abort (confirm) | `abortFlywheel`: pause without a report. |
| `pan flywheel report` | `POST /api/flywheel/report`, `GET /api/flywheel/report` | Report, **Report tab** | `requestFlywheelReport` when the flywheel is running. Otherwise the CLI prints `report.md` or "No report yet." The Report tab renders the same file as markdown. |
| `pan flywheel stop [--timeout <ms>]` | `POST /api/flywheel/stop` `{ timeoutMs? }` → 202 | Stop (confirm) | `stopFlywheel`: sends the stop request, then checks the transcript every 5 seconds for a `phase=stopping` tick newer than the request (the skill prints it only after committing and pushing the report) or until the time limit (120 seconds by default) passes, then pauses either way. Returns `{ stopped, reportWritten }`; `reportWritten` is whether `report.md` in the project root's plan home changed after the request. |
...
The elapsed time counts from the flywheel conversation's creation, not from the loop's first tick, so a conversation that sat paused and was resumed reads as one long run. A tick-only id — one the loop named but that has no `workspaces/feature-*` directory — is derived against the flywheel's own project, so an id belonging to another project derives as `backlog` with no branch or PR rather than being resolved elsewhere. The headline strip's "bugs fixed" counts only bugs *filed* inside the window that have since closed; one filed earlier and closed inside it is not counted.
