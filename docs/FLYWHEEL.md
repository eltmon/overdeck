...
The planning consent flag is current-cycle and one-shot: every planning launch replaces the previous value, an explicit `autoSpawn: false` invalidates it, and a successful direct, rebuild, or orphan-recovery handoff consumes it. Merely queueing container startup does not consume consent; the delayed work launch owns that transition after `pan start` is accepted. If the primary cleanup write fails after a successful launch, a durable spent marker makes the release source fail closed until the next planning launch replaces it. The flag does not waive readiness or any blocker. If tracker labels are unavailable, the predicate refuses the dispatch. Refusals create a durable needs-you trip; reactive dispatch also emits one warning, while the patrol-based orphan reconciler persists only a changed refusal reason instead of writing the same trip on every patrol.
