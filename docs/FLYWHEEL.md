...
| --- | --- | --- | --- |
| `pan flywheel start [--orders <id>] [--fresh] [--model] [--harness] [--cwd]` | `POST /api/flywheel/start` `{ model?, harness?, orders?, fresh? }` | Start, Start fresh | `startFlywheel`: creates `conv-flywheel` and sends `/pan-flywheel [book]`. Refuses a running flywheel (409). Refuses a paused one unless `--fresh` (409): `paused flywheel exists — pan flywheel resume to continue, pan flywheel start --fresh to start over`. A start that fails after writing the conversation row archives that row and stops any half-started session, so the next start is not refused as paused (an archived row reads as `idle`). |
