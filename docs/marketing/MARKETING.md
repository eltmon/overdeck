...
| Tweet bio | *Command Deck: spawn agents, watch them work, stay in control. `npx @panctl/cli`* |
...
Google, Kimi, MiniMax, and OpenRouter. Get started with `npx @panctl/cli`.
...
**Calls to action**
- `npx @panctl/cli` — no install step.
...
**X / Twitter (≤280)**
- IDEs were built for people who type code. Panopticon is built for people who *direct* it. Spawn agents, watch every diff land live, hot-swap models mid-task, then let the pipeline review + test + merge. `npx @panctl/cli` ⌁ open source.
...
**Q: How do I start?**
`npx @panctl/cli`. No install step — it starts Command Deck and opens the dashboard in your
browser.
...
- **npm:** https://www.npmjs.com/package/@panctl/cli
- **GitHub:** https://github.com/eltmon/panopticon-cli
- **Install:** `npx @panctl/cli`
- **Desktop app:** `@panctl/desktop`
