...
`.pan/` lives in the project repo (or the configured plan-home repo for polyrepo projects), committed by whoever writes it. Per-issue artifacts (specs, continues, drafts) are committed on the feature branch. Project-level artifacts (orders, notes, the backlog sequence) are committed on `main` in the plan home, and `pan backlog write-sequence` and the `pan orders` write verbs also push their commit so local `main` does not drift ahead of origin (PAN-3923, #4108). When origin has moved, the push replays only the `.pan/` commits onto it and pushes. It never forces, and it warns instead of pushing when local `main` holds unpushed commits outside `.pan/`. The push also skips any local commit already landed on origin under a different sha (a patch-equivalence check), so it never creates an empty duplicate commit; when every local commit has already landed this way, it moves local `main` onto origin instead of pushing. If moving onto the pushed or landed tip would newly track a path that already sits untracked in the working tree — origin gained a `.pan/` file through a merge the checkout never staged — that file is cleared out of the way: deleted if its bytes match what's now tracked, or backed up under `.overdeck/plan-artifact-backups/<stamp>/` and reported if they differ (PAN-4224). Either kind of push problem — a warning, a failed push, or a backed-up file — is also surfaced as a `plan-artifacts` warning in the dashboard activity feed, not just printed to the CLI's stderr. Complete-planning commits the PRD draft on the issue's own feature branch in the workspace's plan home, not the primary checkout, so the PRD travels with the issue's own commits instead of landing as an untracked file elsewhere.
