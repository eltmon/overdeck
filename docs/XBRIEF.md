...
1. **PRD drafted** — a human writes a markdown PRD to `drafts/` on `overdeck-state`, or a planning agent authors one at `<workspace>/.pan/drafts/<ISSUE>.md`. `pan plan finalize` enforces the PRD's existence (PRD-first gate, PAN-2234) and complete-planning promotes a workspace-authored draft to `drafts/` on `overdeck-state` through the draft write door (`promoteWorkspacePrdDraft()`), never overwriting an existing canonical draft
2. **Planning completes** — planning agent converts the PRD into a machine-readable xBRIEF spec in `specs/` on `overdeck-state` with `status: "proposed"`
