...
| Agent enforcement | `src/lib/agents/spawn.ts`; `src/lib/beads/bd-shim.ts`; `src/lib/cloister/prompts/work.md`; `src/lib/cloister/verification-runner.ts`; bundled beads skill entrypoints | Work agents use `pan beads list/ready/show` for reads and `pan beads claim/update/close/create/dep/delete` for mutations; direct `bd` access is not an agent surface. |
