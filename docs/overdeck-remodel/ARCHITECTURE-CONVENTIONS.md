...
import { drizzle } from "drizzle-orm/node-sqlite"      // cache = Drizzle on Node (NOT @effect/sql — Bun-only). Bun uses drizzle-orm/bun-sqlite. NOT an ABI-locked native addon (PAN-1579).
...
dashboard is **Node-22-only**. The cache is **Drizzle ORM over the runtime-bundled
`node:sqlite` (Node) / `bun:sqlite` (Bun) — the same driver the codebase
standardized on in PAN-1579 when it dropped the ABI-locked native SQLite driver**,
wrapped behind one Effect `Db` service. **Dependency gate (hard acceptance gate,
not a TODO):** **neither** `drizzle-orm` **nor** `drizzle-kit` is a root
`package.json` dependency today (verified: root `package.json` has
`effect@4.0.0-beta.73` but no drizzle/drizzle-kit entry) — **both must be added**
as the first cutover step. We add **only** `drizzle-orm` + `drizzle-kit`: there is
**no native SQLite addon dependency** (PAN-1579), because `node:sqlite` is built
into Node 22. A compile smoke-test (a tiny `sqliteTable` + `.references()` + a partial
`uniqueIndex(...).where(sql\`…\`)` + FK enforcement (`PRAGMA foreign_keys=ON`) +
one resolver/writer transaction, run on **`node:sqlite` under Node 22**) must pass
**before any domain code is written**; the smoke-test must **confirm Drizzle's
`node:sqlite` adapter works**. (If a pinned Drizzle version lacks a `node:sqlite`
adapter, wrap the existing `src/lib/database/driver.ts` behind a thin
Drizzle-compatible interface — fallback stated up front, not discovered mid-cutover.) The dashboard is Node-22-only, so the smoke test runs on Node, not Bun. Drizzle
...
export const DbLive = Layer.effect(Db, Effect.sync(() =>
  Db.of({ q: drizzle(new DatabaseSync(dbPath)) })))   // node:sqlite, Node 22

// resolvers/writers use it; node:sqlite (`DatabaseSync`) is synchronous → wrap in Effect.sync:
...
- Transactions use node:sqlite (`DatabaseSync`)'s synchronous `q.transaction(...)` wrapped in
