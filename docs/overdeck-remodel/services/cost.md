...
| `pan cost budget create <name>` (`cost.ts:318`) | writes | **`CostWriter.createBudget(spec)`** | `createBudgetSync` → `~/.overdeck/costs/budgets.json` (`lib/cost.ts:452`, `BUDGETS_FILE` cost.ts:429). A **live** store with a live caller (this CLI) — preserved, not deleted. |
