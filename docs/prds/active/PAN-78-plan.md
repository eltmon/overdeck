...
Integrate [claude-code-router](https://github.com/musistudio/claude-code-router) to enable Overdeck to use multiple AI providers (OpenAI, Google, Z.AI) alongside Anthropic models. Overdeck will fully own and manage the router configuration, providing a settings UI for model selection and API key management.

## Decisions Made

### 1. Configuration Ownership
**Decision**: Overdeck completely owns `~/.claude-code-router/config.json`
- Overdeck generates and overwrites the entire config file
...
~/.claude-code-router/
└── config.json            # Generated by Overdeck (owned completely)
```
...
Overdeck will generate `~/.claude-code-router/config.json` based on:
...
- **Project**: `projects/7429816648963385369` (Overdeck Agent Orchestration Dashboard)
