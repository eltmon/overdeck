...
```
network overdeck was found but has incorrect label com.docker.compose.network set to "" (expected: "overdeck")
```
...
In `src/cli/commands/install.ts`, after the Traefik setup section, check if existing `~/.overdeck/traefik/docker-compose.yml` needs patching:
...
|----|-------|--------|------------|
| overdeck-6a6o | Fix templates/traefik/docker-compose.yml | open | - |
| overdeck-z5oc | Add migration to pan install | open | - |
| overdeck-hm01 | Add auto-fix to pan up | open | - |
| overdeck-mlc0 | Add regression test to e2e-traefik.sh | open | - |
