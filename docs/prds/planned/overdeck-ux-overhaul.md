...
> **Implementation status (2026-07-20, rev 2): BOTH COMPLETE.** C-VERB landed: every assistant turn leads with its digest sentence (`TurnBody` in `messagesTimeline/messageRows.tsx`); bodies beyond 6 rendered lines collapse behind a per-turn '▸ N more lines' toggle that never resets scroll (the virtualizer re-measures the row in place); streaming turns never collapse; completion/failure messages render as structured verdict cards (pass/fail tones with glyphs — verified live on MIN-865: 28 collapsed turns, 71 verdict cards, bare 'blocked' guarded against over-fire); command runs collapse via the existing work-log group (newest-6 + overflow). The §3.9 gate (`messageRows.test.tsx`) pins budgets, verdict tones, group collapse, and no-remount expansion. C-THEME's gate (`tests/theme-conformance.spec.ts`) runs the five key screens × both schemes (10/10 live).
>
> _(Earlier: C-THEME gate live; C-VERB scoped in #2967 — now delivered and closed.)_
