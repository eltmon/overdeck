...
> **Implementation status (2026-07-20): the one component exists — `issue-detail/IssueDetail.tsx` at all three densities.** The drawer's entire anatomy (paused banner, tab strip, phase rail + specialist strip as the per-agent conversation switcher, all tab bodies, the 320px status rail) is extracted into the single component with `density: 'drawer' | 'page' | 'rail'`; `IssueDrawer.tsx` is now only the frame (scrim, header, close) + `IssueDetail` + the shared action bar. `density="page"` (route-width composition, 360px status rail) and `density="rail"` (compact: rail + active conversation + one action strip) are implemented and unit-tested. The replaced `DrawerTabs` component is deleted. **Adoption follow-ups (#2962):** rail density in the deck tree's FeatureItem (its session-children tree + resources groups need a designed integration, not a swap); page density on the cockpit route (the cockpit deliberately uses session-tree → SessionPanel for conversations — test-enshrined — and its Code/Costs/Discussion/Ship/Timeline tabs have no home in the binding tab set; decisions enumerated in the issue). Still open: `useIssueView` as the only data path (both shells currently share the store + `IssueView` boundary; deleting the bespoke per-shell derivations is the remaining deep item, also #2962).
>
