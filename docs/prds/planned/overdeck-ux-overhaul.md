...
> **Implementation status (2026-07-19): conversation-first drawer live.** The drawer now opens on Conversation by default (store-level), renders the shared `PhaseRail` + `SpecialistStrip` under its tabs as always-visible anatomy, and both double as the per-agent conversation switcher — clicking a phase opens that phase's agent, clicking a specialist chip opens *that specialist's* conversation (verified live: chip → `agent-min-865-review-correctness`). The legacy inert `DrawerReviewSpecialists` section is superseded by the strip (component kept for the cockpit's ReviewVerificationCard). Peek derivation hardened (issue-record merge truth). Still open from this contract: the single shell itself (one `IssueDetail` component at three densities — the drawer is now its anatomy in everything but name; the cockpit and rail densities follow), and `useIssueView` as the only data path.
