...
> **Implementation status (2026-07-20, rev 2): the one component exists and the drawer is at full binding parity.** `issue-detail/IssueDetail.tsx` carries `density: 'drawer' | 'page' | 'rail'`; `IssueDrawer.tsx` is only the frame (scrim, header, close) + `IssueDetail` + the shared action bar; the replaced `DrawerTabs` component is deleted. At drawer density every line of the binding spec now holds, verified live: conversation-first by default with the phase rail + specialist strip as the per-agent conversation switcher; the full tab set with the right bodies — **Plan map** opens on the DAG (List/Raw one sub-tab away), **Files** is the changed-files tree + per-file diff (the cockpit Code tab's component), Tasks/Terminal/Activity/Artifacts/Overview real; explicit `?tab=` deep-links win over the conversation-first default; the gray stream-excerpt box and "No recent stream output" are deleted, not re-skinned; and the always-visible right rail carries the status facets — verification gates, resources (container health dots), live activity — with review specialists in the strip above. Page density (route-width, 360px rail) and rail density (compact: rail + active conversation + one action strip) are implemented and unit-tested. **Adoption follow-ups (#2962):** rail density in the deck tree's FeatureItem (its session-children tree + resources groups need a designed integration, not a swap); page density on the cockpit route (the cockpit deliberately uses session-tree → SessionPanel for conversations — test-enshrined — and its Code/Costs/Discussion/Ship/Timeline tabs have no home in the binding tab set; decisions enumerated in the issue). Still open: `useIssueView` as the only data path (both shells currently share the store + `IssueView` boundary; the remaining bespoke derivations — FeatureItem's and useIssueView's session-tree pipeline models — are richer than the enum machine and converge as part of that designed integration, also #2962).
>
