...
> **Implementation status (2026-07-20, rev 3): COMPLETE — all three densities adopted.** `issue-detail/IssueDetail.tsx` (`density: 'drawer' | 'page' | 'rail'`) is the one component for the binding tab set everywhere: the drawer frame mounts it (full binding parity — conversation-first, Plan map DAG-first, Files = changed-files tree, deep-linkable `?tab=`, stream fossil deleted, status facets in the always-visible rail); the **deck tree's FeatureItem mounts `density="rail"`** additively above its session rows (operator decision A); the **cockpit route mounts `density="page"`** with the binding set driven by its nav, Conversation default, its own Overview plus Code/Timeline/Discussion/Costs/Ship appended, and the session-tree lane collapsed behind a toggle (operator decisions 2a/2b/2c). The conformance gate sanctions exactly those three hosts; the shell itself is reachable only through IssueDetail. Remaining fragment (documented, out of scope): `useIssueView` transport parity — the WS snapshot vs enriched HTTP review detail is backend schema work the PRD's non-goals exclude; the frontend's bespoke re-derivations are deleted (cockpit pill runs the shared machine).
>
