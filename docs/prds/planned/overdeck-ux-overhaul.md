...
> **Implementation status (2026-07-20): v3 — simple mode is complete.** The home composer is **Talk it through** (discuss-first seeded conversation, model chosen inline, files only when the operator says it's ready); **Just filed** surfaces new issues one click from planning. The simple issue page mounts the shared real transcript (`DrawerAgentSession` — one renderer everywhere, single jargon-free composer that steers the agent you're viewing). Expectations ("started 2h ago · about 1h to go") render on home rows and the issue page. **Get help** routes to a prefilled tracker new-issue URL. The scripted junior-dev core-loop run passes (3/3 live). The deck's popstate selection race for fresh conversations is fixed (prompt refetch retry instead of waiting out the 10s poll — hard nav no longer required). Nothing remains open from this contract.
>
> _(2026-07-25, [PAN-3090](https://github.com/eltmon/overdeck/issues/3090)): the simple issue page's feed is now a **narrative feed**, not the shared operator transcript — the machine kickoff prompt collapses to a one-line system entry (raw text behind a disclosure), assistant prose clamps with "Read more", tool calls render as quiet one-line actions, and a live row carries the single state color. A blocked agent turns the progress-track node amber with a "waiting on you" caption (no more blue working pulse), and a single-select pending question renders as a rich card quoting the question with its options and an in-card answer composer. Advanced chrome ("Drifted master", "Tools", the one-agent picker) left simple mode. Design reference: [`docs/design/simple-issue-page-mockup.html`](../../design/simple-issue-page-mockup.html)._

> _(2026-07-20 earlier): v2 — the conversation-first hand-off (Talk it through + Just filed)._
...
5. **The issue page in simple mode:** status card (icon + one sentence + expectation), a four-word progress track (Started → Writing code → Checking → Ready) — **and the live conversation as the main content.** Talking to the agent is not a secondary button here; it's the interface. The feed is a narrative (PAN-3090): the machine prompt never renders as content (one-line system entry, raw text behind a disclosure), agent turns render as prose clamped with "Read more", a compact "doing" log (edited file → ran tests → searched), and messages sent to it as neutral rows. The composer ("Say something to the agent… steer it, correct it, ask why") is always visible; steering never interrupts work — the agent reads between tasks. A question from the agent becomes the page's hero card — the question quoted with its options, the answer box inside the card. A "What happened" timeline and an `<Advanced — the machinery behind this task>` disclosure (deep-linking into §3.3) sit below.
...
> **Implementation status (2026-07-20, rev 4): COMPLETE — densities adopted where the operator wants them.** `issue-detail/IssueDetail.tsx` (`density: 'drawer' | 'page' | 'rail'`) is the one component for the binding tab set: the drawer frame mounts it (full binding parity — conversation-first, Plan map DAG-first, Files = changed-files tree, deep-linkable `?tab=`, stream fossil deleted, status facets in the always-visible rail); the **cockpit route mounts `density="page"`** (binding set driven by its nav, Conversation default, its Overview plus Code/Timeline/Discussion/Costs/Ship appended, session-tree lane collapsed behind a toggle). **Deck tree: NO IssueDetail — rejected by the operator after seeing the additive mount live** ("the phase rail is obnoxious here; no conversations in the tree"): the tree keeps its bespoke session rows + context menu, and the conformance gate bans IssueDetail hosts outside the drawer frame and cockpit. Rail density stays implemented/tested (unused by any host today). Related hardening: an agent conversation whose transcript is gone (queued specialist, wiped workspace) now renders the honest "no saved history" state instead of the "Couldn't load history" warning card (404 + agentId → empty payload; real conversations keep the failure card + Retry). Remaining fragment (documented, out of scope): `useIssueView` transport parity — backend schema work the PRD's non-goals exclude.
>
...
> **Implementation status (2026-07-20): v3 — the ≤60 metric is met (26 cards at first render, live-measured).** Backlog rolls up by project above 30 issues (`BacklogRollup.tsx`); the Plan column splits planning-active/plan-ready issues out of Backlog by derived phase (`planColumns.ts`); "Needs you" strip above the columns; mini phase dots on every card (shared classifier); vocabulary column titles (Backlog · Plan · Work · Review · Ship · Done); **Done collapses to the cycle's most recent 12** with a "…and N more this cycle — show all" expansion (uncapped it rendered ~149 cards — 82% of the board's DOM). Live card count: 26 at first render (was 164, and ~860 pre-PRD). Still open: true virtualized rendering inside active columns (unnecessary at current counts — the cap+rollup keep mounts low), drag-and-drop simplification.
>
> _(2026-07-19): v1 — backlog rollup, needs-you strip, phase dots, vocabulary titles._
...
> **Implementation status (2026-07-20, rev 3 — amended by the operator): C-THEME complete; C-VERB amended.** The per-turn line-budget collapse ("▸ N more lines") shipped and was **rejected by the operator after seeing it live — turns render in full again** ("let's not do this after all"). What remains from C-VERB: structured **verdict cards** for completion/failure messages (pass/fail tones with glyphs; bare 'blocked' guarded), the pre-existing work-log group collapse (newest-6 + overflow), and the depth budgets by design (peek = digest glance, dock = digest slice, full transcript in IssueDetail). The §3.9 gate (`messageRows.test.tsx`) now pins: full turns with NO budget toggle, verdict cards with full bodies, and the group collapse. C-THEME's gate (`tests/theme-conformance.spec.ts`) runs the five key screens × both schemes (10/10 live).
>
> _(History: C-VERB items 1–3 delivered, then item 1's auto-collapse rejected — items 2 (grouped commands, pre-existing) and 3 (verdict cards) kept.)_
