...
## Critical Discovery: PAN-759 Root Cause

**The `SessionStart` hook that was supposed to write `ready.json` simply never existed.**

- `src/cli/commands/setup/hooks.ts:151` lists hook scripts to install — `session-start-hook` was NOT in the list
- `hooksAlreadyConfigured()` at line 93 only checks `PreToolUse`, `PostToolUse`, `Stop` — `SessionStart` was never checked
- The settings.json registration code (lines 256-322) registers those three hooks but NEVER registers `SessionStart`
- This is why `waitForClaudePrompt()` and `waitForReadySignal()` tmux fallbacks exist — there was no hook to replace them
- **This is NOT a race condition, permissions issue, or version breakage. It is a missing installation/registration step.**

**Fix required before any tmux removal:**
1. Create `scripts/session-start-hook` (follow pattern of existing hooks)
2. Add to `hookScripts` array, `hooksAlreadyConfigured()` check, and settings.json registration in `src/cli/commands/setup/hooks.ts`
3. Existing agents re-run `pan setup hooks`
...
Already at `~/.overdeck/agents/<id>/runtime.json`. Written by bash hooks on `PostToolUse` / `SessionStart`. Read by Deacon, dashboard, health checks.
...
### Phase 1: Fix hooks (BLOCKS EVERYTHING)
- Fix PAN-759: create missing `session-start-hook`, register it, verify it fires
- Extend `runtime.json` schema: `model`, `activity`, `thinkingSince`, `waitingOnHuman`, `waitingReason`
- Extend PostToolUse hook to write new fields

### Phase 2: Structured event files + JSONL readers (can start after Phase 1)
- Specialist event files: `git-events.jsonl`, `test-results.json`, `merge-results.json`, `review-results.json`
- JSONL tail reader for dashboard transcripts
- Store JSONL path in `runtime.json.jsonlPath`

### Phase 3: Remove high-risk tmux patterns (can start after Phase 1)
- **Delete `confirmDelivery()`** — research shows it has false-positive risk and no evidence of catching real failures
- **Delete `parseThinkingDuration()`** — use `runtime.json.thinkingSince` timestamp instead
- **Collapse `checkLazyAgent()` 17 patterns** — all trigger same action; replace with `waitingOnHuman` boolean
- Add PTY hub in-memory ring buffer for WebSocket snapshots

### Phase 4: Migrate remaining consumers (can start after Phase 1)
- Dashboard transcripts: `mission-control.ts:161,297`, `agents.ts:526`
- Health checks: `health.ts:91`, `health-filtering.ts:13`
- Model extraction: `workspaces.ts:931`
- Heartbeat deprecation: redirect all readers to `runtime.json.lastActivity`
...
- [ ] Heartbeat JSON files (`~/.overdeck/heartbeats/*.json`) eliminated — `lastActivity` lives in `runtime.json`
...
- **Master PRD:** `docs/prds/planned/PAN-798.md` (this file)
- **Pattern audit:** `docs/prds/planned/PAN-798-audit.md` (22 call sites, 70+ patterns, risk assessment)
- **Research results:** `docs/prds/planned/PAN-798-research.md` (all 15 items resolved, including critical PAN-759 finding)
