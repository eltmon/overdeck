...
Verification runs in a supervised worker outside the dashboard service boundary. On Linux, each worker runs in its own transient systemd user scope; other platforms use a detached process group. The worker owns the gate process groups, incremental artifact writes, and terminal result under `~/.overdeck/verification-workers/<issue>/`. A dashboard restart reconnects through durable worker state; merge-time reattachment also requires the exact code HEAD. Boot reconciliation resets a dead worker to `pending`, clears its old commit evidence, and requires a fresh terminal local result before merge.
