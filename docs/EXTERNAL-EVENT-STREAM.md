...
- **`agent.output_received` is live-only.** It is streamed in real time to connected clients, but it is **not persisted** and will **not** appear in `?since=` or `Last-Event-ID` replay. Consumers that need recent terminal output should read `GET /api/agents/:id/output?lines=N`, which reads the pane on either terminal backend (#4097).
