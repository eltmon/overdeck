...
program
  ...
  .option('--no-resume', 'Start dashboard with agent auto-resume disabled for this boot')
  .option('--seed-from-legacy', 'Seed a fresh overdeck.db from panopticon.db (copy conversations + reconstruct in-flight agents/issues). Default is an empty overdeck.db.')
  .action(async (options) => {
    ...
    if (options.seedFromLegacy) {
      ...
      console.log(chalk.yellow('  [--seed-from-legacy] overdeck.db will be seeded from panopticon.db (conversations + in-flight state)'));
    }
