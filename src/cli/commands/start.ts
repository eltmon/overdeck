...
export async function issueCommand(id: string, options: IssueOptions): Promise<void> {
  ...
  try {
    ...
    if (options.dryRun) {
      ...
      console.log(`  vBRIEF:     ${findPlanSync(workspace) ? 'Found' : 'None'}`);
    ...
    if (!findPlanSync(workspace)) {
      await failPostCreateValidation({
        ...
        projectRoot,
        workspaceCreatedThisRun,
        message: `No vBRIEF checklist found for ${id}`,
        printDetails: () => {
          console.log(chalk.dim(`Run ${chalk.cyan(`pan plan ${id}`)} to create the implementation checklist.`));
        },
      });
    }
    ...
    const planningContext = await readPlanningContext(workspace);
    if (planningContext || findPlanSync(workspace)) {
      console.log('');
      console.log(chalk.bold('Context Loaded:'));
      if (planningContext) console.log(`  Planning:   ${chalk.green('✓')} continue.json`);
      if (findPlanSync(workspace)) console.log(`  vBRIEF:     ${chalk.green('✓')} checklist loaded`);
    }
