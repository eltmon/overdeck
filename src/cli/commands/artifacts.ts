...
export function registerArtifactCommands(program: Command): void {
  ...
  artifacts
    ...
    .option('--agent-harness <harness>', 'Agent harness: claude-code, pi, ohmypi, codex, acp, kimi-code, opencode, or user')
...
export function parseAgentHarness(value: string | undefined): ArtifactAgentHarness | undefined {
  if (value === undefined) return undefined;
  if (value === 'claude-code' || value === 'pi' || value === 'ohmypi' || value === 'codex' || value === 'acp' || value === 'kimi-code' || value === 'opencode' || value === 'user') return value;
