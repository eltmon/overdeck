 ...
 *
 * Prints every issue currently sitting in a parked orbit (operator gates,
 * zombie sessions, idle-running agents), oldest first, with the two sentences
 * an operator needs: why it is parked and what would release it. Reads
 * exclusively through resolveParkedPopulation() — never re-derives parking.
 */
...
export function createParkedCommand(): Command {
  return new Command('parked')
    .description('List every issue parked in a stall orbit (operator gates, zombie sessions, idle-running agents), oldest first, with why + release condition')
