...
import { getProviderForModelSync, qualifyPiModel } from './providers.js';
...
function buildPiCommand(config: LauncherConfig, useExec: boolean): string[] {
  ...
  if (config.model) {
    // Ollama models need the `ollama:` prefix stripped and replaced with the
    // Pi ollama provider selector. All other models are provider-qualified so
    // Pi binds to the intended provider instead of its registry default
    // (PAN-1799).
    const providerName = getProviderForModelSync(config.model).name;
    const piModel = providerName === 'ollama'
      ? toPiOllamaModelSelector(config.model)
      : qualifyPiModel(config.model);
    tokens.push('--model', shellQuoteModelIdSync(piModel));
  }
