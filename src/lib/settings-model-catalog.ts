...
export function getAvailableModelsApi(openCodeModels: readonly AvailableModel[] = []): AvailableModelsApi {
  ...
  // Order OpenAI models with latest family first: gpt-6-astra → 6-sol → 6-luna → 5.6-sol (current default) → 5.6-terra → 5.6-luna → 5.5 → 5.4 → 5.3-codex → 5.2 → o-series → gpt-4o legacy.
  const openaiOrder: Record<string, number> = {
    'gpt-6-astra': 0, 'gpt-6-sol': 1, 'gpt-6-luna': 2,
    'gpt-5.6-sol': 3, 'gpt-5.6-terra': 4, 'gpt-5.6-luna': 5,
