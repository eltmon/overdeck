...
export type { AcpSessionEventStreamBarrier, AcpSessionRuntimeEvent } from "./session-events.js";

function resolveSpawnCommand(command: string, args: ReadonlyArray<string>) {
...
> =>
  Effect.gen(function* () {
    ...
    const spawnCommand = resolveSpawnCommand(options.spawn.command, options.spawn.args);
