...
describe("AcpRuntimeModel", () => {
  ...
  it("finds config options and collects flat or grouped select values", () => {
    const configOptions = [
      {
        id: " model ",
        name: "Model",
    ...
    expect(findSessionConfigOption(configOptions, " model ")).toBe(configOptions[0]);
    expect(collectSessionConfigOptionValues(configOptions[0])).toEqual(["fast", "smart"]);
    expect(collectSessionConfigOptionValues(configOptions[1])).toEqual([]);
  });
  ...
  it("waits for the configured replay idle gap using fake timers", async () => {
    ...
    const gateRef = await Effect.runPromise(
      Ref.make(
        Option.some({
          active: true,
          lastActivityAtMillis: Date.now(),
          idleGap: Duration.seconds(2),
          initializeResult,
        }),
      ),
    );
    ...
    let resolved = false;
    const responsePromise = Effect.runPromise(waitForSessionLoadReplayIdle({ gateRef }));
    void responsePromise.then(() => {
      resolved = true;
    });

    await vi.advanceTimersByTimeAsync(1_975);
