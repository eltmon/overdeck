...
\`pan show 1148\` reports the agent as DEAD with \"tmux session is not running\", even though \`agent-pan-1148\` is actively running in \`tmux -L panopticon list-sessions\`. Same for \`pan show 1052\`, \`pan show 1170\`.
...
The comment claims the function accepts \"bare issue IDs (PAN-821)\" and \"prefixed agent IDs (agent-pan-821)\" — but it does NOT handle the bare-number case (\`1148\` without a prefix), which is what users actually type. \`pingAgent(agentId)\` then looks at \`~/.panopticon/agents/agent-1148/\` which doesn't have state.json, and reports dead.

As a side-effect of the failed lookup, \`pan show\` creates \`~/.panopticon/agents/agent-1148/cv.json\` and \`health.json\` — polluting the state directory with bogus entries for non-existent agents.

## Fix

When \`id\` is bare digits (no prefix):
1. Scan \`~/.panopticon/agents/\` for any directory matching \`agent-*-<num>\` with a valid \`state.json\`.
...
Until fixed: \`pan show PAN-1148\` (full prefix) or check \`tmux -L panopticon list-sessions\` directly. Dashboard API at \`/api/issues\` is the source of truth.
...
- `src/lib/issue-id.ts:144-179` is intended to scan `~/.panopticon/agents`, but `src/lib/issue-id.ts:151-164` uses `require('node:fs')` inside an ESM TypeScript module and silently returns `null` when run through the source CLI path.
...
mkdir -p "$tmp/.panopticon/agents/agent-pan-1148"
printf '{"issueId":"PAN-1148"}' > "$tmp/.panopticon/agents/agent-pan-1148/state.json"
...
What remains: fix bare numeric resolution in the source CLI path, add a regression test that constructs a temporary `~/.panopticon/agents/agent-pan-XXXX/state.json`, and audit/fix the related lifecycle commands that still derive `agent-<number>`.
