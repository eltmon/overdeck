...
async function createBeadsFromVBriefPromise(workspacePath: string, planPath?: string): Promise<CreateBeadsResult> {
...
export const createBeadsFromVBrief = (
  ...
  Effect.tryPromise({
    try: () => createBeadsFromVBriefPromise(workspacePath, planPath),
    catch: (cause) =>
