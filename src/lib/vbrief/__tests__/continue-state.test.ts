...
describe('writeContinueState / readContinueState', () => {
  it('round-trips a state document', () => {
    writeContinueState(TEST_DIR, 'PAN-946', freshState('PAN-946'));
  ...
  it('updates the `updated` timestamp on every write', async () => {
    ...
    const first = readContinueState(TEST_DIR, 'PAN-946')!;
    await new Promise(r => setTimeout(r, 5));
    writeContinueState(TEST_DIR, 'PAN-946', { ...first });
    const second = readContinueState(TEST_DIR, 'PAN-946')!;
  ...
  it('preserves `created` across writes', () => {
    ...
    writeContinueState(TEST_DIR, 'PAN-946', state);
    const first = readContinueState(TEST_DIR, 'PAN-946')!;
    writeContinueState(TEST_DIR, 'PAN-946', { ...first, decisions: [{ id: 'D1', summary: 'pick A', recordedAt: '2026-05-03T01:00:00Z' }] });
    const second = readContinueState(TEST_DIR, 'PAN-946')!;
    expect(second.created).toBe(first.created);
  });

  it('returns null when file is missing', () => {
    expect(readContinueState(TEST_DIR, 'PAN-999')).toBeNull();
  });
...
describe('appendSessionEntry', () => {
  it('creates fresh state if file does not exist', () => {
    const next = appendSessionEntry(TEST_DIR, 'PAN-946', {
      reason: 'planning',
      agentModel: 'claude-opus-4-7',
  ...
  it('appends to existing state', () => {
    writeContinueState(TEST_DIR, 'PAN-946', freshState('PAN-946'));
    appendSessionEntry(TEST_DIR, 'PAN-946', { reason: 'start', agentModel: 'claude-opus-4-7' });
    appendSessionEntry(TEST_DIR, 'PAN-946', { reason: 'end' });
    const read = readContinueState(TEST_DIR, 'PAN-946')!;
  ...
  it('records crash info when provided', () => {
    appendSessionEntry(TEST_DIR, 'PAN-946', {
