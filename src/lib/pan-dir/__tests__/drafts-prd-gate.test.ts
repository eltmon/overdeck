...
import { checkPrdGate, MIN_PRD_LINES, getDraftsDir } from '../drafts.js';
...
describe('checkPrdGateSync', () => {
  it('passes when an uppercase projectRoot draft has >= MIN_PRD_LINES lines', () => {
    ...
    const result = checkPrdGate({ projectRoot, workspacePath: workspaceRoot, issueId: 'PAN-2234' });
  ...
  it('finds a workspace draft when projectRoot is null (workspace-only search)', () => {
    ...
    const result = checkPrdGate({ projectRoot: null, workspacePath: workspaceRoot, issueId: 'PAN-2234' });
