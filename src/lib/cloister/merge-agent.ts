    ...
    try {
      ...
      if (!remoteBranches.trim()) {
        ...
        if (fileResult.success) {
          await sendMessageToAgent(issueId, `SPECIALIST FEEDBACK: merge-agent reported BLOCKED for ${issueId}.\n\nMUST READ: ${fileResult.filePath}\n\nUse your Read tool to open this file, read every line, then fix the blocking issues and re-submit. Do NOT stop at the prompt — keep working until the merge can proceed.`);
        } else {
              ...
              if (validationResult.valid) {
                ...
                if (preMergeStashRef) {
                  try {
                    ...
                    console.warn(`[merge-agent] ⚠ Failed to restore pre-merge stash after merge: ${popErr.message}`);
                  }
                }
                ...
                if (preMergeStashRef) {
                  try {
                    await popStash(projectPath, preMergeStashRef);
                    console.log(`[merge-agent] ✓ Restored pre-merge stash after revert`);
                  } catch (popErr: any) {
                    console.warn(`[merge-agent] ⚠ Failed to restore pre-merge stash after revert: ${popErr.message}`);
                  }
                }
