/**
 * Slot reconciliation for the dormant swarm (PAN-3917).
 *
 * Ported from `src/lib/agents/slot-reconcile.ts`, which the cut removed along
 * with the rest of the record-fed reconcilers. What remains is not a reconciler
 * in the banned sense: it reads git branches, the live agent inventory, and the
 * swarm's own slot ledger, and returns a view. It writes nothing and repairs
 * nothing. Item done-ness comes from the issue's continue file, the one home
 * for xBRIEF item progress.
 */
...
export interface SlotReconcileResult {
  ...
  superseded?: SwarmSupersededAttempt[];
}
...
export interface SlotReconcileDeps {
  ...
  listSlotCompletions: (issueId: string, workspace: string) => Record<string, SwarmSlotCompletion>;
}
...
export async function reconcileSlotState(
  ...
  const swarm = readSwarmSlotState(workspace, issueId);
  ...
  const result: SlotReconcileResult = {
    ...
    branches,
    agents,
    superseded: swarm?.supersededAttempts ?? [],
  };
  ...
  for (const slotItem of slotItems) {
    ...
    const completed = options.itemStatuses?.[slotItem.itemId] === 'completed'
...
export function listSlotAssignments(issueId: string, workspace: string): ReconciledSlotAssignment[] {
  return (readSwarmSlotState(workspace, issueId)?.slotAssignments ?? [])
