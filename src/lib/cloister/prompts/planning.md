...
- `metadata.kind` is REQUIRED on every plan item for subject-matter routing. Use one of `"docs"`, `"api"`, `"backend"`, `"frontend"`, `"infra"`, `"test"`, `"refactor"`, `"design"`, or `"spike"`; default to `"backend"` when the item has no clearer subject-matter category.
