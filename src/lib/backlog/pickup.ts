 ...
 * item is NOT pickable until released, unless auto_pickup_backlog is ON or active
 * order-book membership supplies the operator release. An open `objection` halts pickup
...
export function isAutoPickable(
  ...
  // Blanket release and active order-book membership both satisfy the operator release
  // gate. vetoed / parked / objection / in-pipeline / epic gate in every mode.
  const released = s.released || autoPickupBacklog || activeBookMember;
...
export function pickableQueue(
  nodes: readonly SequenceNode[],
  lk: ClassifyLookups,
  autoPickupBacklog = false,
  activeBookMembership: ReadonlySet<string> = new Set(),
): ForecastNode[] {
  return nodes
    .map((n) => ({ issue: n.issue, rank: n.rank, size: n.size, state: classifyIssue(n, lk) }))
    .filter((n) => isAutoPickable(n.state, autoPickupBacklog, activeBookMembership.has(n.issue.toUpperCase())))
...
export function computeWaves(
  ...
  const q = pickableQueue(nodes, lk, autoPickupBacklog, activeBookMembership);
...
export function computeLanes(
  ...
  for (const item of pickableQueue(nodes, lk, autoPickupBacklog, activeBookMembership)) {
    let li = 0;
...
export function computeCohort(
  ...
  for (const item of pickableQueue(nodes, lk, autoPickupBacklog, activeBookMembership).slice(0, Math.max(1, n) * 2)) {
    ids.add(item.issue);
  }
  return [...ids];
}
...
export function computeStats(
  ...
  for (const node of nodes) {
    ...
    const activeBookMember = activeBookMembership.has(node.issue.toUpperCase());
    if (isAutoPickable(s, autoPickupBacklog, activeBookMember)) stats.pickable++;
    ...
    if (s.ready && s.planned && !s.released && !activeBookMember && !s.parked && !s.vetoed && !s.objection && !s.inPipeline) stats.needsRelease++;
  }
  return stats;
}
