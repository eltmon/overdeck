...
export function buildAgentHostSecretBoundaryPrelude(
  ...
  if (platform === 'linux') {
    lines.push(
      ...
      `  exec bwrap --bind / / --dev-bind /dev /dev --unshare-pid --proc /proc --unshare-ipc --new-session --bind /dev/null ${keyReference} -- "$0" "$@"`,
    );
    ...
    lines.push(
      ...
      '  exec sandbox-exec -p "$_overdeck_sandbox_profile" "$0" "$@"',
    );
  } else {
