...
describe('parseOAuthPayload', () => {
  it('returns loggedIn: true with valid credentials', async () => {
    ...
    const result = await runEffect((await Effect.runPromise(parseOAuthPayload(raw))));
  ...
  it('fails with ClaudeCredentialParseError on malformed JSON', async () => {
    const err = await runEffectFail((await Effect.runPromise(parseOAuthPayload('not json'))));
  ...
  it('handles empty object', async () => {
    const result = await runEffect((await Effect.runPromise(parseOAuthPayload('{}'))));
    expect(result.loggedIn).toBe(false);
  });
});
