...
describe('config-yaml background_ai normalization', () => {
  it('defaults to cheapMode ON, not onboarded, with registry feature defaults (PAN-1589)', () => {
    const { config } = mergeConfigs(null);
    expect(config.backgroundAi.cheapMode).toBe(true);
    expect(config.backgroundAi.onboarded).toBe(false);
    expect(config.backgroundAi.features).toEqual(defaultBackgroundAiFeatures());
  });

  it('merges cheap_mode, onboarded, and per-feature overrides from raw YAML', () => {
    ...
    expect(config.backgroundAi.onboarded).toBe(true);
