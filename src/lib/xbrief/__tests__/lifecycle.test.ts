...
beforeEach(() => {
  TEST_DIR = mkdtempSync(join(tmpdir(), 'xbrief-lifecycle-'));
});
...
describe('LEGACY_VBRIEF_LIFECYCLE_DIRS', () => {
  it('contains exactly the four lifecycle dirs in canonical order', () => {
    expect(LEGACY_VBRIEF_LIFECYCLE_DIRS).toEqual(['proposed', 'active', 'completed', 'cancelled']);
  });
});

describe('slugify', () => {
  it('lowercases and dashes', () => {
    expect(slugify('xBRIEF Lifecycle Foundation')).toBe('xbrief-lifecycle-foundation');
  });
...
describe('generateXBriefFilename', () => {
  it('produces YYYY-MM-DD-ISSUE-ID-slug.xbrief.json', () => {
    const fname = generateXBriefFilename('PAN-946', 'xbrief-lifecycle', '2026-05-03');
    expect(fname).toBe('2026-05-03-PAN-946-xbrief-lifecycle.xbrief.json');
  });
...
describe('isXBriefFilename', () => {
  it.each([
    ...
    '2026-05-03-PAN-946-xbrief-lifecycle.xbrief.json',
  ])('accepts %s', (filename) => {
...
describe('parseXBriefFilename', () => {
  it.each([
    ['2026-05-03-PAN-946-vbrief-lifecycle.vbrief.json', 'vbrief-lifecycle'],
    ['2026-05-03-PAN-946-xbrief-lifecycle.xbrief.json', 'xbrief-lifecycle'],
  ])('extracts parts from %s', (filename, slug) => {
    const parts = parseXBriefFilename(filename);
    expect(parts).toEqual({ date: '2026-05-03', issueId: 'PAN-946', slug });
  });
