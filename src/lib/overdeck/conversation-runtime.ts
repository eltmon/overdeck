...
export async function resolveAllowedHarness(requested: unknown, model?: string | null): Promise<RuntimeName> {
  ...
  const explicit: RuntimeName | undefined =
    requested === 'ohmypi' || requested === 'claude-code' || requested === 'codex' || requested === 'acp' || requested === 'kimi-code' || requested === 'opencode' || requested === 'muse' || requested === 'prime-agent'
      ? requested
      : undefined;
...
export async function handleConversationSwitchModel(
  ...
  if (requestedHarness === 'ohmypi' || requestedHarness === 'pi' || requestedHarness === 'claude-code' || requestedHarness === 'codex' || requestedHarness === 'acp' || requestedHarness === 'kimi-code' || requestedHarness === 'opencode' || requestedHarness === 'muse' || requestedHarness === 'prime-agent') {
