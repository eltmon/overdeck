...
import {
  parseCodexSessionCostEventsSync,
  parseCodexSessionSync,
} from '../cost-parsers/codex-parser.js';
      ...
      Effect.gen(function* () {
        ...
        for (const agentName of agentNames) {
          ...
          for (const sessionFile of sessionFiles) {
            ...
            const codexEvents = yield* Effect.sync(() => parseCodexSessionCostEventsSync(sessionFile));
            for (const usage of codexEvents) {
              const event: CostEvent = {
                ts:          new Date(usage.timestamp),
                issueId:     issueIdFromAgentName(agentName),
                agentId:     agentName,
