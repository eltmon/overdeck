...
let testDb: OverdeckTestDb | undefined;
...
afterEach(() => {
  if (testDb) teardownOverdeckTestDb(testDb);
  testDb = undefined;
});
...
describe('release-set-db', () => {
  ...
  it('deletes release sets and cascades component rows', () => {
    ...
    const components = testDb!.raw().prepare('SELECT * FROM release_set_components WHERE issue_id = ?').all('PAN-399');
