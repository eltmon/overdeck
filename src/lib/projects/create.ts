...
// Re-exported so existing importers of the core keep working; the class itself
// lives with the failure taxonomy it carries.
export { ProjectCreateFailureError };
import {
  getProjectSync,
  listProjectsAsync,
  type ProjectConfig,
...
/**
 * The public projection of a resolved intent.
 *
 * `cloneUrl` is the operator's exact transport URL, which may carry credentials
 * (`https://user:token@host/repo.git`). The core needs it verbatim to hand to
 * git; nothing outside the server does. Every surface that serializes an intent —
 * the resolve route, the create route, the CLI's dry-run document — sends this
 * instead, so there is one place to get the redaction right rather than one per
 * caller.
 */
export type PublicProjectIntent = Omit<ResolvedProjectIntent, 'cloneUrl'> & {
  cloneUrl: string | null;
};

export function toPublicProjectIntent(intent: ResolvedProjectIntent): PublicProjectIntent {
  return { ...intent, cloneUrl: redactTransportUrl(intent.cloneUrl) };
}
