...
interface NormalizedCodexEntry {
  type?: string;
...
function normalizeEntry(entry: Record<string, unknown>): NormalizedCodexEntry {
  const payload = entry['payload'] && typeof entry['payload'] === 'object'
    ? (entry['payload'] as Record<string, unknown>)
    : null;
  return {
    type: (payload?.['type'] ?? entry['type']) as string | undefined,
    data: payload ?? entry,
    timestamp: typeof entry['timestamp'] === 'string' ? (entry['timestamp'] as string) : '',
  };
}

function usageTokens(usage: CodexTokenUsageFields | undefined): {
  input: number;
  output: number;
  cacheRead: number;
} | null {
  ...
  return {
    ...
    cacheRead: pickUsage(usage.cached_input, usage.cached_input_tokens) ?? 0,
  };
}

function costFor(model: string, input: number, output: number, cacheRead: number): number {
  const pricing = getPricingSync('openai', model);
  const nonCachedInput = Math.max(0, input - cacheRead);
  return (
    (nonCachedInput / 1000) * (pricing?.inputPer1k ?? 0)
    + (cacheRead / 1000) * (pricing?.cacheReadPer1k ?? 0)
    + (output / 1000) * (pricing?.outputPer1k ?? 0)
  );
}
...
export function parseCodexSessionCostEventsSync(sessionFile: string): CodexCostEventUsage[] {
  ...
  for (const line of raw.split('\n')) {
    ...
    const { type, data, timestamp } = normalizeEntry(entry);
    ...
    if (type === 'session_meta') {
      ...
      if (typeof data['model'] === 'string' && data['model']) model = data['model'];
      if (!threadId && typeof data['thread_id'] === 'string') threadId = data['thread_id'];
      ...
      const usage = usageTokens(info?.last_token_usage);
      if (!usage) continue;

      const sessionId = threadId || sessionFile;
      // Verified against /home/eltmon/.codex/sessions/2026/05/31/rollout-2026-05-31T03-31-14-019e7cf1-b148-7a80-80a4-2b891cb13d4c.jsonl:
      // last_token_usage is per-turn; summing last_token_usage equals the final total_token_usage.
      events.push({
        requestId: `codex:${sessionId}:${seq}`,
        timestamp,
        sessionId,
        sessionFile,
        provider: 'openai',
        ...
        cacheWrite: 0,
        cost: costFor(model, usage.input, usage.output, usage.cacheRead),
      });
      seq++;
    }
  }
