...
export function useSwitchModel(agentId: string | undefined, issueId: string) {
  ...
  const switchMutation = useMutation<SwitchModelResult, Error, { model: string; message?: string; harness?: 'claude-code' | 'pi' | 'ohmypi' | 'codex' | 'acp' | 'kimi-code' | 'opencode' | 'muse' | 'prime-agent' }>({
