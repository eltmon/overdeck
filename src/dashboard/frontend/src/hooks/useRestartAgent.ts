...
export function useRestartAgent(agentId: string | undefined) {
  ...
  const restartMutation = useMutation<RestartResult, Error, { model?: string; graceful?: boolean; message?: string }>({
    mutationFn: async ({ model, graceful = true, message }) => {
      if (!agentId) throw new Error('No agent to restart');

      const res = await fetch(`/api/agents/${agentId}/restart`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ model, graceful, message }),
      });
