...
export function WorkspacePanel({ agent, issueId, issueUrl, issue, onClose }: WorkspacePanelProps) {
  ...
  const handleCleanWorkspace = async () => {
    if (await confirm({ message: `Clean and recreate corrupted workspace for ${issueId}?\n\nThis will:\n- Remove the corrupted workspace directory\n- Create a fresh workspace`, title: 'Clean Workspace', variant: 'danger', confirmLabel: 'Clean & Recreate' })) {
  ...
  const handleReview = async () => {
    ...
    if (await confirm({ message: msg, title: isReReview ? 'Re-run Review' : 'Start Review' })) {
      reviewMutation.mutate();
    }
  };

  const handleMerge = async () => {
    if (await confirm({ message: `Merge ${issueId} to main?\n\nReview and tests have passed. This will:\n- Merge the feature branch to main\n- Run final verification tests\n- Clean up workspace`, title: 'Merge to Main', variant: 'warning', confirmLabel: 'Merge' })) {
      mergeMutation.mutate();
    }
  };

  const handleClose = async () => {
    if (await confirm({ message: `Close ${issueId} without merging? This will:\n- Close the issue (no merge)\n- Stop any running agent\n- Remove the workspace\n(Feature branch is preserved for history)`, title: 'Close Without Merge', variant: 'danger', confirmLabel: 'Close' })) {
          ...
          <>
            ...
            {containerMenu.containerName === 'postgres' && (
              <>
                ...
                <button
                  onClick={async () => {
                    if (await confirm({ message: 'Drop and reload database from seed file?\n\nThis will:\n- Stop the API container\n- Drop the existing database\n- Reload from seed-cleaned.sql\n- Restart the API\n\nAll workspace data will be replaced.', title: 'Refresh Database', variant: 'danger', confirmLabel: 'Drop & Reload' })) {
