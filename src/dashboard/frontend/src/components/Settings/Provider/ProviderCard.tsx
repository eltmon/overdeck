...
export function ProviderCard({
  ...
  return (
    <div className="bg-surface-raised rounded-lg p-6 flex flex-col gap-5 border border-divider">
      {/* Header with Toggle */}
      ...
      >
        <div className="space-y-2">
          <label className="text-xs font-semibold text-content-subtle uppercase tracking-wider flex items-center gap-2">
            API Key
          ...
          <div className={cn('relative', showThinkingLevel && 'space-y-4')}>
            <div className="relative">
              <input
                ...
                disabled={locked}
                className={cn(
                  'w-full bg-input-bg border border-divider rounded-lg text-sm text-content px-3 py-2 pr-10 focus:ring-[#a078f7] focus:border-[#a078f7]',
                  locked && 'bg-surface-overlay text-content-muted cursor-not-allowed'
                )}
              />
              <button
                type="button"
                onClick={() => setShowKey(!showKey)}
                className="absolute right-3 top-2 text-content-muted hover:text-content"
                disabled={locked}
              >
                {locked ? (
            ...
            {!locked && !connected && isConfigured && (
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={testing}
                  className="bg-surface-overlay hover:bg-surface-emphasis text-xs font-bold px-3 py-2 rounded-lg transition-colors flex items-center gap-2"
                >
            ...
            {showThinkingLevel && connected && (
              <div className="pl-2 border-l-2 border-divider ml-5">
