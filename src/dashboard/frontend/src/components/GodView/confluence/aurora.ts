...
const STATIC_BACKGROUND = 'radial-gradient(ellipse at 50% 30%, #101a33 0%, #0a0e1a 70%)';
...
function createNullAurora(canvas: HTMLCanvasElement): AuroraController {
  ...
  const resize = () => {
    if (disposed) return;
    canvas.width = Math.floor(window.innerWidth * GDPR);
    canvas.height = Math.floor(window.innerHeight * GDPR);
  };
  const dispose = () => {
    if (disposed) return;
    disposed = true;
...
export function createAurora(canvas: HTMLCanvasElement): AuroraController {
  ...
  } = { res: null, time: null, energy: null };
  ...
  try {
    ...
    if (gl) {
      const activeGl = gl;
      const mk = (type: number, src: string) => { const s = activeGl.createShader(type); if (!s) throw new Error('Unable to create WebGL shader'); activeGl.shaderSource(s, src); activeGl.compileShader(s);
        if (!activeGl.getShaderParameter(s, activeGl.COMPILE_STATUS)) throw new Error(activeGl.getShaderInfoLog(s) ?? undefined); return s; };
      glProg = gl.createProgram();
      if (!glProg) throw new Error('Unable to create WebGL program');
      vertexShader = mk(gl.VERTEX_SHADER, VERT);
      fragmentShader = mk(gl.FRAGMENT_SHADER, FRAG);
      gl.attachShader(glProg, vertexShader);
      gl.attachShader(glProg, fragmentShader);
      gl.linkProgram(glProg);
      if (!gl.getProgramParameter(glProg, gl.LINK_STATUS)) throw new Error(gl.getProgramInfoLog(glProg) ?? undefined);
  ...
  if (!gl || !glProg) return createNullAurora(canvas);

  canvas.style.background = '';
  const context = gl;
  const program = glProg;
  const GDPR = Math.min(window.devicePixelRatio || 1, 1.75);
  let energy = .35;
  let disposed = false;
  ...
  const frame = (now: number) => {
    if (disposed) return;
    if (!document.hidden) {
      context.uniform2f(glU.res, canvas.width, canvas.height);
      context.uniform1f(glU.time, (now - startedAt) / 1000);
      context.uniform1f(glU.energy, energy);
  ...
  const setEnergy = (value: number) => {
    energy = value;
    if (!disposed && !document.hidden) context.uniform1f(glU.energy, energy);
  };
  ...
  return { setEnergy, resize, dispose };
}
