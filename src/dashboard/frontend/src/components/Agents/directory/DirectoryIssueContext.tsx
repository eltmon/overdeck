...
function Field({ label, full, children }: { label: string; full?: string; children: ReactNode }) {
  return (
    <div className="flex min-w-0 items-baseline gap-2">
      <dt className="w-20 shrink-0 text-[11px] text-muted-foreground">{label}</dt>
      <dd className="min-w-0 truncate text-[12px] text-foreground" title={full}>{children}</dd>
    </div>
  );
}

export function DirectoryIssueContext({ issueId, issueTitle = null }: { issueId: string; issueTitle?: string | null }) {
  ...
  return (
    <section aria-label={`Issue ${issueId}`} data-component="directory-issue-context" className="border-b border-border px-4 py-3">
      <div className="mb-2 flex items-center gap-2">
        <span className="shrink-0 font-mono-ui text-[12px] text-foreground">{issueId}</span>
        {issueTitle && (
          <span className="min-w-0 truncate text-[12px] font-medium text-foreground" title={issueTitle}>{issueTitle}</span>
        )}
        {derived && <span className="shrink-0 text-[12px] text-muted-foreground">{derived.state}</span>}
        <button
          type="button"
      ...
      {derived ? (
        <dl className="grid grid-cols-1 gap-1 @[520px]/detail:grid-cols-2">
          {derived.pr ? (
            <>
              <Field label="PR" full={derived.pr.url}>
          ...
          {derived.branch && (
            <>
              <Field label="Branch" full={derived.branch.name}><span className="font-mono-ui">{derived.branch.name}</span></Field>
              <Field label="Ahead">
