...
function ProjectSpecialistCard({ specialist }: ProjectSpecialistCardProps) {
  ...
  const handleTerminate = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (await confirm({ message: `Terminate ${specialist.specialistType} for ${specialist.projectKey}?`, title: 'Terminate Specialist', variant: 'danger', confirmLabel: 'Terminate' })) {
