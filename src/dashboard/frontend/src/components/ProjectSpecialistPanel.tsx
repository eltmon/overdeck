...
function ProjectSpecialistCard({ specialist }: ProjectSpecialistCardProps) {
  ...
  const handleTerminate = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (await confirmDialog({ message: `Terminate ${specialist.specialistType} for ${specialist.projectKey}?`, title: 'Terminate Specialist', variant: 'destructive', confirmLabel: 'Terminate' })) {
