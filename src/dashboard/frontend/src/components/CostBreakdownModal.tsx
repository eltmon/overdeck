      ...
      <div className="relative bg-surface-raised rounded-xl shadow-2xl w-full max-w-lg mx-4 max-h-[70vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-divider">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-content">Cost Breakdown: {issueId}</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {isLoading && (
            <div className="space-y-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="h-8 bg-surface-overlay/50 rounded animate-pulse" />
              ))}
          ...
          {data && (
            <>
              ...
              <div className="flex items-baseline justify-between">
                <span className="text-content-subtle text-sm">Total Cost</span>
                <span className="text-2xl font-bold text-emerald-400">{formatCost(totalCost)}</span>
              </div>

              {/* Token summary */}
              <div className="grid grid-cols-4 gap-2 text-xs">
                <div className="bg-surface-overlay/40 rounded p-2 text-center">
                  <div className="text-content-muted">Input</div>
                  <div className="text-content font-medium">{formatTokens(data.inputTokens)}</div>
                </div>
                <div className="bg-surface-overlay/40 rounded p-2 text-center">
                  <div className="text-content-muted">Output</div>
                  <div className="text-content font-medium">{formatTokens(data.outputTokens)}</div>
                </div>
                <div className="bg-surface-overlay/40 rounded p-2 text-center">
                  <div className="text-content-muted">Cache Read</div>
                  <div className="text-content font-medium">{formatTokens(data.cacheReadTokens)}</div>
                </div>
                <div className="bg-surface-overlay/40 rounded p-2 text-center">
                  <div className="text-content-muted">Cache Write</div>
                  <div className="text-content font-medium">{formatTokens(data.cacheWriteTokens)}</div>
                </div>
              </div>
              ...
              {stages.length > 0 && (
                <div>
                  <div className="flex items-center gap-1.5 text-sm font-medium text-content mb-2">
                    <Layers className="w-4 h-4 text-content-subtle" />
                    By Stage
                  </div>
                      ...
                      return (
                        <div key={stage} className="group">
                          <div className="flex items-center justify-between text-sm py-1">
                            ...
                            <div className="flex items-center gap-3 text-content-subtle text-xs">
                              <span>{formatTokens(info.tokens)} tokens</span>
                              <span>{info.calls} calls</span>
                              <span className="text-content font-medium w-16 text-right">{formatCost(info.cost)}</span>
                            </div>
                          </div>
              ...
              {models.length > 0 && (
                <div>
                  <div className="flex items-center gap-1.5 text-sm font-medium text-content mb-2">
                    <Cpu className="w-4 h-4 text-content-subtle" />
                    By Model
                  </div>
                  <div className="space-y-1">
                    {models.map(([model, info]) => {
                      ...
                      return (
                        <div key={model} className="group">
                          <div className="flex items-center justify-between text-sm py-1">
                            <span className="text-content">{friendlyModelName(model)}</span>
                            <div className="flex items-center gap-3 text-content-subtle text-xs">
                              <span>{formatTokens(info.tokens)} tokens</span>
                              <span>{info.calls} calls</span>
                              <span className="text-content font-medium w-16 text-right">{formatCost(info.cost)}</span>
              ...
              {/* Last updated */}
              {data.lastUpdated && (
                <div className="text-xs text-content-muted text-right pt-1 border-t border-divider">
