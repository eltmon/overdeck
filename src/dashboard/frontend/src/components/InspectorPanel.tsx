...
export function InspectorPanel({ agent, issueId, issueUrl, issue, onClose, onOpenTerminal }: InspectorPanelProps) {
  ...
  const handleReview = () => {
    ...
    if (confirm(message)) reviewMutation.mutate();
  };

  const handleMerge = () => {
    if (confirm(`Merge ${issueId} to main?\n\nReview and tests have passed.`)) mergeMutation.mutate();
  };

  const handleClose = () => {
    if (confirm(`Close ${issueId} without merging?`)) closeMutation.mutate();
  };

  const handleKill = () => {
    if (agent && confirm(`Kill agent ${agent.id}?`)) killMutation.mutate();
  };

  const handleCleanWorkspace = () => {
    if (confirm(`Clean and recreate corrupted workspace for ${issueId}?`)) cleanMutation.mutate();
  };
  ...
  return (
    <>
              ...
              <div className="mt-1.5 space-y-0.5">
                {Object.entries(costData.byModel).sort(([, a], [, b]) => b.cost - a.cost).map(([model, info]) => (
                  <div key={model} className="flex justify-between text-[10px]">
                    ...
                    <span className="text-white ml-2">{formatCost(info.cost)}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      {/* Issue labels/tags section for no-agent view */}
      {!agent && issue && issue.labels.length > 3 && (
        <div className="fixed bottom-4 right-4 flex flex-wrap gap-1 max-w-xs">
