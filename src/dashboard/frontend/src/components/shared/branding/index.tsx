...
const PiHarnessIcon = (props: IconProps) => (
  <svg viewBox="0 0 24 24" role="img" aria-label="Pi harness mark" {...props}>
    <rect width="24" height="24" rx="5" fill="currentColor" />
    <text x="12" y="16" textAnchor="middle" fontSize="13" fontWeight="700" fill="white">
      π
    </text>
  </svg>
);
...
export const PROVIDER_BRANDS: Record<Provider, BrandDef> = {
  ...
  google: { id: 'google', label: 'Google', color: '#4285f4', Icon: createLetterIcon('G') },
...
export const PROVIDER_COLORS: Record<Provider, string> = Object.fromEntries(
  Object.entries(PROVIDER_BRANDS).map(([provider, brand]) => [provider, brand.color])
) as Record<Provider, string>;

export function ProviderLetterBadge({ provider, label }: { provider: string; label: string }) {
  const brand = PROVIDER_BRANDS[provider as Provider];
...
export function ProviderLogo({ provider, label, className }: { provider: Provider | string; label?: string; className?: string }) {
  const brand = PROVIDER_BRANDS[provider as Provider];
  if (!brand) {
    return <ProviderLetterBadge provider={provider} label={label ?? provider} />;
  }
  const Icon = brand.Icon;
  return <Icon className={className} style={{ color: brand.color }} aria-label={`${brand.label} logo`} />;
}

export function HarnessLogo({ harness, className }: { harness: Harness; className?: string }) {
  const brand = HARNESS_BRANDS[harness];
  const Icon = brand.Icon;
  return <Icon className={className} style={{ color: brand.color }} aria-label={`${brand.label} logo`} />;
}

export function ProviderDot({ provider }: { provider: Provider | string }) {
  const brand = PROVIDER_BRANDS[provider as Provider];
  return <span className={styles.pickerOptionProviderDot} style={{ background: brand?.color ?? '#6b7280' }} />;
}

export const ProviderIcon = ProviderLogo;
