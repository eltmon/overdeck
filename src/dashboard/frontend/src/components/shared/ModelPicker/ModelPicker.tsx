...
export function HarnessSelect({
  ...
  // forbids (e.g. config drift), fall back to claude-code.
  useEffect(() => {
            ...
            {HARNESS_OPTIONS.map((harness) => {
              ...
              return (
                <button
                  ...
                  title={isBlocked ? decision.reason : harness.description}
                ...
                >
                  ...
                  {isBlocked && decision.reason && (
                    <span className={styles.pickerOptionCost}>{decision.reason}</span>
                  )}
                </button>
              );
            })}
