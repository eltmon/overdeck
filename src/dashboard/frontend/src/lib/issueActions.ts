...
const ISSUE_ACTION_DEFINITIONS: Omit<IssueActionEntry, 'scope'>[] = [
  { key: 'plan', label: 'Plan', description: 'Have an AI planner interview you and write the implementation plan for this issue.', panVerb: 'plan', endpoint: '/api/issues/:id/start-planning', enabledWhen: canPlan, phasePrimary: phasePrimary('plan'), kind: 'dialog', group: 'lifecycle' },
  { key: 'autoPlan', label: 'Auto-plan', description: 'Write the plan automatically, no questions asked. Good for well-understood work.', panVerb: 'plan --auto', endpoint: '/api/issues/:id/plan', enabledWhen: canPlan, phasePrimary: [], kind: 'dialog', group: 'lifecycle' },
  { key: 'watchPlanning', label: 'Watch planning', description: 'Open the live planning session to watch or steer it.', panVerb: null, endpoint: null, enabledWhen: (state) => deriveIssueActionPhase(state) === 'PLANNING', phasePrimary: phasePrimary('watchPlanning'), kind: 'dialog', group: 'lifecycle' },
  { key: 'donePlanning', label: 'Done planning', description: 'Accept the finished plan so work can start.', panVerb: 'plan finalize', endpoint: '/api/issues/:id/complete-planning', enabledWhen: canFinalizePlanning, phasePrimary: phasePrimary('donePlanning'), kind: 'safe', group: 'lifecycle' },
  { key: 'startAgent', label: 'Start agent', description: 'Start an AI agent implementing this issue from its plan.', panVerb: 'start', endpoint: '/api/agents', enabledWhen: canStartAgent, phasePrimary: phasePrimary('startAgent'), kind: 'dialog', group: 'lifecycle' },
  { key: 'startSkipPlanning', label: 'Start without planning', description: 'Skip planning: generate a minimal plan from the issue text and start immediately.', panVerb: 'start --auto', endpoint: '/api/agents', enabledWhen: canStartWithoutPlanning, phasePrimary: [], kind: 'dialog', group: 'lifecycle' },
  ...
  { key: 'discussions', label: 'Discussions', description: 'Open recorded design discussions.', panVerb: null, endpoint: null, enabledWhen: (state) => state.hasDiscussions === true, phasePrimary: [], kind: 'safe', group: 'inspect' },
  ...
  { key: 'restartAgent', label: 'Restart agent', description: 'Stop the agent and start a replacement that keeps the issue context.', panVerb: null, endpoint: '/api/agents/:agentId/restart', enabledWhen: (state) => hasAgent(state) && !isMerged(state), phasePrimary: [], kind: 'destructive', group: 'danger' },
];
