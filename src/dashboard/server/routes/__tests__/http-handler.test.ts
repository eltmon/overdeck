...
describe('httpHandler', () => {
  it('passes through successful responses unchanged', async () => {
    const effect = Effect.succeed(
      HttpServerResponse.text(JSON.stringify({ ok: true }), {
        status: 200,
  ...
  it('maps IssueNotFound to 404', async () => {
    ...
    const { status, body } = await runRoute(effect as Effect.Effect<typeof HttpServerResponse.Type, IssueNotFound, never>);
  ...
  it('maps WorkspaceNotFound to 404', async () => {
    ...
    const { status } = await runRoute(effect as Effect.Effect<typeof HttpServerResponse.Type, WorkspaceNotFound, never>);
    expect(status).toBe(404);
  });

  it('maps TrackerNotConfigured to 503', async () => {
    const effect = Effect.fail(new TrackerNotConfigured({ tracker: 'linear' }));
    const { status } = await runRoute(effect as Effect.Effect<typeof HttpServerResponse.Type, TrackerNotConfigured, never>);
    expect(status).toBe(503);
  });

  it('maps RateLimited to 429 with retryAfter', async () => {
    const effect = Effect.fail(new RateLimited({ retryAfter: 60 }));
    const { status, body } = await runRoute(effect as Effect.Effect<typeof HttpServerResponse.Type, RateLimited, never>);
  ...
  it('maps AgentAlreadyRunning to 409', async () => {
    ...
    const { status } = await runRoute(effect as Effect.Effect<typeof HttpServerResponse.Type, AgentAlreadyRunning, never>);
    expect(status).toBe(409);
  });

  it('maps BeadsNotInitialized to 422', async () => {
    const effect = Effect.fail(new BeadsNotInitialized({ workspace: '/tmp/ws' }));
    const { status } = await runRoute(effect as Effect.Effect<typeof HttpServerResponse.Type, BeadsNotInitialized, never>);
    expect(status).toBe(422);
  });

  it('maps PlanEmpty to 422', async () => {
    const effect = Effect.fail(new PlanEmpty({ id: 'PAN-4' }));
    const { status } = await runRoute(effect as Effect.Effect<typeof HttpServerResponse.Type, PlanEmpty, never>);
    expect(status).toBe(422);
  });

  it('maps TrackerApiError to 502', async () => {
    const effect = Effect.fail(new TrackerApiError({ tracker: 'github', message: 'API down' }));
    const { status } = await runRoute(effect as Effect.Effect<typeof HttpServerResponse.Type, TrackerApiError, never>);
  ...
  it('maps WorkspaceCreateError to 500', async () => {
    ...
    const { status } = await runRoute(effect as Effect.Effect<typeof HttpServerResponse.Type, WorkspaceCreateError, never>);
    expect(status).toBe(500);
  });

  it('maps AgentStartError to 500', async () => {
    const effect = Effect.fail(new AgentStartError({ id: 'PAN-6', message: 'crash' }));
    const { status } = await runRoute(effect as Effect.Effect<typeof HttpServerResponse.Type, AgentStartError, never>);
    expect(status).toBe(500);
  });

  it('maps unknown errors to 500', async () => {
    const effect = Effect.die(new Error('unexpected'));
    const { status } = await runRoute(effect as unknown as Effect.Effect<typeof HttpServerResponse.Type, unknown, never>);
    expect(status).toBe(500);
  });
});
