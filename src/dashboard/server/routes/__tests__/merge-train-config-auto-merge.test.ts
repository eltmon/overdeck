...
const { mergeCancelCommand } = await import('../../../../cli/commands/merge.js');
...
// #3954: `pan merge cancel` kept calling the removed /api/flywheel route. Drive
// the real CLI command into the real route layer so the URL, method, and
// internal-token header are checked against the route the dashboard serves.
describe('pan merge cancel against the merge-train route layer', () => {
  ...
  it('cancels the pending row through DELETE /api/merge-train/auto-merge/:id', async () => {
    process.env.OVERDECK_INTERNAL_TOKEN = 'cli-route-token';
