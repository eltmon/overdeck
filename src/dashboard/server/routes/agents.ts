    ...
    if (!hasBeads) {
      ...
      try {
        ...
        if (hasBeads) {
          ...
          recoveryFailureReason = recovery.errors[0] ?? 'Unknown error during beads creation';
          console.warn(`[agents] Auto-recovery errors: ${recovery.errors.join(', ')}`);
        } else {
          recoveryFailureReason = 'createBeadsFromVBrief returned no beads and no errors';
        }
      } catch (recoveryErr: any) {
        recoveryFailureReason = recoveryErr.message;
        console.warn(`[agents] Auto-recovery failed: ${recoveryErr.message}`);
      }
    }

    if (!hasBeads) {
      const errorDetail = recoveryFailureReason
        ? ` Recovery failed: ${recoveryFailureReason}.`
        : '';
      return jsonResponse({
