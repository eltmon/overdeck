...
describe('syncMainIntoWorkspace pipeline-owned conflicts (PAN-1841)', () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(join(repo, '.tasks', 'issues.jsonl'), '{"id":"PAN-1","state":"base"}\n');
    ...
    writeFileSync(join(repo, '.tasks', 'issues.jsonl'), '{"id":"PAN-1","state":"feature"}\n');
    ...
    execSync('git add .tasks .pan', { cwd: repo });
    execSync('git commit -q -m "feature state"', { cwd: repo });

    execSync('git switch -q main', { cwd: repo });
    writeFileSync(join(repo, '.tasks', 'issues.jsonl'), '{"id":"PAN-1","state":"main"}\n');
  ...
  it('prefers origin/main for divergent .pan lifecycle and .tasks files', async () => {
    ...
    expect(readFileSync(join(repo, '.tasks', 'issues.jsonl'), 'utf-8')).toBe('{"id":"PAN-1","state":"main"}\n');
