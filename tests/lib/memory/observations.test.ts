...
describe('observation writer', () => {
  it('appends observations to date-scoped JSONL and updates the markdown mirror', async () => {
    ...
    expect(result).toEqual({
      jsonlPath: join(tempDir!, 'memory/overdeck/feature-pan-1052/observations/2026-05-16.jsonl'),
      markdownPath: join(tempDir!, 'memory/overdeck/feature-pan-1052/observations/2026-05-16.md'),
    });
  ...
  it('keeps JSONL, FTS, and markdown idempotent by observation id', async () => {
    ...
    const jsonlPath = join(tempDir!, 'memory/overdeck/feature-pan-1052/observations/2026-05-16.jsonl');
    const markdownPath = join(tempDir!, 'memory/overdeck/feature-pan-1052/observations/2026-05-16.md');
    ...
    const files = await readdir(join(tempDir!, 'memory/overdeck/feature-pan-1052/observations'));
  ...
  it('repairs small legacy JSONL files with missing observation_index rows before appending', async () => {
    ...
    const jsonlPath = join(tempDir!, 'memory/overdeck/feature-pan-1052/observations/2026-05-16.jsonl');
    await mkdir(join(tempDir!, 'memory/overdeck/feature-pan-1052/observations'), { recursive: true });
