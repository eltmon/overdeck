...
import { detectTerminalApiError } from '../../src/lib/tmux.js';

describe('detectTerminalApiError', () => {
  it('returns null for empty input', () => {
    expect(detectTerminalApiError('')).toBeNull();
  });
  ...
  it('returns null for ordinary processing output', () => {
    ...
    expect(detectTerminalApiError(pane)).toBeNull();
  });
  ...
  it('detects Kimi-style quota error (the PAN-1015 repro)', () => {
    ...
    const result = detectTerminalApiError(pane);
  ...
  it('detects "You\'ve hit your limit" Anthropic-style quota message', () => {
    const result = detectTerminalApiError("You've hit your limit. Upgrade plan...");
    expect(result?.kind).toBe('quota_exhausted');
  });

  it('detects credit-balance message as quota_exhausted', () => {
    const result = detectTerminalApiError('Error: Your credit balance is too low to access the API.');
    expect(result?.kind).toBe('quota_exhausted');
  });

  it('detects "Please run /login" as login_required', () => {
    const result = detectTerminalApiError('  Please run /login\n❯ ');
    expect(result?.kind).toBe('login_required');
  });

  it('detects API Error: 401 as auth_failed', () => {
    const result = detectTerminalApiError('API Error: 401 Unauthorized');
    expect(result?.kind).toBe('auth_failed');
  });

  it('detects authentication_error as auth_failed', () => {
    const result = detectTerminalApiError('{"error":{"type":"authentication_error","message":"invalid api key"}}');
    expect(result?.kind).toBe('auth_failed');
  });

  it('detects bare 403 as permission_denied when no quota line is present', () => {
    const result = detectTerminalApiError('API Error: 403 Forbidden');
  ...
  it('does NOT match transient retry messages — those belong to the deacon retry path', () => {
    ...
    // terminal failures.
    expect(detectTerminalApiError('API Error: Overloaded')).toBeNull();
    expect(detectTerminalApiError('API Error: Rate limit')).toBeNull();
    expect(detectTerminalApiError('API Error: Timed out')).toBeNull();
    expect(detectTerminalApiError('529 Overloaded')).toBeNull();
    expect(detectTerminalApiError('502 Bad Gateway')).toBeNull();
    expect(detectTerminalApiError('503 Service Unavailable')).toBeNull();
  });
  ...
  it('captures the matched line in raw for diagnostics, truncated to 240 chars', () => {
    ...
    const result = detectTerminalApiError(longLine);
    expect(result).not.toBeNull();
