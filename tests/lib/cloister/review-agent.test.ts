 ...
 *   - parseReviewerTemplate: YAML frontmatter parsing
 *   - resolveReviewerModel: work-type routing with agent/template overrides
 *   - parseReviewSynthesis: REVIEW_RESULT marker extraction from synthesis output
...
describe('parseReviewerTemplate', () => {
  ...
  it('parses model from YAML frontmatter and returns body content', () => {
    ...
    const result = parseReviewerTemplate(templatePath);
    expect(result.model).toBe('claude-opus-4-6');
    expect(result.content).toBe('Review the code for correctness.');
  });

  it('falls back to "sonnet" when frontmatter has no model field', () => {
  ...
  it('throws when template file does not exist', () => {
    ...
    ).toThrow('Reviewer template not found');
  });

  it('throws when template has no YAML frontmatter', () => {
    const templatePath = join(tmpDir, 'bad-template.md');
    writeFileSync(templatePath, 'Just content, no frontmatter.');

    expect(() => parseReviewerTemplate(templatePath)).toThrow('Invalid template format');
  });
});
...
describe('resolveReviewerModel', () => {
  ...
  it('returns defaultModel when agent has no model and role has no work-type mapping', () => {
    // 'synthesis' maps to review:synthesis but getModelId may throw if no config
    // — the function must not throw; it either resolves or falls back
    const model = resolveReviewerModel(
...
describe('parseReviewSynthesis', () => {
  ...
  it('extracts APPROVED result from synthesis.md', () => {
    ...
    const result = parseReviewSynthesis(tmpDir);
    expect(result.success).toBe(true);
    expect(result.reviewResult).toBe('APPROVED');
    expect(result.notes).toBe('Looks good');
  });

  it('extracts CHANGES_REQUESTED result from synthesis.md', () => {
    writeFileSync(join(tmpDir, 'synthesis.md'), [
  ...
  it('returns COMMENTED/failure when synthesis.md is missing', () => {
    const result = parseReviewSynthesis(tmpDir);
  ...
  it('returns COMMENTED/failure when synthesis.md has no result markers', () => {
  ...
  it('collects file references from reviewer output files alongside synthesis', () => {
