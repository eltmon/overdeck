...
const KIMI_NATIVE_OVERFLOW_LINE =
  'API Error: 400 Invalid request: Your request exceeded model token limit: 262144 (requested: 336148)';
...
describe('isContextOverflowTail', () => {
  ...
  it('returns true for kimi-native model token limit errors', () => {
    expect(isContextOverflowTail(['working...', KIMI_NATIVE_OVERFLOW_LINE, '❯ '].join('\n'))).toBe(true);
    expect(isContextOverflowError(new Error(KIMI_NATIVE_OVERFLOW_LINE))).toBe(true);
  });
