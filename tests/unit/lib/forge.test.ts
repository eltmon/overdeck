...
describe('forge adapters', () => {
  ...
  it('finds a merged GitHub artifact through the GitHub App', async () => {
    isGitHubAppConfiguredMock.mockReturnValue(true);
    listPullRequestsForHeadMock.mockReturnValue(Effect.succeed([
      {
        number: 42,
  ...
  it('rejects open GitHub artifacts through the GitHub App', async () => {
    isGitHubAppConfiguredMock.mockReturnValue(true);
    listPullRequestsForHeadMock.mockReturnValue(Effect.succeed([
      {
        number: 42,
  ...
  it('finds a merged GitHub artifact through the gh fallback', async () => {
    execMock.mockResolvedValueOnce({
  ...
  it('finds only merged GitLab artifacts', async () => {
    execMock.mockResolvedValueOnce({
  ...
  it.each(['opened', 'closed'])('rejects GitLab artifacts with state %s', async (state) => {
    execMock.mockResolvedValueOnce({
      stdout: JSON.stringify([
  ...
  it('returns null when no merged artifacts exist', async () => {
