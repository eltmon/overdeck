...
vi.mock('../../../../src/lib/cost-parsers/pi-parser.js', () => ({
  ...
  parsePiSessionCostEventsSync: vi.fn(),
}));
...
import { parsePiSessionCostEventsSync, parsePiSessionSync } from '../../../../src/lib/cost-parsers/pi-parser.js';
...
describe('CostWriter.reconcile — pi source', () => {
  beforeEach(() => {
    ...
    vi.mocked(parsePiSessionCostEventsSync).mockReturnValue([]);
  });
  ...
  it('imports pi assistant usage events and returns the inserted count', async () => {
    ...
    vi.mocked(parsePiSessionCostEventsSync).mockReturnValue(makePiCostEvents(sessionFile));
    ...
    expect(row.requestId).toBe('pi:sess-abc:e1');
    expect(row.sourceFile).toBe(sessionFile);
    expect(row.cost).toBe(0.03);
    expect((insertedValues[1] as Record<string, unknown>).requestId).toBe('pi:sess-abc:e2');
  });
