...
describe('CostWriter.reconcile — ohmypi source', () => {
  ...
  it('reports unpriced ohmypi models while preserving their events', async () => {
    ...
    expect(result).toEqual({ imported: 2, skipped: [{ file: sessionFile, reason: 'unpriced-model' }] });
    expect(insertedValues).toHaveLength(2);
...
describe('CostWriter.reconcile — codex source', () => {
  ...
  it('imports only new codex turns when a rollout file grows', async () => {
    ...
    expect(first).toEqual({ imported: 1, skipped: [] });
    expect(second).toEqual({ imported: 1, skipped: [] });
    expect(insertedValues).toHaveLength(2);
  ...
  it('reports unknown-model codex sessions while preserving their events', async () => {
    ...
    expect(result).toEqual({ imported: 1, skipped: [{ file: rolloutFile, reason: 'unknown-model' }] });
