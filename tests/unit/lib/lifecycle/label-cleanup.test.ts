...
describe('cleanupMergedLabels', () => {
  ...
  describe('GitHub', () => {
    ...
    it('calls gh issue edit to remove workflow labels that exist on the issue', async () => {
      ...
      expect(removeCalls).toEqual([
        expect.stringContaining('--remove-label "in-review"'),
        expect.stringContaining('--remove-label "in-progress"'),
      ]);
      expect(removeCalls.some(c => c.includes('"merge-agent"'))).toBe(false);
    });
