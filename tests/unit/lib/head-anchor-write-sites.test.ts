...
const ALLOWED_WRITE_SITES: AllowedWriteSite[] = [
  allow('src/cli/commands/specialists/done.ts', 'reviewedAtCommit', 'update.reviewedAtCommit = workspaceHead', 'Producer-fed terminal review verdict CLI stamp; verdict and anchor land in one door write (PAN-3847).'),
  ...
  allow('src/lib/cloister/review-verdict-writer.ts', 'reviewedAtCommit', 'reviewedAtCommit: input.evidenceHead', 'Verdict write door persists the evidence anchor when the row is created by the verdict (PAN-3847).'),
  allow('src/lib/cloister/review-verdict-writer.ts', 'reviewedAtCommit', 'reviewedAtCommit: input.evidenceHead', 'Verdict write door lands verdict and anchor together when the row has no verification anchor (PAN-3847).'),
  ...
  allow('src/lib/cloister/verification-runner.ts', 'lastVerifiedCommit', 'lastVerifiedCommit: undefined', 'Interrupted worker recovery invalidates the prior verification anchor before a fresh run (PAN-3814).'),
