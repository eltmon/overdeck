...
const ALLOWED_WRITE_SITES: AllowedWriteSite[] = [
  ...
  allow('src/lib/agents/agent-state.ts', 'roleRunHead', 'roleRunHead: state.roleRunHead', 'Review-artifact context read-through from agent state.'),
  ...
  allow('src/lib/reconstruct/reconstruct-cache.ts', 'roleRunHead', 'roleRunHead: state.roleRunHead || undefined', 'Cache reconstruction from agent state.json.'),
