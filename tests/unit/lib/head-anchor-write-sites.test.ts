...
const ALLOWED_WRITE_SITES: AllowedWriteSite[] = [
  ...
  allow('src/dashboard/server/routes/specialists/legacy-routes.ts', 'reviewedAtCommit', 'update.reviewedAtCommit = headAnchor', 'Pre-verdict evidence-head snapshot feeding recordReviewVerdict (PAN-3512).'),
  ...
  allow('src/lib/cloister/deacon-review-status.ts', 'reviewedAtCommit', 'reviewedAtCommit: artifact.headSha', 'Synthesis-artifact verdict restore stamps the artifact head SHA (PAN-1577 race guard).'),
  ...
  allow('src/lib/cloister/review-verdict-writer.ts', 'reviewedAtCommit', 'reviewedAtCommit: input.evidenceHead as HeadAnchor', 'Verdict write door (PAN-3512) anchors a fresh-classified verdict to its evidence head.'),
