...
describe('verification worker supervisor', () => {
  ...
  it('passes the plan-completion policy to the detached worker', async () => {
    useFixture();
    process.env.VERIFICATION_FIXTURE_ECHO_OPTIONS = '1';

    await expect(runSupervisedVerification(
      'PAN-2844',
      '/tmp/strike-workspace',
      { isRemote: false },
      'test',
      { syncTargetBranch: false, requirePlanCompletion: false },
    )).resolves.toEqual({
      outcome: 'passed',
      options: { syncTargetBranch: false, requirePlanCompletion: false },
    });
  });
