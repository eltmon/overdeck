...
describe('reconcileIdleWorkspaceStacks (PAN-1817)', () => {
  ...
  // PAN-3049 review fix: the prefix must be unconstrained — not just "no
  // prefix or exactly one alphanumeric segment" — so a multi-segment
  // (hyphenated) declared project name is still matched. Exact reproduction
  // from review: my-company-feature-min-901-server-1 was previously invisible.
  it('reaps UI containers under a multi-segment, hyphenated compose project prefix', async () => {
    let nowMs = 1_000_000;
    const stopped: string[][] = [];
    const deps: Partial<IdleStackReaperDeps> = {
      listContainerNames: async () => [
        'my-company-feature-min-901-server-1',
        'my-company-feature-min-901-frontend-1',
      ],
      listComposeContainers: async () => [],
      listSessions: async () => [],
      stopContainers: async (names) => { stopped.push(names); },
    ...
    expect(stopped[0].sort()).toEqual(
      ['my-company-feature-min-901-frontend-1', 'my-company-feature-min-901-server-1'].sort(),
    );
  });

  it('reaps UI containers under an underscore-containing compose project prefix', async () => {
    let nowMs = 1_000_000;
    const stopped: string[][] = [];
    const deps: Partial<IdleStackReaperDeps> = {
      listContainerNames: async () => [
        'my_company-feature-min-901-server-1',
        'my_company-feature-min-901-frontend-1',
      ],
      listComposeContainers: async () => [],
      listSessions: async () => [],
      stopContainers: async (names) => { stopped.push(names); },
    ...
    expect(stopped).toHaveLength(1);
    expect(stopped[0].sort()).toEqual(
      ['my_company-feature-min-901-frontend-1', 'my_company-feature-min-901-server-1'].sort(),
    );
  });
});
