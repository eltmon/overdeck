...
describe('installPreRebaseHook', () => {
  ...
  it('preserves the interpreter of an existing non-POSIX hook', async () => {
    const hooksDir = join(worktreePath, '.test-hooks');
    const hookPath = join(hooksDir, 'pre-rebase');
    const markerPath = join(worktreePath, 'existing-node-hook-ran');
    mkdirSync(hooksDir, { recursive: true });
    writeFileSync(
      hookPath,
      `#!/usr/bin/env node\nrequire('node:fs').writeFileSync(${JSON.stringify(markerPath)}, 'preserved');\n`,
    );
    chmodSync(hookPath, 0o755);
