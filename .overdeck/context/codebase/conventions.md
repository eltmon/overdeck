...
## Planning artifacts (xBRIEF v0.8; PAN-3917 moved them into the repo)
- Everything lives under `.pan/` in the project repo (or the configured plan-home
  repo for polyrepo projects), committed on the feature branch; the retired
  `overdeck-state` branch and `${OVERDECK_HOME}/state/...` paths are never written.
- PRD drafts: `.pan/drafts/<issue-lowercase>.md` (the PRD-first gate accepts either
  case in the workspace; promotion writes the lowercase canonical name).
- Spec: `.pan/specs/<date>-<ISSUE>-<slug>.xbrief.json` — immutable after planning
  except `plan.status`, which `pan plan finalize` flips (`draft → proposed → …`).
- Item status / claims: `.pan/continues/<issue>.xbrief.json`, written only by
  `pan task`. Workspace continue context: `<workspace>/.overdeck/continue.json`.
