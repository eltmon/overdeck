...
- **Agent substrate:** a terminal backend — Herdr (`herdr --session overdeck`,
  v0.9.1 / protocol 22, unix-socket NDJSON API) by default, tmux on the dedicated
  `overdeck` socket (`tmux -L overdeck`) as fallback; per-agent launch state under
  `~/.overdeck/agents/`; harnesses: claude-code, ohmypi, codex, acp/opencode, kimi-code,
  muse. `docs/TERMINAL-BACKENDS.md` is the reference.
- **Issue tracking:** GitHub Issues (`PAN-<n>` = `#<n>` on eltmon/overdeck);
  xBRIEF v0.8 specs and task state live under `.pan/` in the project repo (committed on
  the feature branch) and are exposed through `pan task`.
