...
- **The record plane is gone (PAN-3917).** `src/lib/pan-dir/records.ts`, `projectPipeline()`,
  and `pickNewerPipeline` no longer exist; the PAN-3092 `pipeline.updatedAt` verdict-vs-recency
  hazard went with them. Pipeline membership is now derived only from tracker, forge, git, and
  `.pan/specs` signals (`src/lib/pipeline-membership-gather.ts`); a tracker-closed issue with an
  open PR always resolves `zombie_pr`.
