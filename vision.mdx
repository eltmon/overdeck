...
criterion bubbles to the top of the sequencer rather than P-level and age alone). This is now
implemented: substrate bugs declare affected criteria via the `Flywheel-Affects-Criterion` trailer,
the Flywheel computes a per-bug weight from the 30-day stats snapshot, and both the dashboard badge
and `pan flywheel weights` expose the ranking. The Flywheel and the backlog sequencer prioritize
substrate-hardening epics first: a stable substrate is the prerequisite for everything else on the
path.
