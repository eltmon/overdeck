...
- **Substrate bug** — a defect in Overdeck itself: a broken route, a misbehaving role prompt, a wrong gate condition, a flaky lifecycle transition, a UI that lies about state. Substrate bugs get filed against `eltmon/panopticon-cli` and fixed through the normal Overdeck pipeline.
