#!/usr/bin/env node
/**
 * panopticon launcher — invoked via `npx panopticon` or `panopticon` (after global install).
 *
 * Responsibilities:
 *   1. Verify Node 22+ is running (Electron bundles Node 22).
 *   2. Check that tmux is installed (required for agent terminal sessions).
...
function checkNodeVersion() {
  ...
  if (major < 22) {
    console.error(
      `[panopticon] Node.js 22 or later is required (found ${process.versions.node}).`
    );
...
/** Check that tmux is installed and accessible via PATH. */
function checkTmux() {
  try {
    execFileSync("tmux", ["-V"], { stdio: "pipe" });
  } catch {
    console.error("[panopticon] tmux is required but was not found in PATH.");
...
function checkServerBundle() {
  ...
  if (!existsSync(serverPath)) {
    console.error("[panopticon] Dashboard server bundle not found.");
    console.error(`  Expected: ${serverPath}`);
    console.error("  This package may not have been built correctly.");
    console.error("  Try reinstalling: npm install -g panopticon");
    process.exit(1);
  }
}
...
function resolveElectron() {
  ...
  try {
    ...
    console.error("[panopticon] electron package not found.");
    console.error("  Try reinstalling: npm install -g panopticon");
    process.exit(1);
  }
}
...
function launch() {
  ...
  child.on("error", (err) => {
    console.error("[panopticon] Failed to start:", err.message);
    process.exit(1);
  });
