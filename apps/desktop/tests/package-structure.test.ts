 ...
 * and correct for the `panopticon` npm package.
 */
...
describe("package.json", () => {
  ...
  it("has name 'panopticon'", () => {
    const pkg = readPkg();
    expect(pkg.name).toBe("panopticon");
  });

  it("has a bin entry pointing to bin/panopticon.mjs", () => {
    const pkg = readPkg();
    const bin = pkg.bin as Record<string, string> | undefined;
    expect(bin).toBeDefined();
    expect(bin?.["panopticon"]).toBe("./bin/panopticon.mjs");
  });
...
describe("bin/panopticon.mjs", () => {
  const binPath = Path.join(desktopDir, "bin/panopticon.mjs");
  ...
  it("checks for tmux before launching", () => {
    const content = FS.readFileSync(binPath, "utf8");
