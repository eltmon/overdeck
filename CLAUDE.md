...
- The 60 s Deacon tick is stuck detection and pipeline progress only (14 steps); the 29 housekeeping chores run from `src/lib/cloister/housekeeping-scheduler.ts` on fast/hourly/daily cadences with durable due-times in `~/.overdeck/deacon/housekeeping.json` (PAN-3894). `src/lib/cloister/patrol-registry.ts` is the one list both read; `pan doctor` prints it. [docs/DEACON-HEALTH-MONITORING.md](docs/DEACON-HEALTH-MONITORING.md)
